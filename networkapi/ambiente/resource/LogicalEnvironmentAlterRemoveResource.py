    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(id_logicalenvironment):
                self.log.error(u'The id_logicalenvironment parameter is not a valid value: %s.', id_logicalenvironment)
                raise InvalidValueError(None, 'id_logicalenvironment', id_logicalenvironment)
            ...
            with distributedlock(LOCK_LOGICAL_ENVIRONMENT % id_logicalenvironment):
                ...
                try:
                    if loc_env.nome.lower() != name.lower():
                        ...
                        raise AmbienteLogicoNameDuplicatedError(None, u'Já existe um Ambiente Lógico com o valor name %s.' % name)
                ...
                try:
                    ...
                    self.log.error(u'Failed to update the Logical Environment.')
                    raise AmbienteError(e, u'Failed to update the Logical Environment.')
            ...
            with distributedlock(LOCK_LOGICAL_ENVIRONMENT % id_logicalenvironment):
                ...
                try:
                    ...
                    if loc_env.ambiente_set.count() > 0:
                        raise AmbienteLogicoUsedByEnvironmentError(None, u"O Ambiente Lógico %s tem ambiente associado." % loc_env.id)
                    ...
                    self.log.error(u'Failed to remove the Logical Environment.')
                    raise AmbienteError(e, u'Failed to remove the Logical Environment.')
