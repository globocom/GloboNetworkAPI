...
from networkapi.util import is_valid_string_maxsize, is_valid_string_minsize, is_valid_regex
...
class TipoAcessoResource(RestResource):
    ...
    def handle_get(self, request, user, *args, **kwargs):
        ...
        try:
            if not has_perm(
                    user,
                    AdminPermission.ACCESS_TYPE_MANAGEMENT,
        ...
        try:
            ...
            if networkapi_map is None:
                return self.response_error(
                    3,
                    u'There is no networkapi tag in request XML.')

            tipo_acesso_map = networkapi_map.get('tipo_acesso')
            if tipo_acesso_map is None:
                return self.response_error(
                    3,
                    u'There is no tipo_acesso tag in request XML.')
            ...
            if not is_valid_string_minsize(
                ...
                protocol,
                45) or not is_valid_regex(
                protocol,
                    r'^[- a-zA-Z0-9]+$'):
                self.log.error(
                    u'Parameter protocol is invalid. Value: %s',
                    protocol)
            ...
            try:
                ...
                raise DuplicateProtocolError(
                    None,
                    u'Access Type with protocol %s already exists' %
                    protocol)
            ...
            return self.response(
                dumps_networkapi({'tipo_acesso': {'id': access_type.id}}))
            ...
            with distributedlock(LOCK_TYPE_ACCESS % tipo_acesso_id):
                ...
                if tpa.equipamentoacesso_set.count() > 0:
                    ...
                    raise AccessTypeUsedByEquipmentError(
                        None,
                        u'Access Type in use by equipment.')
