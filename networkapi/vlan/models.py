    ...
    def get_by_pk(cls, id):
        try:
            ...
            raise NetworkTypeNotFoundError(
                e,
                u'There is no network type with pk = %s.' %
                id)
    ...
    def get_by_name(cls, name):
        try:
            ...
            if len(tipos) == 0:
                raise NetworkTypeNotFoundError(
                    None,
                    u'There is no network type with name = %s.' %
                    name)
            return tipos[0]
...
class Vlan(BaseModel):
    ...
    def get_by_pk(self, id):
        ...
        try:
            ...
            raise VlanNotFoundError(
                e,
                u'Dont there is a Vlan by pk = %s.' %
                id)
        except OperationalError as e:
    ...
    def get_vlan_by_acl(self, acl_file):
        try:
            ...
            raise VlanACLDuplicatedError(
                None,
                'uThere is already an Vlan with the Acl - Ipv4 = %s.' %
                acl_file)
    ...
    def get_vlan_by_acl_v6(self, acl_file_v6):
        try:
            ...
            raise VlanACLDuplicatedError(
                None,
                'uThere is already an Vlan with the Acl - Ipv6 = %s.' %
                acl_file_v6)
        except VlanACLDuplicatedError as e:
            raise VlanACLDuplicatedError(e, e.message)
        except ObjectDoesNotExist as e:
        ...
        try:
            return Vlan.objects.filter(
                num_vlan__range=(
                    min_num,
                    max_num),
                ambiente__id=environment_id).values_list(
                'num_vlan',
                flat=True).distinct().order_by('num_vlan')
        except Exception as e:
    ...
    def calculate_vlan_number(self, min_num, max_num, list_available=False):
        ...
        id_equipamentos = EquipamentoAmbiente.objects.filter(
            equipamento__tipo_equipamento__id__in=[
                1,
                3,
                5],
            ambiente__id=self.ambiente_id).values_list(
            'equipamento',
            flat=True)
        ...
        if self.num_vlan is None:
            ...
            if self.num_vlan is None:
                raise VlanNumberNotAvailableError(
                    None,
                    u'Number VLAN unavailable for environment %d.' %
                    self.ambiente.id)
    ...
    def create(
        ...
        if self.existVlanNameInEnvironment():
            raise VlanNameDuplicatedError(
                None,
                'VLAN com nome duplicado dentro do ambiente.')
        ...
        if self.num_vlan is None:
            ...
            if self.num_vlan is None:
                raise VlanNumberNotAvailableError(
                    None,
                    u'Não existe número de VLAN disponível para o ambiente %d.' %
                    self.ambiente.id)
        ...
        if (len(vlan_address) == 0):
            raise VlanNetworkAddressNotAvailableError(
                None,
                u'Não existe endereço de rede disponível para o cadastro da VLAN.')
    ...
    def get_by_number_and_environment(self, number, environment):
        ...
        try:
            return Vlan.objects.filter(
                num_vlan=number,
                ambiente=environment).uniqueResult()
        except ObjectDoesNotExist as e:
            raise VlanNotFoundError(
                e,
                u'Dont there is a Vlan by number = %s.' %
                number)
        except OperationalError as e:
            self.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(
    ...
    def get_by_name(self, name):
        ...
        try:
            ...
            raise VlanNotFoundError(
                e,
                u'Dont there is a Vlan by name = %s.' %
                name)
        except OperationalError as e:
            self.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(
    ...
    def insert_vlan(self, authenticated_user):
        ...
        try:
            ...
            ambiente = "%s - %s - %s" % (self.ambiente.divisao_dc.nome,
                                         self.ambiente.ambiente_logico.nome,
                                         self.ambiente.grupo_l3.nome)
            raise VlanNumberEnvironmentNotAvailableError(
                None, "Já existe uma VLAN cadastrada com o número %s no ambiente %s" %
                (self.num_vlan, ambiente))
        ...
        for env in envs:
            for vlan in env.vlan_set.all():
                if int(vlan.num_vlan) == int(self.num_vlan):
                    if self.ambiente.filter_id is None or vlan.ambiente.filter_id is None or int(
                            vlan.ambiente.filter_id) != int(
                            self.ambiente.filter_id):
                        raise VlanNumberEnvironmentNotAvailableError(
                            None,
                            "Já existe uma VLAN cadastrada com o número %s com um equipamento compartilhado nesse ambiente" %
                            (self.num_vlan))
        ...
        try:
            ...
            raise VlanNameDuplicatedError(
                None,
                "Já existe uma VLAN cadastrada com o nome %s" %
                self.nome)
        except VlanNotFoundError:
            pass
            ...
            if old_env.filter is not None:
                if self.check_env_shared_equipment(old_env):
                    if self.ambiente.filter_id != old_env.filter_id:
                        raise VlanNumberEnvironmentNotAvailableError(
                            None,
                            "Um dos equipamentos associados com o ambiente desta Vlan também está associado com outro ambiente que tem uma rede com a mesma faixa, adicione filtros nos ambientes se necessário.")
    ...
    def check_env_shared_equipment(self, old_env):
        ...
        for i in range(0, len(nets_ipv4)):
            ...
            if verify_subnet_and_equip(
                    nets_ipv4_aux,
                    network_ip_verify,
                    'v4',
                    net,
                    nets_ipv4[i].get('vlan_env')):
        ...
        for i in range(0, len(nets_ipv6)):
            ...
            ip = "%s:%s:%s:%s:%s:%s:%s:%s/%d" % (net.block1,
                                                 net.block2,
                                                 net.block3,
                                                 net.block4,
                                                 net.block5,
    ...
    def delete(self, authenticated_user):
        ...
        try:
            ...
            raise IpCantBeRemovedFromVip(
                cause,
                "Esta Vlan possui uma Rede com Requisição Vip apontando para ela, e não pode ser excluída")
