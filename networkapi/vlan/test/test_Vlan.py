...
class VlanConfigTest(BasicTestCase):
    ...
    def search_attr(self, dicts):
        ...
        for (k, v) in dicts.iteritems():
            mock[k] = v
        response = self.client_autenticado().postXML(self.URL_SEARCH, {self.XML_KEY: mock})
        return response
    ...
    def allocate_invalid(self, key, attr):
        ...
        mock[key] = attr
        response = self.client_autenticado().postXML(self.URL_ALLOCATE_VLAN, {self.XML_KEY: mock})
        return response

    def allocate_invalid_v6(self, key, attr):
        mock = self.mock_valid_allocate_v6()
        mock[key] = attr
        response = self.client_autenticado().postXML(self.URL_ALLOCATE_VLAN_V6, {self.XML_KEY: mock})
        return response

    def allocate_invalid_no_net(self, key, attr):
        mock = self.mock_valid_allocate_no_net()
        mock[key] = attr
        response = self.client_autenticado().postXML(self.URL_ALLOCATE_NO_NETWORK, {self.XML_KEY: mock})
        return response
...
class VlanConsultationTest(VlanConfigTest, ConsultationTest):
    ...
    def test_get_by_id_with_network_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.ID_VALID)
        valid_response(response)
        content = valid_content(response, self.XML_KEY)
    ...
    def test_get_by_id_with_network_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_get_by_id_with_network_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_get_by_id_with_network_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_get_by_id_with_network_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_get_by_id_with_network_empty(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID_WITH_NETWORKS % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_get_by_environment_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.ID_ENVIRONMENT_VALID)
        valid_response(response)
        content = valid_content(response, self.XML_KEY)
    ...
    def test_get_by_environment_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.ID_NONEXISTENT)
        self._not_found(response, self.CODE_ERROR_ENVIRONMENT_NOT_FOUND)

    def test_get_by_environment_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_get_by_environment_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_get_by_environment_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_get_by_environment_empty(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ENVIRONMENT % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class VlanValidateTest(VlanConfigTest, AttrTest):
    ...
    def test_validate_v4_valid(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response)

    def test_validate_v4_no_permission(self):
        response = self.client_no_permission().put(self.URL_VALIDATE_ACL_v4 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_v4_no_write_permission(self):
        response = self.client_no_write_permission().put(self.URL_VALIDATE_ACL_v4 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_v4_nonexistent(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_validate_v4_negative(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_validate_v4_letter(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_validate_v4_zero(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_validate_v4_empty(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v4 % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_validate_v6_valid(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response)

    def test_validate_v6_no_permission(self):
        response = self.client_no_permission().put(self.URL_VALIDATE_ACL_v6 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_v6_no_write_permission(self):
        response = self.client_no_write_permission().put(self.URL_VALIDATE_ACL_v6 % self.ID_VLAN_FOR_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_v6_nonexistent(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_validate_v6_negative(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_validate_v6_letter(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_validate_v6_zero(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_validate_v6_empty(self):
        response = self.client_autenticado().put(self.URL_VALIDATE_ACL_v6 % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class VlanInvalidateTest(VlanConfigTest, AttrTest):
    ...
    def test_invalidate_v4_valid(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response)

    def test_invalidate_v4_no_permission(self):
        response = self.client_no_permission().put(self.URL_INVALIDATE_ACL_v4 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_invalidate_v4_no_write_permission(self):
        response = self.client_no_write_permission().put(self.URL_INVALIDATE_ACL_v4 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_invalidate_v4_nonexistent(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_invalidate_v4_negative(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v4_letter(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v4_zero(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v4_empty(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v4 % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v6_valid(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response)

    def test_invalidate_v6_no_permission(self):
        response = self.client_no_permission().put(self.URL_INVALIDATE_ACL_v6 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_invalidate_v6_no_write_permission(self):
        response = self.client_no_write_permission().put(self.URL_INVALIDATE_ACL_v6 % self.ID_VLAN_FOR_INVALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_invalidate_v6_nonexistent(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_invalidate_v6_negative(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v6_letter(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v6_zero(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_invalidate_v6_empty(self):
        response = self.client_autenticado().put(self.URL_INVALIDATE_ACL_v6 % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class VlanAttrNumberCheckTest(VlanConfigTest, ConsultationTest):
    ...
    def test_check_valid_number(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENV_VALID_TO_ALLOCATE_V6, self.NUM_VLAN_VALID, self.IS_INSERT))
        valid_response(response)
        content = valid_content(response, 'has_numbers_availables')
        assert content == 'True'

    def test_check_number_no_permission(self):
        response = self.client_no_permission().delete(self.URL_DEALLOCATE % self.ID_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_check_invalid_number(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENV_VALID_TO_ALLOCATE_V6, self.NUM_VLAN_INVALID, self.IS_INSERT))
        valid_response(response)
        content = valid_content(response, 'has_numbers_availables')
        assert content == 'False'

    def test_check_valid_number_existent(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.NUM_VLAN_IN_ENV_1, self.IS_INSERT))
        valid_response(response)
        content = valid_content(response, 'has_numbers_availables')
        # Returned true, this means that dont need confirmation, the system will return exception when try to insert vlan
        assert content == 'True'

    def test_check_number_env_nonexistent(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_NONEXISTENT, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response, CodeError.ENVIRONMENT_NOT_FOUND)

    def test_check_number_env_negative(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.NEGATIVE_ATTR, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_env_letter(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.LETTER_ATTR, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_env_zero(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ZERO_ATTR, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_env_empty(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.EMPTY_ATTR, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_env_none(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.NONE_ATTR, self.NUM_VLAN_VALID, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_negative(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.NEGATIVE_ATTR, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_letter(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.LETTER_ATTR, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_zero(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.ZERO_ATTR, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_empty(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.EMPTY_ATTR, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_none(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.NONE_ATTR, self.IS_INSERT))
        self._attr_invalid(response)

    def test_check_number_id_vlan_nonexistent(self):
        response = self.client_autenticado().get(self.URL_CHECK_NUMBER % (self.ID_ENVIRONMENT_VALID, self.NUM_VLAN_VALID, self.ID_NONEXISTENT))
...
class VlanAttrAmbienteTest(VlanConfigTest, AttrTest):
    ...
    def test_alter_nonexistent(self):
        self.alter_attr_invalid(self.ID_NONEXISTENT, CodeError.ENVIRONMENT_NOT_FOUND)
...
class VlanAttrAclFileNamev6Test(VlanConfigTest, AttrTest):
    ...
    def test_alter_duplicated(self):
        self.alter_attr_invalid('ACLVLANVALIDAV6', CodeError.VLAN_ACL_DUPLICATE)
...
class VlanDeallocateTest(VlanConfigTest, AttrTest):
    ...
    def test_deallocate_valid(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.ID_REMOVE_VALID)
        valid_response(response)
    ...
    def test_deallocate_no_write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_DEALLOCATE % self.ID_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_deallocate_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_deallocate_negative(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_deallocate_letter(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_deallocate_zero(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_deallocate_empty(self):
        response = self.client_autenticado().delete(self.URL_DEALLOCATE % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class VlanSearchTest(VlanConfigTest):
    ...
    def test_search_no_permission(self):
        mock = self.mock_to_search()
        response = self.client_no_permission().postXML(self.URL_SEARCH, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_search_no_read_permission(self):
        mock = self.mock_to_search()
        response = self.client_no_read_permission().postXML(self.URL_SEARCH, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class VlanSearchAttrsTest(VlanConfigTest, AttrTest):
    ...
    def test_search_exact_invalid(self):
        response = self.search_attr({'nome': 'VLANVALIDA', 'exato': string_generator(4)})
        self._attr_invalid(response)
    ...
    def test_search_net_subnet_valid(self):
        response = self.search_attr({'rede': '192.168.20.0/24', 'subrede': self.ZERO_ATTR})
        valid_response(response)
        valid_content(response, self.XML_KEY)

    def test_search_net_subnet_valid_v6(self):
        response = self.search_attr({'rede': 'ffab:cdef:ffff:ffff:0000:0000:0000:0000/112', 'subrede': self.ZERO_ATTR})
        valid_response(response)
        valid_content(response, self.XML_KEY)
    ...
    def test_search_net_subnet_negative(self):
        response = self.search_attr({'rede': '192.168.20.0/24', 'subrede': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_search_net_subnet_letter(self):
        response = self.search_attr({'rede': '192.168.20.0/24', 'subrede': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_search_net_invalid_subnet(self):
        response = self.search_attr({'rede': '192.168.20.300/24', 'subrede': 0})
        self._attr_invalid(response)
    ...
    def test_composed_name_number_letter(self):
        response = self.search_attr({'numero': self.LETTER_ATTR, 'nome': 'VLANVALIDA'})
        self._attr_invalid(response)
...
class VlanRemoveScriptTest(VlanConfigTest, AttrTest):
    ...
    def teste_remove_script_valid_active(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.ID_REMOVE_SCRIPT_ACTIVE)
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0

    def teste_remove_script_valid_inative(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.ID_VALID)
        self._attr_invalid(response, CodeError.VLAN_INACTIVE)

    def test_remove_script_valid(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.ID_REMOVE_SCRIPT_VALID)
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0

    def test_remove_script_no_permission(self):
        response = self.client_no_permission().delete(self.URL_REMOVE_SCRIPT % self.ID_REMOVE_SCRIPT_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_script_no_write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_REMOVE_SCRIPT % self.ID_REMOVE_SCRIPT_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_script_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_remove_script_negative(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_remove_script_letter(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_remove_script_zero(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_remove_script_empty(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_SCRIPT % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class VlanListPermissionTest(VlanConfigTest, AttrTest):
    ...
    def test_list_permission_valid(self):
        mock = self.mock_list_permission()
        response = self.client_autenticado().putXML(self.URL_LIST_PERMISSION, {'equipamento': mock})
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0
    ...
    def test_list_permission_no_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_permission().putXML(self.URL_LIST_PERMISSION, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_list_permission_no_read_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_read_permission().putXML(self.URL_LIST_PERMISSION, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_list_permission_nonexistent_equipment(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome", 'EquipmentDoesNotExist')
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_list_permission_empty_equipment(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_list_permission_none_equipment(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome", self.NONE_ATTR)
        self._attr_invalid(response)

    def test_list_permission_nonexistent_interface(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome_interface", 'InterfaceDoesNotExist')
        self._not_found(response, CodeError.INTERFACE_NOT_FOUND)

    def test_list_permission_empty_interface(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome_interface", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_list_permission_none_interface(self):
        response = self.permission_script(self.URL_LIST_PERMISSION, "nome_interface", self.NONE_ATTR)
        self._attr_invalid(response)
...
class VlanCheckPermissionTest(VlanConfigTest, AttrTest):
    ...
    def test_check_permission_valid(self):
        mock = self.mock_list_permission()
        response = self.client_autenticado().putXML(self.URL_CHECK_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0
    ...
    def test_check_permission_no_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_permission().putXML(self.URL_CHECK_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_check_permission_no_read_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_read_permission().putXML(self.URL_CHECK_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_check_permission_id_nonexistent(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.VLAN_NOT_FOUND)

    def test_check_permission_id_negative(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_check_permission_id_zero(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_check_permission_id_empty(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_check_permission_id_none(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.NONE_ATTR)
        self._attr_invalid(response)

    def test_check_permission_id_letter(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_check_permission_nonexistent_equipment(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome", 'EquipmentDoesNotExist')
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_check_permission_empty_equipment(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_check_permission_none_equipment(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome", self.NONE_ATTR)
        self._attr_invalid(response)

    def test_check_permission_nonexistent_interface(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome_interface", 'InterfaceDoesNotExist')
        self._not_found(response, CodeError.INTERFACE_NOT_FOUND)

    def test_check_permission_empty_interface(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome_interface", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_check_permission_none_interface(self):
        response = self.permission_script(self.URL_CHECK_PERMISSION % self.ID_VALID, "nome_interface", self.NONE_ATTR)
        self._attr_invalid(response)
...
class VlanAddPermissionTest(VlanConfigTest, AttrTest):
    ...
    def test_add_permission_valid(self):
        mock = self.mock_list_permission()
        response = self.client_autenticado().putXML(self.URL_ADD_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0
    ...
    def test_add_permission_no_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_permission().putXML(self.URL_ADD_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_add_permission_no_read_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_read_permission().putXML(self.URL_ADD_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_add_permission_id_nonexistent(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.VLAN_NOT_FOUND)

    def test_add_permission_id_negative(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_add_permission_id_zero(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_add_permission_id_empty(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_add_permission_id_none(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.NONE_ATTR)
        self._attr_invalid(response)

    def test_add_permission_id_letter(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_add_permission_nonexistent_equipment(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome", 'EquipamentoDoesNotExist')
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_add_permission_empty_equipment(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_add_permission_none_equipment(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome", self.NONE_ATTR)
        self._attr_invalid(response)

    def test_add_permission_nonexistent_interface(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome_interface", 'InterfaceDoesNotExist')
        self._not_found(response, CodeError.INTERFACE_NOT_FOUND)

    def test_add_permission_empty_interface(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome_interface", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_add_permission_none_interface(self):
        response = self.permission_script(self.URL_ADD_PERMISSION % self.ID_VALID, "nome_interface", self.NONE_ATTR)
        self._attr_invalid(response)
...
class VlanRemovePermissionTest(VlanConfigTest, AttrTest):
    ...
    def test_remove_permission_valid(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID)
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0
    ...
    def test_remove_permission_no_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_permission().putXML(self.URL_REMOVE_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_permission_no_read_permission(self):
        mock = self.mock_list_permission()
        response = self.client_no_read_permission().putXML(self.URL_REMOVE_PERMISSION % self.ID_VALID, {'equipamento': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_permission_id_nonexistent(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.VLAN_NOT_FOUND)

    def test_remove_permission_id_negative(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_id_zero(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_id_empty(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_id_none(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.NONE_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_id_letter(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_nonexistent_equipment(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome", "EquipmentDoesNotExist")
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_remove_permission_empty_equipment(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_none_equipment(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome", self.NONE_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_nonexistent_interface(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome_interface", "InterfaceDoesNotExist")
        self._not_found(response, CodeError.INTERFACE_NOT_FOUND)

    def test_remove_permission_empty_interface(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome_interface", self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_remove_permission_none_interface(self):
        response = self.permission_script(self.URL_REMOVE_PERMISSION % self.ID_VALID, "nome_interface", self.NONE_ATTR)
        self._attr_invalid(response)
...
class VlanGetByIdConsultationTest(VlanConfigTest, ConsultationTest):
    ...
    def test_get_by_id_valid_without_nets(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID % self.ID_VLAN_WITHOUT_NET)
    ...
    def test_get_by_id_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_ID % self.ID_VALID_WITH_NET)
        valid_response(response)
...
class VlanAllocateTest(VlanConfigTest, AttrTest):
    ...
    def test_allocate_no_permission(self):
        mock = self.mock_valid_allocate()
        response = self.client_no_permission().postXML(self.URL_ALLOCATE_VLAN, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_allocate_no_write_permission(self):
        mock = self.mock_valid_allocate()
        response = self.client_no_write_permission().postXML(self.URL_ALLOCATE_VLAN, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_allocate_invalid_vip_env_nonexistent(self):
        response = self.allocate_invalid('id_ambiente_vip', self.ID_NONEXISTENT)
...
class VlanAllocateV6(VlanConfigTest, AttrTest):
    ...
    def test_allocate_v6_no_permission(self):
        mock = self.mock_valid_allocate_v6()
        response = self.client_no_permission().postXML(self.URL_ALLOCATE_VLAN_V6, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_allocate_v6_no_write_permission(self):
        mock = self.mock_valid_allocate_v6()
        response = self.client_no_write_permission().postXML(self.URL_ALLOCATE_VLAN_V6, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_allocate_v6_invalid_env_nonexistent(self):
        response = self.allocate_invalid_v6('id_environment', self.ID_NONEXISTENT)
        self._attr_invalid(response, CodeError.ENVIRONMENT_NOT_FOUND)

    def test_allocate_v6_invalid_env_negative(self):
        response = self.allocate_invalid_v6('id_environment', self.NEGATIVE_ATTR)
        self._attr_invalid(response)
    ...
    def test_allocate_v6_invalid_nettype_nonexistent(self):
        response = self.allocate_invalid_v6('id_network_type', self.ID_NONEXISTENT)
        self._attr_invalid(response, CodeError.NETWORK_TYPE_NOT_FOUND)

    def test_allocate_v6_invalid_nettype_negative(self):
        response = self.allocate_invalid_v6('id_network_type', self.NEGATIVE_ATTR)
        self._attr_invalid(response)
    ...
    def test_allocate_v6_invalid_nettype_letter(self):
        response = self.allocate_invalid_v6('id_network_type', self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_allocate_v6_invalid_vip_env_nonexistent(self):
        response = self.allocate_invalid_v6('id_environment_vip', self.ID_NONEXISTENT)
        self._attr_invalid(response, CodeError.ENV_VIP_NOT_FOUND)

    def test_allocate_v6_invalid_vip_env_negative(self):
        response = self.allocate_invalid_v6('id_environment_vip', self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_allocate_v6_invalid_vip_env_letter(self):
        response = self.allocate_invalid_v6('id_environment_vip', self.LETTER_ATTR)
        self._attr_invalid(response)
    ...
    def test_allocate_v6_invalid_description_maxsize(self):
        response = self.allocate_invalid_v6('description', string_generator(201))
        self._attr_invalid(response)
...
class VlanAllocateNoNetworkTest(VlanConfigTest, AttrTest):
    ...
    def test_allocate_no_net_no_permission(self):
        mock = self.mock_valid_allocate_no_net()
        response = self.client_no_permission().postXML(self.URL_ALLOCATE_NO_NETWORK, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_allocate_no_net_no_write_permission(self):
        mock = self.mock_valid_allocate_no_net()
        response = self.client_no_write_permission().postXML(self.URL_ALLOCATE_NO_NETWORK, {self.XML_KEY: mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_allocate_no_net_invalid_env_nonexistent(self):
        response = self.allocate_invalid_no_net('environment_id', self.ID_NONEXISTENT)
        self._attr_invalid(response, CodeError.ENVIRONMENT_NOT_FOUND)

    def test_allocate_no_net_invalid_env_negative(self):
        response = self.allocate_invalid_no_net('environment_id', self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_allocate_no_net_invalid_env_empty(self):
        response = self.allocate_invalid_no_net('environment_id', self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_allocate_no_net_invalid_env_none(self):
        response = self.allocate_invalid_no_net('environment_id', self.NONE_ATTR)
        self._attr_invalid(response)

    def test_allocate_no_net_invalid_env_letter(self):
        response = self.allocate_invalid_no_net('environment_id', self.LETTER_ATTR)
        self._attr_invalid(response)
    ...
    def test_allocate_no_net_invalid_description_maxsize(self):
        response = self.allocate_invalid_no_net('description', string_generator(201))
        self._attr_invalid(response)

    def test_allocate_no_net_invalid_description_minsize(self):
        response = self.allocate_invalid_no_net('description', string_generator(2))
        self._attr_invalid(response)
...
class VlanCreateScriptTest(VlanConfigTest, AttrTest):
    ...
    def test_create_script_valid(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.ID_VALID, {'vlan': None})
        valid_response(response)
        content = valid_content(response, 'sucesso')
        assert int(content['codigo']) == 0

    def test_create_script_no_permission(self):
        response = self.client_no_permission().putXML(self.URL_CREATE_SCRIPT % self.ID_VALID, {'vlan': None})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_no_write_permission(self):
        response = self.client_no_write_permission().putXML(self.URL_CREATE_SCRIPT % self.ID_VALID, {'vlan': None})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_nonexistent(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.ID_NONEXISTENT, {'vlan': None})
        self._not_found(response)

    def test_create_script_negative(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.NEGATIVE_ATTR, {'vlan': None})
        self._attr_invalid(response)

    def test_create_script_letter(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.LETTER_ATTR, {'vlan': None})
        self._attr_invalid(response)

    def test_create_script_zero(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.ZERO_ATTR, {'vlan': None})
        self._attr_invalid(response)

    def test_create_script_empty(self):
        response = self.client_autenticado().putXML(self.URL_CREATE_SCRIPT % self.EMPTY_ATTR, {'vlan': None})
        self._attr_invalid(response)
...
class VlanCreateNetV4ScriptTest(VlanConfigTest, AttrTest):
    ...
    def test_create_script_valid(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.ID_NETV4_CREATE_SCRIPT}})
        valid_response(response)
        content = valid_content(response, 'sucesso')
    ...
    def test_create_script_no_permission(self):
        response = self.client_no_permission().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.ID_NETV4_CREATE_SCRIPT}})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_no_write_permission(self):
        response = self.client_no_write_permission().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.ID_VALID}})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_nonexistent(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.ID_NONEXISTENT}})
        self._not_found(response)

    def test_create_script_negative(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.NEGATIVE_ATTR}})
        self._attr_invalid(response)

    def test_create_script_letter(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.LETTER_ATTR}})
        self._attr_invalid(response)

    def test_create_script_zero(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.ZERO_ATTR}})
        self._attr_invalid(response)

    def test_create_script_empty(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET4, {self.XML_KEY: {'id_network_ip': self.EMPTY_ATTR}})
        self._attr_invalid(response)
...
class VlanCreateNetV6ScriptTest(VlanConfigTest, AttrTest):
    ...
    def test_create_script_valid(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.ID_NETV6_CREATE_SCRIPT}})
        valid_response(response)
        content = valid_content(response, 'sucesso')
        vlan_success = content['vlan']
        net_success = content['network']
    ...
    def test_create_script_no_permission(self):
        response = self.client_no_permission().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.ID_NETV6_CREATE_SCRIPT}})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_no_write_permission(self):
        response = self.client_no_write_permission().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.ID_VALID}})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_script_nonexistent(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.ID_NONEXISTENT}})
        self._not_found(response)

    def test_create_script_negative(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.NEGATIVE_ATTR}})
        self._attr_invalid(response)

    def test_create_script_letter(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.LETTER_ATTR}})
        self._attr_invalid(response)

    def test_create_script_zero(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.ZERO_ATTR}})
        self._attr_invalid(response)

    def test_create_script_empty(self):
        response = self.client_autenticado().postXML(self.URL_CREATE_SCRIPT_NET6, {self.XML_KEY: {'id_network_ip': self.EMPTY_ATTR}})
        self._attr_invalid(response)
