...
class NetworkTypeResource(RestResource):
    ...
    def handle_get(self, request, user, *args, **kwargs):
        ...
        try:
            if not has_perm(
                    user,
                    AdminPermission.NETWORK_TYPE_MANAGEMENT,
        ...
        try:
            ...
            if networkapi_map is None:
                return self.response_error(
                    3,
                    u'There is no networkapi tag from request XML.')
            ...
            if net_type_map is None:
                return self.response_error(
                    3,
                    u'There is no tipo_rede tag from request XML.')
            ...
            try:
                ...
                raise NetworkTypeNameDuplicatedError(
                    None,
                    u'Network type with name %s already exist' %
                    net_type.tipo_rede)
    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if net_type_map is None:
                return self.response_error(
                    3,
                    u'There is no net_type tag from request XML.')
            ...
            with distributedlock(LOCK_TYPE_NETWORK % id_net_type):
                ...
                try:
                    if name.lower() != net_type.tipo_rede.lower():
                        ...
                        raise NetworkTypeNameDuplicatedError(
                            None,
                            u'Network type with name %s already exists' %
                            name)
                except NetworkTypeNotFoundError:
                    pass
    ...
    def handle_delete(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            with distributedlock(LOCK_TYPE_NETWORK % id_net_type):
                ...
                if net_type.networkipv4_set.count(
                ) > 0 or net_type.networkipv6_set.count() > 0:
                    self.log.error(u'Network type used by network.')
                    raise NetTypeUsedByNetworkError(
                        None,
                        u'Network type used by network.')
