    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_string_minsize(name, 3) or not is_valid_string_maxsize(name, 100):
                self.log.error(u'Parameter %s is invalid. Value: %s.', 'name', name)
            ...
            try:
                ...
                raise NetworkTypeNameDuplicatedError(None, u'Network type with name %s already exist' % net_type.tipo_rede)
    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(id_net_type):
                self.log.error(u'Parameter %s is invalid. Value: %s.', 'id_net_type', id_net_type)
            ...
            with distributedlock(LOCK_TYPE_NETWORK % id_net_type):
                ...
                try:
                    if name.lower() != net_type.tipo_rede.lower():
                        ...
                        raise NetworkTypeNameDuplicatedError(None, u'Network type with name %s already exists' % name)
                except NetworkTypeNotFoundError:
                    pass
        ...
        try:
            ...
            with distributedlock(LOCK_TYPE_NETWORK % id_net_type):
                ...
                if net_type.networkipv4_set.count() > 0 or net_type.networkipv6_set.count() > 0:
                    ...
                    raise NetTypeUsedByNetworkError(None, u'Network type used by network.')
