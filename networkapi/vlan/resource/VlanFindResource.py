...
def break_network(network):
    ...
    if version == IP_VERSION.IPv4[0]:
        network_str = blocks[0] + "." + blocks[1] + "." + blocks[2] + "." + blocks[3]
        if blocks[4] != "0":
        ...
        network_str = blocks[0] + ":" + blocks[1] + ":" + blocks[2] + ":" + blocks[3] + ":" + blocks[4] + ":" + blocks[5] + ":" + blocks[6] + ":" + blocks[7]
        if blocks[8] != "0":
...
def prepares_network(vlan, half):
    ...
    if not half:
        ...
        vlan_dict["ambiente_name"] = vlan.ambiente.divisao_dc.nome + " - " + vlan.ambiente.ambiente_logico.nome + " - " + vlan.ambiente.grupo_l3.nome
    
    vlan_dict["is_more"] = False
    netv4_itens = []
    for netv4 in vlan.networkipv4_set.all():
        net_dict = dict()
        net = str(netv4.oct1) + "." + str(netv4.oct2) + "." + str(netv4.oct3) + "." + str(netv4.oct4) + "/" + str(netv4.block)
    ...
    for netv6 in vlan.networkipv6_set.all():
        net_dict = dict()
        net = str(netv6.block1) + ":" + str(netv6.block2) + ":" + str(netv6.block3) + ":" + str(netv6.block4) + ":" + str(netv6.block5) + ":" + str(netv6.block6) + ":" + str(netv6.block7) + ":" + str(netv6.block8) + "/" + str(netv6.block)
    ...
    if (len(netv4_itens) > 1):
        if(vlan_dict["is_more"] == True) or (len(netv4_itens) > 3):
            vlan_dict["more_than_three"] = True
    if (len(netv6_itens) > 1):
        if(vlan_dict["is_more"] == True) or (len(netv6_itens) > 3):
            vlan_dict["more_than_three"] = True
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_equal_zero_param(ip_version):
                ...
                if ip_version == "0":
                    vlans = vlans.filter(Q(networkipv6__isnull=True)|Q(networkipv4__isnull=False))
                elif ip_version == "1":
                    vlans = vlans.filter(Q(networkipv4__isnull=True)|Q(networkipv6__isnull=False))
                ...
                if not (subnet == "0" or subnet == "1"):
                    ...
                    if subnet == "0":
                        ...
                        if version == IP_VERSION.IPv4[0]:
                            ...
                            vlans = vlans.filter(oct1 & oct2 & oct3 & oct4 & blk)
                        else:
                            # Network IP v6
                            oct1 = Q()
                            oct2 = Q()
                            ...
                            vlans = vlans.filter(oct1 & oct2 & oct3 & oct4 & oct5 & oct6 & oct7 & oct8 & blk)
                    # If subnet is 1
                    else:
                        ...
                        for vlan in itens:
                            ...
                            is_subnet = verify_subnet(vlan, network_ip, version)
                            if not is_subnet:
            ...
            if asorting_cols:
                if "ambiente" in asorting_cols:
                    vlans = vlans.order_by("ambiente__divisao_dc__nome", "ambiente__ambiente_logico__nome", "ambiente__grupo_l3__nome")
                    asorting_cols.remove("ambiente")
                if "-ambiente" in asorting_cols:
                    vlans = vlans.order_by("-ambiente__divisao_dc__nome", "-ambiente__ambiente_logico__nome", "-ambiente__grupo_l3__nome")
                    asorting_cols.remove("-ambiente")
                if "tipo_rede" in asorting_cols:
                    vlans = vlans.order_by("networkipv4__network_type__tipo_rede", "networkipv6__network_type__tipo_rede")
                    asorting_cols.remove("tipo_rede")
                if "-tipo_rede" in asorting_cols:
                    vlans = vlans.order_by("-networkipv4__network_type__tipo_rede", "-networkipv6__network_type__tipo_rede")
                    asorting_cols.remove("-tipo_rede")
                if "network" in asorting_cols:
                    vlans = vlans.order_by("networkipv4__oct1", "networkipv4__oct2", "networkipv4__oct3", "networkipv4__oct4", "networkipv4__block", "networkipv6__block1", "networkipv6__block2", "networkipv6__block3", "networkipv6__block4", "networkipv6__block5", "networkipv6__block6", "networkipv6__block7", "networkipv6__block8", "networkipv6__block")
            ...
            # Datatable paginator
            vlans, total = build_query_to_datatable(vlans, asorting_cols, custom_search, searchable_columns, start_record, end_record)
