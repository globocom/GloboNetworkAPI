...
class VlanApplyAcl(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            # Script
            (erro,result) = commands.getstatusoutput("/usr/bin/backuper -T acl -b %s/%s -e -i %s -w 300"% (path, acl, name_equipaments))
            
            if erro:
                logger.error("Erro quando o usuário %s tentou aplicar ACL %s no equipamentos: %s erro: %s" % (user.nome, acl, equipments, result) )
                erro = False
                                            
            for equip in equipments:
                logger.info("%s aplicou a ACL %s no Equipamento:%s" % (user.nome, acl, equip) )
            ...
            logger.error("Erro quando o usuário %s tentou aplicar ACL %s no equipamentos: %s" % (user.nome, acl, equipments) )
            logger.error(e)
            raise Exception(e)
