...
class VlanCreateAclResource(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            return self.response_error(
                self.CODE_MESSAGE_DUPLICATE_ACL,
                acl_name)
    ...
    def __create_suggest_acl_name(self, vlan_object):
        return str(
            vlan_object.nome +
            vlan_object.ambiente.ambiente_logico.nome).replace(
            " ",
            "")

    def validate_duplicate_acl(
            self,
            acl_name,
            environment,
            network_type,
        ...
        if checkAclCvs(acl_name, environment, network_type, user):
            self.log.error(
                u'There is already an Vlan with the Acl - Ipv4 = %s.' %
                acl_name)
    ...
    def validate_id_vlan(self, id_vlan):
        if not is_valid_int_greater_zero_param(id_vlan):
            self.log.error(
                u'The id_valan parameter is not a valid value: %s.',
                id_vlan)
