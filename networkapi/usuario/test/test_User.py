...
class UserAttrUserTest(UserConfigTest, AttrTest):
    ...
    def test_alter_unique(self):
        ...
        mock = self.mock_valid()
        mock["user"] = user["nome"] 
        self.process_alter_attr_invalid(self.ID_ALTER_VALID, mock, CodeError.USER_DUPLICATE)
...
class UserByGroupConsultationTest(UserConfigTest, ConsultationTest):
    ...
    def test_get_by_group_id_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP % self.UGROUP_ID_VALID)
        valid_response(response)
    ...
    def test_get_by_group_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_GROUP % self.UGROUP_ID_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_by_group_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_GROUP % self.UGROUP_ID_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class UserByGroupOutConsultationTest(UserConfigTest, ConsultationTest):
    ...
    def test_get_by_group_out_id_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.UGROUP_ID_VALID)
        valid_response(response)
        content = valid_content(response, "users")
    ...
    def test_get_by_group_out_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_GROUP_OUT % self.UGROUP_ID_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_by_group_out_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_GROUP_OUT % self.UGROUP_ID_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_group_out_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.ID_NONEXISTENT)
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, codigo = CodeError.USER_GROUP_NOT_FOUND)
    
    def test_get_by_group_out_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.NEGATIVE_ATTR)
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        self._attr_invalid(response)
    
    def test_get_by_group_out_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.LETTER_ATTR)
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        self._attr_invalid(response)
    
    def test_get_by_group_out_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.ZERO_ATTR)
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        self._attr_invalid(response)
    
    def test_get_by_group_out_empty(self):
        response = self.client_autenticado().get(self.URL_GET_BY_GROUP_OUT % self.EMPTY_ATTR)
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        self._attr_invalid(response)
...
class UserAuthenticateTest(UserConfigTest, ConsultationTest):     
    ...
    def test_authenticate_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_AUTHENTICATE, {self.XML_KEY : mock})
        valid_response(response)
...
class UserWithGroupConsultationTest(UserConfigTest, ConsultationTest):
    ...
    def test_get_all_with_group_no_permission(self):
        response = self.client_no_permission().get(self.URL_LIST_WITH_USER_GROUP)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_all_with_group_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_LIST_WITH_USER_GROUP)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class UserChangePasswordTest(UserConfigTest, AttrTest):
    ...
    def test_change_pass_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_CHANGE_PASS, {self.XML_KEY: data})
        valid_response(response)
        assert xml2dict(response.content) == None
    
    def test_change_pass_inactive(self):
        data = self.user_change_pass_valid()
        response = self.client_no_active().postXML(self.URL_CHANGE_PASS, {self.XML_KEY: data})
        valid_response(response, httplib.UNAUTHORIZED)
    
    def test_change_pass_no_permission(self):
        data = self.user_change_pass_valid()
        response = self.client_no_permission().postXML(self.URL_CHANGE_PASS, {self.XML_KEY: data})
        valid_response(response, httplib.PAYMENT_REQUIRED)
