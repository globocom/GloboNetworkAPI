...
class Usuario(BaseModel):
    ...
    user_ldap = models.CharField(unique=True, max_length=45, null=True, blank=True)
    ...
    def get_by_pk(cls, pk):
        ...
        try:
            ...
            raise UsuarioNotFoundError(e, u'Dont there is a Group L3 by pk = %s.' % pk)
        except Exception, e:
    ...
    def get_by_user(cls, name):
        ...
        try:
            ...
            raise UsuarioNotFoundError(e, u'There is no User with username = %s.' % name)
        except Exception, e:
            cls.log.error(u'Failure to search the User.')
            raise UsuarioError(e, u'Failure to search the User.')
    ...
    def get_by_ldap_user(cls, ldap_usr, active=False):
        ...
        try:
            ...
            raise UsuarioNotFoundError(e, u'There is no User with ldap_user = %s.' % ldap_usr)
        except Exception, e:
            cls.log.error(u'Failure to search the User.')
            raise UsuarioError(e, u'Failure to search the User.')
    ...
    def get_enabled_user(self, username, password):
        ...
        try:
            ...
            self.log.error(u'Múltiplos usuários encontrados com o mesmo login e senha: %s', username)
        except Exception, e:
...
class UsuarioGrupo(BaseModel):
    ...
    def list_by_user_id(self,user_id):
        ...
        try:
            ...
            raise UsuarioNotFoundError(e, u'Dont there is a UserGroup by user = %s.' % user_id)
        except Exception, e:
    ...
    def get_by_user_group(cls, user_id, group_id):
        ...
        try:
            ...
            raise UserGroupNotFoundError(e, u'Dont there is a UserGroup by user = %s and group = %s.' % ( user_id , group_id))
        except Exception, e:
