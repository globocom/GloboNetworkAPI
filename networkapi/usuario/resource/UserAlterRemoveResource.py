    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_string_minsize(
                    ...
                    45):
                self.log.error(
                    u'Parameter user is invalid. Value: %s',
                    username)
            ...
            if not is_valid_string_minsize(
                    ...
                    3) or not is_valid_string_maxsize(
                    email,
                    200) or not is_valid_email(email):
            ...
            if not is_valid_boolean_param(active):
                self.log.error(
                    u'The active parameter is not a valid value: %s.',
                    active)
                ...
                try:
                    ...
                    if usr.eventlog_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(
                            None,
                            u'Existe event_log associado ao usuario %s' %
                            user.id)

                    if usr.usuariogrupo_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(
                            None,
                            u'Existe grupo associado ao usuario %s' %
                            user.id)
