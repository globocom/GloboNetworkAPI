    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_string_minsize(username,3) or not is_valid_string_maxsize(username, 45):
                self.log.error(u'Parameter user is invalid. Value: %s',username)
            ...
            if not is_valid_boolean_param(active):
                self.log.error(u'The active parameter is not a valid value: %s.', active)
            ...
            if not is_valid_string_minsize(user_ldap,3, False) or not is_valid_string_maxsize(user_ldap, 45, False):
                self.log.error(u'Parameter user_ldap is invalid. Value: %s', user_ldap)
            ...
            with distributedlock(LOCK_USER % id_user):
                ...
                try:
                    if usr.user.lower() != username.lower():
                        ...
                        raise UsuarioNameDuplicatedError(None, u'Já existe um usuário com o valor user %s.' % username)
            ...
            with distributedlock(LOCK_USER % id_user):
                ...
                try:
                    ...
                    if usr.eventlog_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(None, u'Existe event_log associado ao usuario %s' % user.id)

                    if usr.usuariogrupo_set.count() != 0:
                        raise UsuarioHasEventOrGrupoError(None, u'Existe grupo associado ao usuario %s' % user.id)
