...
class GroupVirtualResource(RestResource):
    ...
    def handle_delete(self, request, user, *args, **kwargs):
        ...
        try:
            xml_map, attrs_map = loads(request.raw_post_data, ['vip', 'equipamento', 'id_equipamento'])
        except XMLError, x:
                    ...
                    try:
                        ...
                        for vip_map in vip_maps:
                            ...
                            try:
                                ...
                                self.log.error(u'Valor do id_ip inválido: %s.', ip_id)
                                raise IpNotFoundError(e, u"Valor do id_ip inválido: %s." % ip_id)
                            ...
                            for equip_id in equipment_ids:
                                try:
                                    ...
                                    raise EquipamentoNotFoundError(e, u"Valor do id_equipamento inválido: %s." % equip_id)
                    ...
                    try:
                        ...
                        for equipment_map in equipment_maps:
                            ...
                            try:
                                ...
                                self.log.error(u'Valor do id do equipamento inválido: %s.', equip_id)
                                raise EquipamentoNotFoundError(e, u"Valor do id do equipamento inválido: %s." % equip_id)
            ...
            for equipment_map in equipment_maps:
                ...
                if response[0] == 0:
                    ...
                    if response_ip[0] == 0:
                        ...
                        # Insere um IP para cada VIP e o relacionamento dele com equipamento
                        resp_vip_maps = []
                        for vip_map in vip_maps:
                            ...
                            if (response_ip_vip[0] == 0):
                                ...
                                ip = str(response_ip_vip[1].get('oct1')) + '.' + str(response_ip_vip[1].get('oct2')) + '.' + str(response_ip_vip[1].get('oct3')) + '.' + str(response_ip_vip[1].get('oct4'))
                            
                                equipment_ip_map = {'ip':ip, 'nome_equipamento':equipment_map.get('nome')}
                                
                                equipment_ip_maps = vip_equipment_ip_map.get(vip_map.get('id'))
                                if equipment_ip_maps is None:
                                    equipment_ip_maps = [equipment_ip_map]
                                else:
                                    equipment_ip_maps.append(equipment_ip_map)
                                    
                                vip_equipment_ip_map[vip_map.get('id')] = equipment_ip_maps
                            else:
        ...
        try:
            for vip_map in vip_maps:
                ...
                if (ip_map is not None) and (id_vip_request_map is None):
                    ...
                    # Insere um IP e o relacionamento dele com o primeiro balanceador
                    ...
                    # Insere o relacionamento entre o IP e os demais balanceadores
                    for equip_id in equipments_ids[1:len(equipments_ids)]:
                        insert_ip_equipment(response_ip[1].get('id'), equip_id, user)
                ...
                # Obtem os reals já criados e que foram enviados no XML de requisição
                ...
                if equipment_ip_maps is not None:
                    for equipment_ip_map in equipment_ip_maps:
                        real_name = equipment_ip_map.get('nome_equipamento') # + sufix
                        real_ip = equipment_ip_map.get('ip')
                        real_maps.append({'real_name':real_name, 'real_ip':real_ip})
                
                vip_map['reals'] = {'real':real_maps}
                
                reals_priority_map = vip_map.get('reals_prioritys')
                if reals_priority_map is not None:
                    reals_priority_map = reals_priority_map.get('reals_priority')
                    if reals_priority_map is None:
                        reals_priority_map = ['0' for __real in real_maps]
                else:
                    reals_priority_map = ['0' for __real in real_maps]
                
                vip_map['reals_prioritys'] = {'reals_priority': reals_priority_map}
                ...
                if vip_map.get('reals') is not None:
                    ...
                    evip = EnvironmentVip.get_by_values(vip_map.get('finalidade'), vip_map.get('cliente'), vip_map.get('ambiente'))
                    ...
                    for real in vip_map.get('reals').get('real'):
                        ...
                        vip_map, code = RequisicaoVips().valid_values_reals_priority(vip_map)
                        if code is not None:
                            return self.response_error(code)
                        
                        vip_map, code = RequisicaoVips().valid_values_reals_weight(vip_map)
                        if code is not None:
                            return self.response_error(code)
                ...
                if (id_vip_request_map is not None):
                    ...
                    if not is_valid_int_greater_zero_param(id_vip_request_map.get('id')):
                        self.log.error(u'The requisicao_vip.id parameter is not a valid value: %s.', id_vip_request_map.get('id'))
                        raise InvalidValueError(None, 'requisicao_vip.id', id_vip_request_map.get('id'))
                    
                    vip_request = RequisicaoVips.get_by_pk(id_vip_request_map.get('id'))
                    ...
                    response_vip = update_vip_request(vip_request.id, vip_map, user)
                    if (response_vip != 0):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            xml_map, attrs_map = loads(request.raw_post_data, ['vip','equipamento', 'id_equipamento', 'reals_weight','reals_priority', 'real', 'transbordo', 'porta'])
        except XMLError, x:
        ...
        try:
            ...
            with distributedlock(LOCK_GROUP_VIRTUAL):
                ...
                response = self.__post_virtual_group_equipment(equipment_maps, vip_maps, user, resp_equipment_maps, vip_equipment_ip_map)
                if response is not None:
                    return response
         
                #for nos vips
                resp_vip_maps = []
                response = self.__post_virtual_group_vip(vip_maps, user, vip_equipment_ip_map, resp_vip_maps)
                if response is not None:
                    return response
