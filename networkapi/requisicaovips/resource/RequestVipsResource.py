...
class RequestVipsResource(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            # Load XML data
            xml_map, attrs_map = loads(request.raw_post_data, ['real', 'reals_weight','reals_priority', 'porta'])
            ...
            if ( vip_map.get('id_ipv4') is None and vip_map.get('id_ipv6') is None):
                self.log.error(u'The id_ipv4 and id_ipv6 parameter is not a valid value: %s.', vip_map.get('id_ipv4'))
                raise InvalidValueError(None, 'id_ipv4 e id_vip6', vip_map.get('id_ipv4'))
            
            if (vip_map.get('id_ipv4') != None):
                if not is_valid_int_greater_zero_param(vip_map.get('id_ipv4')):
                    self.log.error(u'The id_ipv4 parameter is not a valid value: %s.', vip_map.get('id_ipv4'))
                    raise InvalidValueError(None, 'id_ipv4', vip_map.get('id_ipv4'))
                
            if (vip_map.get('id_ipv6') != None):
                if not is_valid_int_greater_zero_param(vip_map.get('id_ipv6')):
                    self.log.error(u'The id_ipv6 parameter is not a valid value: %s.', vip_map.get('id_ipv6'))
                    raise InvalidValueError(None, 'id_ipv6', vip_map.get('id_ipv6'))

            # Valid maxcon
            if not is_valid_int_greater_equal_zero_param(vip_map.get('maxcon')):
                self.log.error(u'The maxcon parameter is not a valid value: %s.', vip_map.get('maxcon'))
            ...
            try:
                ...
                raise EnvironmentVipNotFoundError(e, 'The fields finality or client or ambiente is None')
            ...
            vip_map, vip, code = vip.valid_values_healthcheck(vip_map, vip, evip)
            if code is not None:
                return self.response_error(code)
            ...
            if not is_valid_string_minsize(areanegocio, 3) or not is_valid_string_maxsize(areanegocio, 100):
                self.log.error(u'Areanegocio value is invalid: %s.', areanegocio)
            ...
            if not is_valid_string_minsize(nome_servico, 3) or not is_valid_string_maxsize(nome_servico, 100):
                self.log.error(u'Nome_servico value is invalid: %s.', nome_servico)
            ...
            if vip_map.get('rule_id') is not None:
                if not is_valid_int_greater_zero_param(vip_map.get('rule_id')):
                    self.log.error(u'The rule_id parameter is not a valid value: %s.', vip_map.get('rule_id'))
                    raise InvalidValueError(None, 'rule_id', vip_map.get('rule_id'))
    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_boolean_param(vip_map.get('validado')):
                self.log.error(u'The validated parameter is not a valid value: %s.', vip_map.get('validado'))
                raise InvalidValueError(None, 'validated', vip_map.get('validado'))

            # Valid Vip vip_created
            if not is_valid_boolean_param(vip_map.get('vip_criado')):
                self.log.error(u'The vip_created parameter is not a valid value: %s.', vip_map.get('vip_criado'))
                raise InvalidValueError(None, 'vip_created', vip_map.get('vip_criado'))
            ...
            with distributedlock(LOCK_VIP % vip_id):
                ...
                # Valid Vip created
                if vip.vip_criado:
                    self.log.error(u'The IP of the request for VIP %d can not be changed because the VIP is already created.' % vip.id)
