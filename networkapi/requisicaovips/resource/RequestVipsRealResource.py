...
"""
    ...
    def administrate_real(
        ...
        if network_version == IP_VERSION.IPv4[0]:
            ...
            end_ip = "%s.%s.%s.%s" % (IpEquip.ip.oct1,
                                      IpEquip.ip.oct2,
                                      IpEquip.ip.oct3,
                                      IpEquip.ip.oct4)
        ...
        # Valid network_version - IPv6
            ...
            end_ip = "%s:%s:%s:%s:%s:%s:%s:%s" % (Ipv6Equip.ip.block1,
                                                  Ipv6Equip.ip.block2,
                                                  Ipv6Equip.ip.block3,
                                                  Ipv6Equip.ip.block4,
                                                  Ipv6Equip.ip.block5,
                    ...
                    if IP_VERSION.IPv4[0] == network_version:
                        if new_call:
                            ...
                            pool_members = ServerPoolMember.objects.filter(
                                ip=ip_id,
                                server_pool__vipporttopool__requisicao_vip__id=vip_id,
