...
def get_vips(vips):
    ...
    for vip in vips:
        ...
        list_environment.append("%s - %s - %s" % (vip_map['finalidade'], vip_map['cliente'], vip_map['ambiente']))
        try:
            if vip.ip is not None:
            
                descricao_ipv4 = vip.ip.descricao
                list_ips.append("%s.%s.%s.%s" % (vip.ip.oct1, vip.ip.oct2, vip.ip.oct3, vip.ip.oct4))
            ...
            if vip.ipv6 is not None:
                ...
                list_ips.append("%s:%s:%s:%s:%s:%s:%s:%s" % (vip.ipv6.block1,vip.ipv6.block2,vip.ipv6.block3,vip.ipv6.block4,vip.ipv6.block5,vip.ipv6.block6,vip.ipv6.block7,vip.ipv6.block8))
...
class RequestVipGetIdIpResource(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            # Datatable paginator
            vip, total = build_query_to_datatable(vip, asorting_cols, custom_search, searchable_columns, start_record, end_record)
