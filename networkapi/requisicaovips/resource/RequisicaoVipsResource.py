...
def insert_vip_request(vip_map, user):
    ...
    vip_map, vip, code = vip.valid_values_healthcheck(vip_map, vip, environment_vip)
    if code is not None:
        return code, vip
    
    # Valid maxcon
    if not is_valid_int_greater_equal_zero_param(vip_map.get('maxcon')):
        log.error(u'The maxcon parameter is not a valid value: %s.', vip_map.get('maxcon'))
    ...
    if vip_map.get("reals") is not None:
        ...
        for real in vip_map.get('reals').get('real'):
            ...
            RequisicaoVips.valid_real_server(ip_aux_error, equip, environment_vip)
...
def update_vip_request(vip_id, vip_map, user):
    ...
    if healthcheck_expect_id is not None:
        if not is_valid_int_greater_zero_param(healthcheck_expect_id):
            log.error(u'The healthcheck_expect_id parameter is not a valid value: %s.', healthcheck_expect_id)
            raise InvalidValueError(None, 'healthcheck_expect_id', healthcheck_expect_id)
        else:
...
class RequisicaoVipsResource(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            xml_map, attrs_map = loads(request.raw_post_data, ['transbordo', 'porta', 'real'])
        except XMLError, x:
    ...
    def handle_get(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if reals_list:
                ...
                request_vip_map['reals_prioritys'] = {'reals_priority' : reals_priority}
                request_vip_map['reals_weights'] = {'reals_weight' : reals_weight}
            ...
            request_vip_map['validado'] = convert_boolean_to_int(request_vip.validado)
            request_vip_map['vip_criado'] = convert_boolean_to_int(request_vip.vip_criado)
            request_vip_map['id_ip'] = request_vip.ip_id
            request_vip_map['id_ipv6'] = request_vip.ipv6_id
            request_vip_map['id_healthcheck_expect'] = request_vip.healthcheck_expect_id
