        ...
        try:
            ...
            if (reals_prioritys_map is not None):
                ...
                if (reals_priority_map is not None):
                    ...
                    if len(reals_priority_map) > 0:
                        vip_map = RequisicaoVips.is_valid_values_reals_priority(reals_priority_map)
                    else:
                        self.log.error(u'The reals_priority_map parameter is not a valid value: %s.', reals_priority_map)
                        raise InvalidValueError(None, 'reals_priority_map', reals_priority_map)
                else:
                    self.log.error(u'The reals_priority parameter is not a valid value: %s.', reals_priority_map)
                    raise InvalidValueError(None, 'reals_priority', reals_priority_map)
            else:
                self.log.error(u'The reals_prioritys parameter is not a valid value: %s.', reals_prioritys_map)
                raise InvalidValueError(None, 'reals_prioritys', reals_prioritys_map)
            ...
            with distributedlock(LOCK_VIP % vip_id):
                ...
                if not vip.vip_criado:
                    self.log.error(u'Priority can not be changed because VIP has not yet been created.')
                ...
                variables_map['reals_prioritys'] = vip_map.get('reals_prioritys')
