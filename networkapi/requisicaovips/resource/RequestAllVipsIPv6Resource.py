    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_ipv6(ip_str):
                self.log.error(u'Parameter ipv6 is invalid. Value: %s.', ip_str)
                raise InvalidValueError(None, 'ipv6', ip_str)
            
            # Valid all_prop
            if not is_valid_int_param(all_prop):
                self.log.error(u'Parameter all_prop is invalid. Value: %s.', all_prop)
            ...
            ipv6 = Ipv6.get_by_blocks(blocks[0], blocks[1], blocks[2], blocks[3], blocks[4], blocks[5], blocks[6], blocks[7])
            ...
            for ip in ipv6:
                ...
                if all_prop == 1:
                    ...
                    vips = ip.requisicaovips_set.all().values_list('id', flat=True)
