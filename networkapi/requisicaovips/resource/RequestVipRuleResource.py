...
class RequestVipRuleResource(RestResource):
    ...
    def handle_get(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(id_block):
                self.log.error(u'Parameter id_block is invalid. Value: %s.', id_block)
                raise InvalidValueError(None, 'id_block', id_block)

            if not is_valid_boolean_param(override, False):
                self.log.error(u'Parameter override is invalid. Value: %s.', override)
            ...
            if not vip.vip_criado:
                self.log.error(u'Block can not added because VIP has not been created yet.')
            ...
            ###################################################
            if rule_applied:
                block_in_rules = self.insert_block_in_rule(id_block, rule_applied)
                ...
                diff_name = '(' + str(count_rule_vip) + ')' if count_rule_vip else ''
            ...
            ###################################################
                ...
                block_in_rules, environment = self.generate_rule_contents(vip, id_block)
    ...
    def insert_block_in_rule(self, id_block, rule_applied):
        ...
        for content_rule in rule_applied.rulecontent_set.all():
            ...
            blocks_envs = BlockRules.objects.filter(environment=content_rule.rule.environment)
            
            is_add = False
            for b in blocks_envs:
                if clear_newline_chr(b.content) == clear_newline_chr(content_rule.content):
                    block_in_rules.append({'content_block_id': b.id, 'content': content_rule.content})
                    is_add = True
                    break
            
            if not is_add:
                block_in_rules.append({'content_block_id': 0, 'content': content_rule.content})
            #try:
        ...
        block = BlockRules.objects.get(pk=id_block, environment=rule_applied.environment)
        ...
        for i in range(len(block_in_rules)):
            ...
            if block.id == content_block_id:
                raise VipRequestBlockAlreadyInRule(None, "Block is already in rule")
        ...
        if index_to_insert is None:
            raise VipRequestNoBlockInRule(None, "Rule don't have any block associated.")

        # list.insert(index, object) -- insert object before index
        block_in_rules.insert(index_to_insert, {'content_block_id': block.id, 'content': block.content})
    ...
    def generate_rule_contents(self, vip, id_block):
        ...
        if vip.ip is not None:
            ...
            block_v4 = BlockRules.objects.get(pk=id_block, environment=environment_v4)
            blocks_v4 = environment_v4.blockrules_set.all().order_by('order')
            new_rule_content, has_block_in_filter = self.create_rule_by_blocks(blocks_v4, filter_applied, block_v4)
            environment = environment_v4

        elif vip.ipv6 is not None:
            environment_v6 = vip.ipv6.networkipv6.vlan.ambiente
            block_v6 = BlockRules.objects.get(pk=id_block, environment=environment_v6)
            blocks_v6 = environment_v6.blockrules_set.all().order_by('order')
            new_rule_content, has_block_in_filter = self.create_rule_by_blocks(blocks_v6, filter_applied, block_v6)
    ...
    def create_rule_by_blocks(self, blocks, filter_applied, new_block):
        ...
        for block_env in blocks:
            if clear_newline_chr(block_env.content) in clear_newline_chr(filter_applied):
                new_blocks_rule.append(self.create_rule_content_dict(block_env.id, block_env.content))
            ...
            if clear_newline_chr(new_blocks_rule[i]['content']) in clear_newline_chr(filter_applied):
                ...
                if i == 0:
                    if index == 0:
                        ...
                        new_rule_content.insert(0, self.create_rule_content_dict(0, filter_applied[0:index]))
                    # Than append the block
                    new_rule_content.append(self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                    # Saves the index to insert the new block after this content
                    after_index_to_insert_new_rule_content = len(new_rule_content)
                # Last block found
                elif i == (len(new_blocks_rule) - 1):
                    if after_index_to_insert_old_rule_content:
                        # Index to insert the block that already was in filter_applied after the new block
                        new_rule_content.insert(after_index_to_insert_old_rule_content, self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                    else:
                        # None new block was inserted, just append the block that already exists
                        new_rule_content.append(self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                    if index + len(new_blocks_rule[i]['content']) == len(filter_applied):
                        ...
                        new_rule_content.append(self.create_rule_content_dict(0, filter_applied[index + len(new_blocks_rule[i]['content']):len(filter_applied)]))
                else:
                    # Than append the block
                    new_rule_content.append(self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                    # Saves the index to insert the new block after this content
                    after_index_to_insert_new_rule_content = len(new_rule_content)
                    after_index_to_insert_old_rule_content = len(new_rule_content)

            else:
                # if a block was inserted before, then it has the index to insert after
                if after_index_to_insert_new_rule_content:
                    # Insert new block after the last block found
                    new_rule_content.insert(after_index_to_insert_new_rule_content, self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                    # if isn't the last block
                    if i != (len(new_blocks_rule) - 1):
                        # Get all content after the last block and before the next content block make a rule content
                        start = filter_applied.index(new_rule_content[after_index_to_insert_new_rule_content - 1]['content']) + len(new_rule_content[after_index_to_insert_new_rule_content - 1]['content'])
                        end = filter_applied.index(new_blocks_rule[i + 1]['content'])
                        content = filter_applied[start:end]
                        if content.strip():
                            new_rule_content.append(self.create_rule_content_dict(0, content))
                    else:
                        # Get all content after the last block and make a rule content
                        start = filter_applied.index(new_rule_content[after_index_to_insert_new_rule_content - 1]['content']) + len(new_rule_content[after_index_to_insert_new_rule_content - 1]['content'])
                        end = len(filter_applied)
                        new_rule_content.append(self.create_rule_content_dict(0, filter_applied[start:end]))
                else:
                    # if isnt the last block
                    if i != (len(new_blocks_rule) - 1):
                        # Insert new block before the first block found
                        new_rule_content.insert(0, self.create_rule_content_dict(0, filter_applied[0:filter_applied.index(new_blocks_rule[i + 1]['content'])]))
                        new_rule_content.append(self.create_rule_content_dict(new_blocks_rule[i]['content_block_id'], new_blocks_rule[i]['content']))
                        # Index to insert the rule content thas already exists in rule_applied
                        after_index_to_insert_old_rule_content = len(new_rule_content)
