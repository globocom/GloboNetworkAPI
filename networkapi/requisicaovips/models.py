...
class RequisicaoVips(BaseModel):
    ...
    def get_by_pk(cls, id):
        ...
        try:
            ...
            raise RequisicaoVipsNotFoundError(
                e,
                u'Dont there is a request of vips by pk = %s.' %
                id)
    ...
    def get_by_healthcheck_expect(cls, healthcheck_exp):
        ...
        """
        try:
            return RequisicaoVips.objects.filter(
                healthcheck_expect__id=healthcheck_exp)
        except Exception as e:
            cls.log.error(
    ...
    def get_by_ipv4_id(cls, id_ipv4):
        ...
        try:
            ...
            raise RequisicaoVipsError(
                e,
                u'Failure to list Request Vip by ipv4.')
    ...
    def is_valid_values_reals_priority(cls, reals_priority_map):
        ...
        if reals_priority_map == valid_number_map and len(
                reals_priority_map) != 1:
    ...
    def set_variables(self, variables_map):
        ...
        if (transbordos_map is not None):
            ...
            for transbordo in transbordos:
                if (
                    finalidade == "Homologacao" and ambiente == "Homologacao FE-CITTA") or (
                    finalidade == 'Producao' and ambiente in (
                        'Portal FE',
                        'Aplicativos FE',
                        'Streaming FE')):
    ...
    def update(cls, authenticated_user, pk, variables_map, **kwargs):
                ...
                if vip.ip_id != ip_id:
                    if vip.vip_criado:
                        raise RequisicaoVipsAlreadyCreatedError(
                            None,
                            u'O IP da requisição de VIP %d não pode ser alterado porque o VIP já está criado.' %
                            vip.id)
        ...
        if variables_map.get('reals_prioritys') is None:
            if vip_variables.get('reals_prioritys') is None:
                ...
                if vip_variables.get('reals_prioritys').get(
                        'reals_priority') is None:
                    priority_map = []
                    for __reals in variables_map.get('reals').get('real'):
                        priority_map.append('0')
                    variables_map['reals_prioritys'] = {
        ...
        if variables_map.get('reals_weights') is None:
            if vip_variables.get('reals_weights') is None:
                ...
                if vip_variables.get('reals_weights').get(
                        'reals_weight') is None:
                    priority_map = []
                    for __reals in variables_map.get('reals').get('real'):
                        priority_map.append('0')
            ...
            if (port_map is not None and port_map != []):
                ...
                for port in port_map:
                    ...
                    if not is_valid_regex(port, "[0-9]+:[0-9]+"):
                        self.log.error(
                            u'The port parameter is not a valid value: %s.',
                            port)
                ...
                self.log.error(
                    u'The ports parameter is not a valid value: %s.',
                    port_map)
    ...
    def valid_values_reals_priority(self, vip_map):
        ...
        if (reals_prioritys_map is not None):
            ...
            if (reals_priority_map is not None):
                ...
                # list
                if len(
                        vip_map.get('reals').get('real')) != len(
                        vip_map.get('reals_prioritys').get('reals_priority')):
                    self.log.error(
        ...
        if (str(metodo_bal).upper() == "WEIGHTED"):
            ...
            if (reals_weights_map is not None):
                ...
                if (reals_weight_map is not None):
                    ...
                    if len(
                            vip_map.get('reals').get('real')) != len(
                            vip_map.get('reals_weights').get('reals_weight')):
                        self.log.error(
                    ...
                    for reals_weight in reals_weight_map:
                        if not is_valid_int_greater_equal_zero_param(
                                reals_weight):
                            self.log.error(
                                u'The reals_weight parameter is not a valid value: %s.',
                                reals_weight)
                            raise InvalidValueError(
                                None,
                                'reals_weight',
    ...
    def heathcheck_exist(cls, healthcheck_type, id_evironment_vip):
        ...
        for option_vip_healthcheck in healthcheck_list:
            if str(
                    option_vip_healthcheck.nome_opcao_txt).upper() == healthcheck_type:
                    ...
                    for dict_div_amb in lista_amb_div_4:
                        if ipequip.ip.networkipv4.ambient_vip is not None and ipequip.ip.networkipv4.ambient_vip.id == evip.id:
                            if (ipequip.ip.networkipv4.vlan.ambiente.divisao_dc_id == dict_div_amb.get(
                                    'divisao_dc') and ipequip.ip.networkipv4.vlan.ambiente.ambiente_logico_id == dict_div_amb.get('ambiente_logico')):
                                lista_ips_equip.append(ipequip.ip)

            if valid:
                if ip not in lista_ips_equip:
                    raise IpNotFoundByEquipAndVipError(
                        None, 'Ipv4 não está relacionado com equipamento %s e Ambiente Vip: %s' %
                        (equip.name, evip.show_environment_vip()))

        elif is_valid_ipv6(ip):

            ip_list = ip.split(":")
            ip = Ipv6.get_by_octs_and_environment_vip(
                ip_list[0],
                ip_list[1],
                ip_list[2],
                ip_list[3],
                    ...
                    for dict_div_amb in lista_amb_div_6:
                        if ipequip.ip.networkipv6.ambient_vip is not None and ipequip.ip.networkipv6.ambient_vip.id == evip.id:
                            if (ipequip.ip.networkipv6.vlan.ambiente.divisao_dc == dict_div_amb.get(
                                    'divisao_dc') and ipequip.ip.networkipv6.vlan.ambiente.ambiente_logico == dict_div_amb.get('ambiente_logico')):
                                lista_ipsv6_equip.append(ipequip.ip)

            if valid:
                if ip not in lista_ipsv6_equip:
                    raise IpNotFoundByEquipAndVipError(
                        None, 'Ipv6 não está relacionado com equipamento %s e Ambiente Vip: %s' %
                        (equip.name, evip.show_environment_vip()))
    ...
    def save_vips_and_ports(self, vip_map, user):
        ...
        for i in range(0, len(reals)):
            ...
            if not is_valid_int_greater_zero_param(port_real):
                self.log.error(
                    u'The reals.port_real parameter is not a valid value: %s.',
                    port_real)
            ...
            if not is_valid_int_greater_zero_param(ip_id):
                self.log.error(
                    u'The reals.ip_id parameter is not a valid value: %s.',
                    ip_id)
    ...
    def get_vips_and_reals(self, id_vip):
        ...
        for v_port in vip_port:
            ...
            vip_port_list.append(str(v_port.port_vip) +
                                 ':' +
            ...
            for member in members:
                ...
                reals_list.append({'real_ip': ip_string,
                                   'real_name': equip_name,
                                   'port_vip': vip_port.port_vip,
...
class OptionVip(BaseModel):
    ...
    def valid_option_vip(self, optionvip_map):
        ...
        if not is_valid_string_maxsize(
                tipo_opcao,
                50,
                True) or not is_valid_option(tipo_opcao):
            self.log.error(
                u'Parameter tipo_opcao is invalid. Value: %s.',
                tipo_opcao)
        ...
        if not is_valid_string_maxsize(
                nome_opcao_txt,
                50,
                True) or not is_valid_option(nome_opcao_txt):
            self.log.error(
                u'Parameter nome_opcao_txt is invalid. Value: %s.',
                nome_opcao_txt)
    ...
    def get_by_pk(cls, id):
        ...
        try:
            ...
            raise OptionVipNotFoundError(
                e,
                u'Dont there is a option vip by pk = %s.' %
                id)
        except OperationalError as e:
            cls.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(
    ...
    def get_all_healthcheck(cls, id_environment_vip):
        ...
        try:
            ...
            raise OptionVipError(
                e,
                u'Failure to list all Option Vip Healthcheck.')
...
class OptionVipEnvironmentVip(BaseModel):
    ...
    def get_by_option_environment(self, option_id, environment_id):
        ...
        """
        try:
            return OptionVipEnvironmentVip.objects.filter(
            ...
            raise OptionVipEnvironmentVipError(
                e,
                u'Failure to search the OptionVipEnvironmentVip.')
    ...
    def validate(self):
        ...
        try:
            ...
            raise OptionVipEnvironmentVipDuplicatedError(
                None,
                u'Option vip already registered for the environment vip.')
...
class VipPortToPool(BaseModel):
    ...
    def get_by_vip_id(cls, id_vip):
        ...
        try:
            ...
            raise RequisicaoVipsError(
                e,
                u'Failure to list Request VipPortToPool by id_vip.')
