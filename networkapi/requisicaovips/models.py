...
class RequisicaoVips(BaseModel):
    ...
    ipv6 = models.ForeignKey(Ipv6, db_column='ipsv6_id_ipv6',blank=True,null=True)
    l7_filter = models.TextField(blank=True,null=True, db_column='l7_filter_to_apply')
    filter_applied = models.TextField(blank=True,null=True, db_column='l7_filter_current')
    filter_rollback = models.TextField(blank=True,null=True, db_column='l7_filter_rollback')
    filter_valid = models.BooleanField(db_column='l7_filter_is_valid')
    applied_l7_datetime = models.DateTimeField(db_column='l7_filter_applied_datetime')
    healthcheck_expect = models.ForeignKey(HealthcheckExpect, null=True, db_column='id_healthcheck_expect', blank=True)
    rule = models.ForeignKey('blockrules.Rule', db_column='id_rule', blank=True, null=True, on_delete=models.SET_NULL, related_name='rule')
    rule_applied = models.ForeignKey('blockrules.Rule', db_column='id_rule_current', blank=True, null=True, on_delete=models.SET_NULL, related_name='rule_applied')
    rule_rollback = models.ForeignKey('blockrules.Rule', db_column='id_rule_rollback', blank=True, null=True, on_delete=models.SET_NULL, related_name='rule_rollback')
    ...
    def get_by_pk(cls, id):
        ...
        try:
            ...
            raise RequisicaoVipsNotFoundError(e, u'Dont there is a request of vips by pk = %s.' % id)
    ...
    def remove(cls, authenticated_user, vip_id):
        ...
        try:
            ...
            raise RequisicaoVipsNotFoundError(e,'Requisao Vip com id %s nao encontrada' % vip_id)
        except Exception, e:
    ...
    def get_by_healthcheck_expect(cls, healthcheck_exp):
        ...
        try:
            ...
            cls.log.error(u'Failure to list Request Vip with healthcheck expect.')
            raise RequisicaoVipsError(e, u'Failure to list Request Vip with healthcheck expect.')
    ...
    def get_by_ipv4_id(cls, id_ipv4):
        ...
        try:
            ...
            raise RequisicaoVipsError(e, u'Failure to list Request Vip by ipv4.')
    ...
    def is_valid_values_reals_priority(cls, reals_priority_map):
        ...
        if reals_priority_map == valid_number_map and len(reals_priority_map) != 1:
            ...
            reals_priority_map = {'reals_prioritys': {'reals_priority': priority_map } }
        else:
            reals_priority_map = {'reals_prioritys': {'reals_priority': reals_priority_map } }
    ...
    def variables_to_map(self):
        ...
        while (map.get(key_real_name + str(i)) is not None) and (map.get(key_real_ip + str(i)) is not None):
            real_maps.append({'real_name':map[key_real_name + str(i)], 'real_ip':map[key_real_ip + str(i)]})
    ...
    def set_variables(self, variables_map):
        ...
        if self.healthcheck_expect is not None and healthcheck_type != 'HTTP':
                raise InvalidHealthcheckTypeValueError(None, u"Valor do healthcheck_type inconsistente com o valor do healthcheck_expect.")
        ...
        try:
            ...
            raise EnvironmentVipNotFoundError(None,u'Não existe ambiente vip para valores: finalidade %s, cliente %s e ambiente_p44 %s.' %  (finalidade,cliente,ambiente) )
        ...
        grupos_cache = [ (gc.nome_opcao_txt) for gc in OptionVip.get_all_grupo_cache(evip.id) ]
        timeouts = [ (t.nome_opcao_txt) for t in OptionVip.get_all_timeout(evip.id) ]
        persistencias = [ (p.nome_opcao_txt) for p in OptionVip.get_all_persistencia(evip.id) ]
        balanceamentos = [ (b.nome_opcao_txt) for b in OptionVip.get_all_balanceamento(evip.id) ]

        if timeout not in timeouts:
            log.error(u'The timeout not in OptionVip, invalid value: %s.', timeout)
            raise InvalidTimeoutValueError(None, 'timeout com valor inválido: %s.' % timeout)
        self.add_variable('timeout', timeout)

        if balanceamento not in balanceamentos:
            log.error(u'The method_bal not in OptionVip, invalid value: %s.', balanceamento)
            raise InvalidMetodoBalValueError(None, 'metodo_bal com valor inválido: %s.' % balanceamento)
        self.add_variable('metodo_bal', balanceamento)

        if grupo_cache not in grupos_cache:
            log.error(u'The grupo_cache not in OptionVip, invalid value: %s.', grupo_cache)
            raise InvalidCacheValueError(None, 'grupo_cache com valor inválido: %s.' % grupo_cache)
        self.add_variable('cache', grupo_cache)

        if persistencia not in persistencias:
            log.error(u'The persistencia not in OptionVip, invalid value: %s.', persistencia)
            raise InvalidPersistenciaValueError(None, 'persistencia com valor inválido %s.' % persistencia)
        self.add_variable('persistencia', persistencia)

        environment_vip = EnvironmentVip.get_by_values(finalidade, cliente, ambiente)
        healthcheck_is_valid = RequisicaoVips.heathcheck_exist(healthcheck_type, environment_vip.id)

        # healthcheck_type
        if not healthcheck_is_valid:
            raise InvalidHealthcheckTypeValueError(None, u'Healthcheck_type com valor inválido: %s.' % healthcheck_type)
        ...
        if healthcheck is not None:
            if healthcheck_type != 'HTTP':
                raise InvalidHealthcheckValueError(None, u"Valor do healthcheck inconsistente com o valor do healthcheck_type.")
        ...
        try:
            ...
            raise InvalidMaxConValueError(None, u'Maxcon com valor inválido: %s.' % maxcon)
        ...
        if finalidade == 'Producao' and cliente == 'Usuario WEB' and ambiente == 'Streaming FE' and dsr == 'dsr' and bal_ativo is not None:
            if bal_ativo not in ('B11A', 'B12'):
                raise InvalidBalAtivoValueError(None, u'Bal_ativo com valor inválido: %s.' % bal_ativo)
    ...
    def update_vip_created(self, authenticated_user, vip_created):
        try:
            ...
            self.log.error(u'Falha ao atualizar o campo vip_criado da requisição de vip.')
            raise RequisicaoVipsError(e, u'Falha ao atualizar o campo vip_criado da requisição de vip.')
    ...
    def update(cls, authenticated_user, pk, variables_map, **kwargs):
        ...
        with distributedlock(LOCK_VIP % pk):
            ...
            try:
                ...
                if vip.ip_id != ip_id:
                    if vip.vip_criado:
                        raise RequisicaoVipsAlreadyCreatedError(None, u'O IP da requisição de VIP %d não pode ser alterado porque o VIP já está criado.' % vip.id)
        ...
        variables_map, vip, code = vip.valid_values_healthcheck(variables_map, vip, environment_vip)
        if code is not None:
            return code 
        ...
        if variables_map.get('reals_prioritys') is None:
            if vip_variables.get('reals_prioritys') is None:
                ...
                if vip_variables.get('reals_prioritys').get('reals_priority') is None:
                    ...
                    variables_map['reals_prioritys'] = vip_variables.get('reals_prioritys')
        ...
        if variables_map.get('reals_weights') is None:
            if vip_variables.get('reals_weights') is None:
                ...
                if vip_variables.get('reals_weights').get('reals_weight') is None:
                    ...
                    for __reals in variables_map.get('reals').get('real'):
                        priority_map.append('0')
                    variables_map['reals_weights'] = {'reals_weight': priority_map }
                else:
                    variables_map['reals_weights'] = vip_variables.get('reals_weights')
        ...
        try:
            ...
            raise RequisicaoVipsError(e, u'Falha ao atualizar a requisição de vip.')
    ...
    def valida(self,authenticated_user,validado):
        ...
        try:
            ...
            raise RequisicaoVipsError(e, u'Falha ao validar a requisição de vip.')
    ...
    def create(self, authenticated_user, variables_map):
        ...
        try:
            ...
            raise RequisicaoVipsError(e, u'Falha ao inserir a requisição de vip.')
            ...
            if (port_map is not None and port_map != []):
                ...
                for port in port_map:
                    ...
                    if not is_valid_regex(port,"[0-9]+:[0-9]+"):
                        self.log.error(u'The port parameter is not a valid value: %s.', port)
                ...
                self.log.error(u'The ports parameter is not a valid value: %s.', port_map)
                return vip_map, (138, port_map)
        else:
            self.log.error(u'The ports parameter is not a valid value: %s.', portas_servicos_map)
    ...
    def valid_values_reals_priority(self, vip_map):
        ...
        if (reals_prioritys_map is not None):
            ...
            if (reals_priority_map is not None):
                ...
                # Validates the size reals list is equal to the size priority list 
                ...
                for reals_priority in reals_priority_map:
                    if not is_valid_int_greater_equal_zero_param(reals_priority):
                        self.log.error(u'The reals_priority parameter is not a valid value: %s.', reals_priority)
                        raise InvalidValueError(None, 'reals_priority', reals_priority)

                # Valid list reals_prioritys
                vip_map['reals_prioritys'] = self.is_valid_values_reals_priority(vip_map.get('reals_prioritys').get('reals_priority')).get('reals_prioritys')

        else:
            self.log.error(u'The reals_priority parameter is not a valid value: %s.', reals_prioritys_map)
            raise InvalidValueError(None, 'reals_priority', reals_prioritys_map)
        ...
        if ( str(metodo_bal).upper() == "WEIGHTED" ):
            ...
            if (reals_weights_map is not None):
                ...
                if (reals_weight_map is not None):
                    ...
                    # Validates the size reals list is equal to the size _weight list
                    if len(vip_map.get('reals').get('real')) != len(vip_map.get('reals_weights').get('reals_weight')):
                        self.log.error(u'List of reals_weight is higher or lower than list the real_server.')
                        return vip_map, 274
                    
                    #Valid values ​​of reals_weight
                    for reals_weight in reals_weight_map:
                        if not is_valid_int_greater_equal_zero_param(reals_weight):
                            self.log.error(u'The reals_weight parameter is not a valid value: %s.', reals_weight)
                            raise InvalidValueError(None, 'reals_weight', reals_weight)
                
                else:
                    # Validates the size reals list is equal to the size _weight list
                    if len(vip_map.get('reals').get('real')) > 0:
                        self.log.error(u'List of reals_weight is empty but real_server is not.')
                        return vip_map, 274
            else:
                    # Validates the size reals list is equal to the size _weight list
                    if len(vip_map.get('reals').get('real')) > 0:
    ...
    def valid_values_healthcheck(self, vip_map, vip, evironment_vip):
        ...
        healthcheck_is_valid = RequisicaoVips.heathcheck_exist(healthcheck_type, evironment_vip.id)
        ...
        if healthcheck_type != 'HTTP':
            if not (id_healthcheck_expect == None and healthcheck == None):
                self.log.error(u'The healthcheck_type parameter is %s, then healthcheck and id_healthcheck_expect must be None.', healthcheck_type)
        ...
        # If healthcheck_type is 'HTTP' id_healthcheck_expect and healthcheck must NOT be None
            ...
            if id_healthcheck_expect == None or healthcheck == None:
                self.log.error(u'The healthcheck_type parameter is HTTP, then healthcheck and id_healthcheck_expect must NOT be None.')
                ...
                try:
                    ...
                    if not isinstance(healthcheck, basestring):
                        ...
                        raise InvalidValueError(None, 'healthcheck', healthcheck)

                except HealthcheckExpectNotFoundError:
                    self.log.error('The id_healthcheck_expect parameter does not exist.')
        ...
        if is_valid_ipv4(ip):
            ...
            if valid == True:
                if not ip in lista_ips_equip:
                    raise IpNotFoundByEquipAndVipError(None,'Ipv4 não está relacionado com equipamento %s e Ambiente Vip: %s' % (equip.name, evip.show_environment_vip()))
        
        elif is_valid_ipv6(ip):
            
            ip_list = ip.split(":")
            ip = Ipv6.get_by_octs_and_environment_vip(ip_list[0], ip_list[1], ip_list[2], ip_list[3], ip_list[4], ip_list[5], ip_list[6], ip_list[7], evip.id, valid)
            ...
            for net in evip.networkipv6_set.select_related().all():
                ...
                dict_div_6['ambiente_logico'] = net.vlan.ambiente.ambiente_logico
            ...
            if valid == True:
                if not ip in lista_ipsv6_equip:
                    raise IpNotFoundByEquipAndVipError(None,'Ipv6 não está relacionado com equipamento %s e Ambiente Vip: %s' % (equip.name, evip.show_environment_vip()))
    ...
    def save_vips_and_ports(self, vip_map, user):
        ...
        for port_vip in ports_vip:
            ...
            vip_port_to_pool.prepare_and_save(port_vip[0], server_pool, self, user)
            
            vip_port_list.append({'port_vip':port_vip[0], 'server_pool':server_pool})
        ...
        for i in range(0, len(reals)):
            ...
            if not is_valid_int_greater_zero_param(port_real):
                self.log.error(u'The reals.port_real parameter is not a valid value: %s.', port_real)
            ...
            if not is_valid_int_greater_zero_param(ip_id):
                self.log.error(u'The reals.ip_id parameter is not a valid value: %s.', ip_id)
            ...
            # save ServerPoolMember
            server_pool_member.prepare_and_save(server_pool, ip, ip_type, priority, weight, port_real, user)
    ...
    def get_vips_and_reals(self, id_vip):   
        ...
        for v_port in vip_port:
            ...
            vip_port_list.append(str(v_port.port_vip)+':'+str(v_port.server_pool.default_port))
            ...
            for member in members:
                ...
                reals_list.append({'real_ip': ip_string, 'real_name': equip_name, 'port_vip': vip_port.port_vip, 'port_real': member.port_real, 'id_ip': ip_id})
    ...
    def delete_vips_and_reals(self, user):
        ...
        for vip_port in vip_ports:
            server_pool_members = ServerPoolMember.objects.filter(server_pool = vip_port.server_pool)
...
class OptionVip(BaseModel):
    ...
    tipo_opcao = models.CharField(max_length=50, blank=False, db_column='tipo_opcao')
    nome_opcao_txt = models.CharField(max_length=50, blank=False, db_column='nome_opcao_txt')
    ...
    def valid_option_vip(self, optionvip_map):
        ...
        if not is_valid_string_maxsize(tipo_opcao, 50, True) or not is_valid_option(tipo_opcao):
            self.log.error(u'Parameter tipo_opcao is invalid. Value: %s.', tipo_opcao)
        ...
        if not is_valid_string_maxsize(nome_opcao_txt, 50, True) or not is_valid_option(nome_opcao_txt):
            self.log.error(u'Parameter nome_opcao_txt is invalid. Value: %s.', nome_opcao_txt)
    ...
    def get_by_pk(cls, id):
        ...
        try:
            ...
            raise OptionVipNotFoundError(e, u'Dont there is a option vip by pk = %s.' % id)
        except OperationalError, e:
            cls.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(e, u'Lock wait timeout exceeded; try restarting transaction')        
        except Exception, e:
    ...
    def get_all_timeout(cls,id_environment_vip):
        ...
        try:
            ...
            ovips = ovips.filter(optionvipenvironmentvip__environment__id = int(id_environment_vip))
    ...
    def get_all_healthcheck(cls, id_environment_vip):
        ...
        try:
            ...
            raise OptionVipError(e, u'Failure to list all Option Vip Healthcheck.')
...
class OptionVipEnvironmentVip(BaseModel):
    ...
    def get_by_option_environment(self, option_id, environment_id):
        ...
        try: 
            ...
            raise OptionVipEnvironmentVipNotFoundError(e, u'Dont there is a OptionVipEnvironmentVip by option_id = %s and environment_id = %s' % (option_id, environment_id))
        except OperationalError, e:
            self.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(e, u'Lock wait timeout exceeded; try restarting transaction')
        except Exception, e:
            self.log.error(u'Failure to search the OptionVipEnvironmentVip.')
            raise OptionVipEnvironmentVipError(e, u'Failure to search the OptionVipEnvironmentVip.')
    ...
    def validate(self):
        ...
        """
        try: 
            OptionVipEnvironmentVip.objects.get(option=self.option, environment=self.environment)
            raise OptionVipEnvironmentVipDuplicatedError(None, u'Option vip already registered for the environment vip.')
...
class ServerPool(BaseModel):
    ...
    healthcheck = models.ForeignKey(Healthcheck, db_column='healthcheck_id_healthcheck')
...
class ServerPoolMember(BaseModel):
    ...
    def save_with_default_port(self, vip_id, ip, ip_version, user):
        ...
        server_pools = ServerPool.objects.filter(vipporttopool__requisicao_vip__id=vip_id)
        
        for server_pool in server_pools:
            server_pool_member = ServerPoolMember()
            server_pool_member.prepare_and_save(server_pool, ip, ip_version, 0, 1, server_pool.default_port, user, commit=True)
            
            
    def save_specified_port(self, vip_id, port_vip, ip, ip_version, port_real, user):
        """ Save with commit = True """
        vipporttopool = VipPortToPool.objects.filter(requisicao_vip__id=vip_id, port_vip=port_vip)
        if vipporttopool:
            vipporttopool = vipporttopool[0]
            ServerPoolMember().prepare_and_save(vipporttopool.server_pool, ip, ip_version, 0, 1, port_real, user, commit=True)
...
class VipPortToPool(BaseModel):
    ...
    def get_by_vip_id(cls, id_vip):
        ...
        try:
            ...
            raise RequisicaoVipsError(e, u'Failure to list Request VipPortToPool by id_vip.')  