...
class OptionVipGetByEnvVipTest(OptionVipConfigTest, ConsultationTest):
    ...
    def test_get_by_env_vip_valid(self):
        ...
        valid_response(response)
        content = valid_content(response, self.XML_KEY)
        assert len(content) == len(self.OBJ.objects.select_related().filter(optionvipenvironmentvip__environment=self.ID_ENV_VIP_VALID))
        
    def test_get_by_env_vip_no_permission(self):
        response = self.get_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_env_vip_no_read_permission(self):
        response = self.get_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class OptionVipGetTimeoutByEnvVipTest(OptionVipConfigTest, ConsultationTest):
    ...
    def test_get_timeout_by_env_vip_valid(self):
        ...
        assert len(content) == len(self.OBJ.objects.select_related().filter(tipo_opcao__icontains = 'timeout', optionvipenvironmentvip__environment=self.ID_ENV_VIP_VALID))
        
    def test_get_timeout_by_env_vip_no_permission(self):
        response = self.get_timeout_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_timeout_by_env_vip_no_read_permission(self):
        response = self.get_timeout_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class OptionVipGetGroupCacheByEnvVipTest(OptionVipConfigTest, ConsultationTest):
    ...
    def test_get_group_cache_by_env_vip_valid(self):
        ...
        assert len(content) == len(self.OBJ.objects.select_related().filter(tipo_opcao__icontains = 'cache', optionvipenvironmentvip__environment=self.ID_ENV_VIP_VALID))
        
    def test_get_group_cache_by_env_vip_no_permission(self):
        response = self.get_group_cache_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_group_cache_by_env_vip_no_read_permission(self):
        response = self.get_group_cache_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class OptionVipGetBalancingByEnvVipTest(OptionVipConfigTest, ConsultationTest):
    ...
    def test_get_balancing_by_env_vip_valid(self):
        ...
        assert len(content) == len(self.OBJ.objects.select_related().filter(tipo_opcao__icontains = 'balanceamento', optionvipenvironmentvip__environment=self.ID_ENV_VIP_VALID))
        
    def test_get_balancing_by_env_vip_no_permission(self):
        response = self.get_balancing_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_balancing_by_env_vip_no_read_permission(self):
        response = self.get_balancing_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class OptionVipGetPersistenceByEnvVipTest(OptionVipConfigTest, ConsultationTest):
    ...
    def test_get_persistence_by_env_vip_valid(self):
        ...
        assert len(content) == len(self.OBJ.objects.select_related().filter(tipo_opcao__icontains = 'persistencia', optionvipenvironmentvip__environment=self.ID_ENV_VIP_VALID))
        
    def test_get_persistence_by_env_vip_no_permission(self):
        response = self.get_persistence_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_persistence_by_env_vip_no_read_permission(self):
        response = self.get_persistence_by_env_vip(self.ID_ENV_VIP_VALID, CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class OptionVipAssociateTest(OptionVipConfigTest, AttrTest):
    ...
    def test_associate_valid(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        valid_response(response)
        valid_content(response, "opcoesvip_ambiente_xref")
        
        # Dissociate so it does not require cleaning db manually
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        valid_response(response)
    
    def test_associate_no_permission(self):
        response = self.client_no_permission().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_associate_no__write_permission(self):
        response = self.client_no_write_permission().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_associate_option_vip_nonexistent(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_NONEXISTENT, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._not_found(response, CodeError.OPTION_VIP_NOT_FOUND)
    
    def test_associate_option_vip_negative(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.NEGATIVE_ATTR, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._attr_invalid(response)

    def test_associate_option_vip_letter(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.LETTER_ATTR, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._attr_invalid(response)

    def test_associate_option_vip_zero(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ZERO_ATTR, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._attr_invalid(response)

    def test_associate_option_vip_empty(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.EMPTY_ATTR, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._attr_invalid(response)
    
    def test_associate_option_vip_none(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.NONE_ATTR, self.ID_ENV_VIP_FOR_ASSOCIATION ))
        self._attr_invalid(response)
    
    def test_associate_env_vip_nonexistent(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ID_NONEXISTENT ))
        self._not_found(response, CodeError.ENV_VIP_NOT_FOUND)

    def test_associate_env_vip_negative(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.NEGATIVE_ATTR ))
        self._attr_invalid(response)

    def test_associate_env_vip_letter(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.LETTER_ATTR ))
        self._attr_invalid(response)

    def test_associate_env_vip_zero(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.ZERO_ATTR ))
        self._attr_invalid(response)

    def test_associate_env_vip_empty(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.EMPTY_ATTR ))
        self._attr_invalid(response)
    
    def test_associate_env_vip_none(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_OPTION_VIP_FOR_ASSOCIATION, self.NONE_ATTR ))
        self._attr_invalid(response)
...
class OptionVipDissociateTest(OptionVipConfigTest, AttrTest):
    ...
    def test_dissociate_valid(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        valid_response(response)
    
    def test_dissociate_no_permission(self):
        response = self.client_no_permission().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_dissociate_no__write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_dissociate_option_vip_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_NONEXISTENT, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._not_found(response, CodeError.OPTION_VIP_NOT_FOUND)
    
    def test_dissociate_option_vip_negative(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.NEGATIVE_ATTR, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._attr_invalid(response)
    
    def test_dissociate_option_vip_letter(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.LETTER_ATTR , self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._attr_invalid(response)
    
    def test_dissociate_option_vip_zero(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ZERO_ATTR, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._attr_invalid(response)
    
    def test_dissociate_option_vip_empty(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.EMPTY_ATTR, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._attr_invalid(response)
    
    def test_dissociate_option_vip_none(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.NONE_ATTR, self.ID_ENV_VIP_FOR_DISSOCIATION ))
        self._attr_invalid(response)
    
    def test_dissociate_env_vip_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.ID_NONEXISTENT ))
        self._not_found(response, CodeError.ENV_VIP_NOT_FOUND)
    
    def test_dissociate_env_vip_negative(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.NEGATIVE_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_env_vip_letter(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.LETTER_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_env_vip_zero(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.ZERO_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_env_vip_empty(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.EMPTY_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_env_vip_none(self):
        response = self.client_autenticado().delete(self.URL_DISSOCIATE % ( self.ID_OPTION_VIP_FOR_DISSOCIATION, self.NONE_ATTR ))
        self._attr_invalid(response)
