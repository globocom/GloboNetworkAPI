...
class VipRequestConfigTest(BasicTestCase):
    ...
    def get_by_ipv4(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_GET_BY_IPV4, {self.XML_KEY: mock})
        return response
    ...
    def get_by_ipv6(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_GET_BY_IPV6, {self.XML_KEY: mock})
        return response
    ...
    def search_paginate(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_SEARCH_PAGINATE, {self.XML_KEY: mock})
        return response
    ...
    def create_vip_script(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_CREATE_SCRIPT, {self.XML_KEY: mock})
        return response
    ...
    def remove_vip_script(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_REMOVE_SCRIPT, {self.XML_KEY: mock})
        return response
    ...
    def mock_to_add_vip(self):
        ...
        mock['l7_filter'] = None
        mock['reals'] = {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': '192.168.55.130', 'port_vip': '800', 'id_ip': self.ID_IPV4_TO_ADD_VIP , 'port_real': '8080'}]}
    ...
    def add_vip(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_ADD, {self.XML_KEY: mock})
        return response
    ...
    def add_vip_with_real_different_envs(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_ADD_VIP_REQUEST, {self.XML_KEY: mock})
        return response
    ...
    def mock_to_add_vip_with_real_different_envs(self):
        ...
        mock['timeout'] = '5 ms'
        mock['portas_servicos'] = {'porta':['80:8000']}
        mock['reals'] = {'real':[{'real_name': 'EQUIPVIPTEST2', 'real_ip': '60.60.60.60'}]}
        mock['host'] = 'Host'
        mock['dsr'] = 'dsr'
    ...
    def script_real(self, mock, dicts=None, client=CLIENT_TYPES.TEST):
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_SCRIPT_REAL, {self.XML_KEY: mock})
        return response
    ...
    def edit_reals(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).putXML(self.URL_EDIT_REALS, {self.XML_KEY: mock})
        return response
    ...
    def add2_vip(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_ADD2, {self.XML_KEY: mock})
        return response
    ...
    def mock_to_alter_vip(self):
        ...
        mock['nome_servico'] = 'NomeServico'
        mock['l7_filter'] = None
        mock['reals'] = {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': '192.168.55.130', 'port_vip': '80', 'id_ip': self.ID_IPV4_TO_ADD_VIP , 'port_real': '8090'}]}
        mock['reals_prioritys'] = {'reals_priority':[1]}
        mock['reals_weights'] = {'reals_weight':[1]}
        mock['portas_servicos'] = {'porta':['80:8000']}
        return mock
    ...
    def alter_vip(self, dicts=None, client=CLIENT_TYPES.TEST, vip_id=ID_VIP_TO_ALTER):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).putXML(self.URL_ALTER % vip_id, {self.XML_KEY: mock})
        return response
    ...
    def alter2_vip(self, dicts=None, client=CLIENT_TYPES.TEST, vip_id=ID_VIP_TO_ALTER2):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).putXML(self.URL_ALTER2 % vip_id, {self.XML_KEY: mock})
        return response
    ...
    def alter_healthcheck(self, dicts=None, client=CLIENT_TYPES.TEST, vip_id=ID_VIP_ALTER_HEALTHCHECK):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).putXML(self.URL_ALTER_HEALTHCHECK % vip_id, {self.XML_KEY: mock})
        return response
    ...
    def alter_priority(self, dicts=None, client=CLIENT_TYPES.TEST, vip_id=ID_VIP_ALTER_PRIORITY):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).putXML(self.URL_ALTER_PRIORITY % vip_id, {self.XML_KEY: mock})
        return response
...
class VipRequestConsultationTest(VipRequestConfigTest, ConsultationTest):
    ...
    def test_get_by_ipv6_nonexistent(self):
        response = self.get_by_ipv6(dicts={'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:2122'})
        self._attr_invalid(response, CodeError.IP_NOT_FOUND)

    def test_get_by_ipv6_invalid_chars(self):
        response = self.get_by_ipv6(dicts={'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:uugd'})
        self._attr_invalid(response)

    def test_get_by_ipv6_invalid_range(self):
        response = self.get_by_ipv6(dicts={'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:90122'})
        self._attr_invalid(response)

    def test_get_by_ipv6_invalid_comma(self):
        response = self.get_by_ipv6(dicts={'ipv6': 'ffab,cdef,ffff,ffff,0000,0000,0000,0122'})
        self._attr_invalid(response)
...
class VipRequestValidateTest(VipRequestConfigTest, AttrTest):
    ...
    def test_validate_vip_valid(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.ID_VIP_TO_VALIDATE)
        valid_response(response)

    def test_validate_vip_no_permission(self):
        response = self.client_no_permission().get(self.URL_VALIDATE % self.ID_VIP_TO_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_vip_no_write_permission(self):
        response = self.client_no_write_permission().get(self.URL_VALIDATE % self.ID_VIP_TO_VALIDATE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_validate_vip_nonexistent(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.ID_NONEXISTENT)
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_validate_vip_letter(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_validate_vip_zero(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_validate_vip_negative(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_validate_vip_empty(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.EMPTY_ATTR)
        self._attr_invalid(response)

    def test_validate_vip_none(self):
        response = self.client_autenticado().get(self.URL_VALIDATE % self.NONE_ATTR)
        self._attr_invalid(response)
...
class VipRequestCreateScriptTest(VipRequestConfigTest, AttrTest):
    ...
    def test_create_vip_no_write_permission(self):
        response = self.create_vip_script(client=CLIENT_TYPES.NO_WRITE_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_create_vip_not_validated(self):
        # ID_VALID_VIP_TO_CHECK_REAL isn't validated
        response = self.create_vip_script({'id_vip': self.ID_VALID_VIP_TO_CHECK_REAL})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_YET_VALIDATED)

    def test_create_vip_already_created(self):
        response = self.create_vip_script({'id_vip': self.ID_VIP_ALREADY_CREATED})
...
class VipRequestRemoveScriptTest(VipRequestConfigTest, AttrTest):
    ...
    def test_remove_vip_no_write_permission(self):
        response = self.remove_vip_script(client=CLIENT_TYPES.NO_WRITE_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_vip_not_validated(self):
        # ID_VALID_VIP_TO_CHECK_REAL isn't validated
        response = self.remove_vip_script({'id_vip': self.ID_VALID_VIP_TO_CHECK_REAL})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_YET_VALIDATED)
...
class VipRequestRemoveTest(VipRequestConfigTest, RemoveTest):
    ...
    def test_remove_valid(self):
        response = self.client_autenticado().delete(self.URL_DELETE % self.ID_VIP_TO_DELETE)
        valid_response(response)

    def test_remove_no_permission(self):
        response = self.client_no_permission().delete(self.URL_DELETE % self.ID_VIP_TO_DELETE)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_no_write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_DELETE % self.ID_VIP_TO_DELETE)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class VipRequestAddTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_add_ipv6_nonexistent(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_FOUND)

    def test_vip_add_ipv6_negative(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_ipv6_letter(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_ipv6_empty(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_ipv6_none(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_ipv6_zero(self):
        response = self.add_vip(dicts={'id_ipv4': None, 'id_ipv6': self.ZERO_ATTR})
        self._attr_invalid(response)

    # # Invalid id_healthcheck_expect
    
    def test_vip_add_healthcheck_expect_nonexistent(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.HEALTHCHECKEXPECT_NOT_FOUND)

    
    def test_vip_add_healthcheck_expect_negative(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    
    def test_vip_add_healthcheck_expect_letter(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.LETTER_ATTR})
        self._attr_invalid(response)

    
    def test_vip_add_healthcheck_expect_empty(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    
    def test_vip_add_healthcheck_expect_none(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    
    def test_vip_add_healthcheck_expect_zero(self):
        response = self.add_vip(dicts={'id_healthcheck_expect': self.ZERO_ATTR})
        self._attr_invalid(response)
    ...
    def test_vip_add_finality_nonexistent(self):
        ...
        self._attr_invalid(response, CodeError.VIP_ENVIRONMENT_NOT_FOUND_IN_VIP_ADD)
    ...
    def test_vip_add_healthcheck_type_nonexistent(self):
        response = self.add_vip(dicts={'healthcheck_type': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_HEALTHCHECK_TYPE_VALUE)
    ...
    def test_vip_add_healthcheck_invalid_tcp(self):
        response = self.add_vip(dicts={'healthcheck_type': 'TCP', 'id_healthcheck_expect': self.NONE_ATTR, 'healthcheck': string_generator(10)})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_NOT_HTTP_NOT_NONE)
    ...
    def test_vip_add_service_ports_invalid_first(self):
        response = self.add_vip(dicts={'portas_servicos': {'porta':['njk:8000']}})
        self._attr_invalid(response)

    def test_vip_add_service_ports_invalid_second(self):
        response = self.add_vip(dicts={'portas_servicos': {'porta':['80:---m']}})
        self._attr_invalid(response)

    def test_vip_add_service_ports_empty_first(self):
        response = self.add_vip(dicts={'portas_servicos': {'porta':[':8000']}})
        self._attr_invalid(response)
    ...
    def test_vip_add_service_ports_none(self):
        response = self.add_vip(dicts={'portas_servicos': {'porta': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)

    def test_vip_add_service_ports_empty(self):
        response = self.add_vip(dicts={'portas_servicos': {'porta': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)

    # # Invalid reals
    def test_vip_add_reals_none(self):
        response = self.add_vip(dicts={'reals': {'real': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)
    ...
    def test_vip_add_reals_invalid_equip(self):
        response = self.add_vip(dicts={'reals': {'real':[{'real_name': string_generator(10), 'real_ip': '192.168.55.130'}]}})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_add_reals_invalid_ip(self):
        response = self.add_vip(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': string_generator(10)}]}})
        self._attr_invalid(response)

    def test_vip_add_reals_no_equip(self):
        response = self.add_vip(dicts={'reals': {'real':[{'real_ip': '192.168.55.130'}]}})
        self._attr_invalid(response)
    
    def test_vip_add_reals_no_ip(self):
        response = self.add_vip(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2'}]}})
        self._attr_invalid(response)
    ...
    def test_vip_add_reals_priority_none(self):
        response = self.add_vip(dicts={'reals_prioritys': {'reals_priority': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)

    def test_vip_add_reals_priority_empty(self):
        response = self.add_vip(dicts={'reals_prioritys': {'reals_priority': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)

    def test_vip_add_reals_priority_letter(self):
        response = self.add_vip(dicts={'reals_prioritys': {'reals_priority': [self.LETTER_ATTR]}})
        self._attr_invalid(response)

    def test_vip_add_reals_priority_negative(self):
        response = self.add_vip(dicts={'reals_prioritys': {'reals_priority':[self.NEGATIVE_ATTR]}})
        self._attr_invalid(response)

    def test_vip_add_reals_priority_wrong_number(self):
        response = self.add_vip(dicts={'reals_prioritys': {'reals_priority':[1, 2]}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)

    # # Invalid reals weights
    def test_vip_add_reals_weight_no_dict_entry(self):
        response = self.add_vip(dicts={'reals_weights': {}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)

    def test_vip_add_reals_weight_none(self):
        response = self.add_vip(dicts={'reals_weights': {'reals_weight': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)

    def test_vip_add_reals_weight_empty(self):
        response = self.add_vip(dicts={'reals_weights': {'reals_weight': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)

    def test_vip_add_reals_weight_letter(self):
        response = self.add_vip(dicts={'reals_weights': {'reals_weight': [self.LETTER_ATTR]}})
        self._attr_invalid(response)

    def test_vip_add_reals_weight_negative(self):
        response = self.add_vip(dicts={'reals_weights': {'reals_weight':[self.NEGATIVE_ATTR]}})
        self._attr_invalid(response)

    def test_vip_add_reals_weight_wrong_number(self):
        response = self.add_vip(dicts={'reals_weights': {'reals_weight':[1, 2]}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)
...
class VipRequestCheckRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_check_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_check_real(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_check_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_check_real(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_check_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_check_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_check_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_check_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_check_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_check_real(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestCheckRealsIpv6Test(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_check_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_check_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_check_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_check_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_check_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_check_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_check_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_check_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_check_real_ipv6(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestAddRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_add_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_add_real(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_add_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_add_real(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_add_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_add_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_add_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_add_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_add_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_add_real(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestAddRealsIpv6Test(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_add_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_add_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_add_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_add_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_add_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_add_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_add_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_add_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_add_real_ipv6(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestEditRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_edit_reals_valid_add_new_real(self):
        response = self.edit_reals(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': '192.168.55.136', 'port_vip': '81', 'id_ip': self.ID_IPV4_TO_ADD_REAL , 'port_real': '8099'}, \
                                                            {'real_name': 'BalancingEquipForVip2', 'real_ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:0126', 'port_vip': '81', 'id_ip': self.ID_IPV6_TO_ADD_REAL , 'port_real': '8591'}, \
    ...
    def test_vip_edit_reals_add_new_wrong_priority(self):
        response = self.edit_reals(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': '192.168.55.136'}, \
                                                    ...
                                                    'reals_weights': {'reals_weight':[1, 2, 3]}})
        self._attr_invalid(response, CodeError.VIP_EDIT_REALS_PRIORITY_INCONSISTENCY)
    ...
    def test_vip_edit_reals_add_new_wrong_weights(self):
        ...
        self._attr_invalid(response, CodeError.VIP_EDIT_REALS_WEIGHT_INCONSISTENCY)
...
class VipRequestAdd2Test(VipRequestConfigTest, AttrTest):
    ...
    # # Invalid id_healthcheck_expect
    def test_vip_add_healthcheck_expect_nonexistent(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.HEALTHCHECKEXPECT_NOT_FOUND)

    def test_vip_add_healthcheck_expect_negative(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_add_healthcheck_expect_letter(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_add_healthcheck_expect_empty(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_add_healthcheck_expect_none(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_add_healthcheck_expect_zero(self):
        response = self.add2_vip(dicts={'id_healthcheck_expect': self.ZERO_ATTR})
        self._attr_invalid(response)
    ...
    # # Invalid healthcheck_type
    def test_vip_add_healthcheck_type_nonexistent(self):
        response = self.add2_vip(dicts={'healthcheck_type': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_HEALTHCHECK_TYPE_VALUE)
    ...
    def test_vip_add_healthcheck_invalid_tcp(self):
        response = self.add2_vip(dicts={'healthcheck_type': 'TCP', 'id_healthcheck_expect': self.NONE_ATTR, 'healthcheck': string_generator(10)})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_NOT_HTTP_NOT_NONE)
    ...
    # # Invalid service ports
    def test_vip_add_service_ports_invalid_first(self):
        response = self.add2_vip(dicts={'portas_servicos': {'porta':['njk:8000']}})
        self._attr_invalid(response)

    def test_vip_add_service_ports_invalid_second(self):
        response = self.add2_vip(dicts={'portas_servicos': {'porta':['80:---m']}})
        self._attr_invalid(response)

    def test_vip_add_service_ports_empty_first(self):
        response = self.add2_vip(dicts={'portas_servicos': {'porta':[':8000']}})
        self._attr_invalid(response)
    ...
    def test_vip_add_service_ports_none(self):
        response = self.add2_vip(dicts={'portas_servicos': {'porta': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)

     
    def test_vip_add_service_ports_empty(self):
        response = self.add2_vip(dicts={'portas_servicos': {'porta': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)
    ...
    def test_vip_add_reals_no_equip(self):
        response = self.add2_vip(dicts={'reals': {'real':[{'real_ip': '192.168.55.130'}]}})
        self._attr_invalid(response)

    def test_vip_add_reals_no_ip(self):
        response = self.add2_vip(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2'}]}})
        self._attr_invalid(response)
        #valid_response(response)
...
class VipRequestAlterTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_alter_ipv6_valid(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.ID_IPV6_TO_ADD_VIP})
        valid_response(response)
    ...
    def test_vip_alter_ipv6_nonexistent(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_FOUND)

    def test_vip_alter_ipv6_negative(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_ipv6_letter(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_ipv6_empty(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_ipv6_none(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_ipv6_zero(self):
        response = self.alter_vip(dicts={'id_ipv4': None, 'id_ipv6': self.ZERO_ATTR})
        self._attr_invalid(response)

    # # Invalid id_healthcheck_expect
    def test_vip_alter_healthcheck_expect_nonexistent(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.HEALTHCHECKEXPECT_NOT_FOUND)

    def test_vip_alter_healthcheck_expect_negative(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_healthcheck_expect_letter(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_healthcheck_expect_empty(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_alter_healthcheck_expect_none(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_alter_healthcheck_expect_zero(self):
        response = self.alter_vip(dicts={'id_healthcheck_expect': self.ZERO_ATTR})
        self._attr_invalid(response)
    ...
    def test_vip_alter_healthcheck_type_nonexistent(self):
        response = self.alter_vip(dicts={'healthcheck_type': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_HEALTHCHECK_TYPE_VALUE)
    ...
    def test_vip_alter_healthcheck_invalid_tcp(self):
        response = self.alter_vip(dicts={'healthcheck_type': 'TCP', 'id_healthcheck_expect': self.NONE_ATTR, 'healthcheck': string_generator(10)})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_NOT_HTTP_NOT_NONE)
    ...
    def test_vip_alter_service_ports_invalid_first(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta':['njk:8000']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_invalid_second(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta':['80:---m']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_empty_first(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta':[':8000']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_empty_second(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta':['80:']}})
        self._attr_invalid(response)
    ...
    def test_vip_alter_service_ports_none(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)

     
    def test_vip_alter_service_ports_empty(self):
        response = self.alter_vip(dicts={'portas_servicos': {'porta': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)
    ...
    def test_vip_alter_reals_invalid_equip(self):
        response = self.alter_vip(dicts={'reals': {'real':[{'real_name': string_generator(10), 'real_ip': '192.168.55.130'}]}})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_alter_reals_invalid_ip(self):
        response = self.alter_vip(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2', 'real_ip': string_generator(10)}]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_no_equip(self):
        response = self.alter_vip(dicts={'reals': {'real':[{'real_ip': '192.168.55.130'}]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_no_ip(self):
        response = self.alter_vip(dicts={'reals': {'real':[{'real_name': 'BalancingEquipForVip2'}]}})
        self._attr_invalid(response)
    ...
    def test_vip_alter_reals_priority_none(self):
        response = self.alter_vip(dicts={'reals_prioritys': {'reals_priority': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)

    def test_vip_alter_reals_priority_empty(self):
        response = self.alter_vip(dicts={'reals_prioritys': {'reals_priority': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)

    def test_vip_alter_reals_priority_letter(self):
        response = self.alter_vip(dicts={'reals_prioritys': {'reals_priority': [self.LETTER_ATTR]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_priority_negative(self):
        response = self.alter_vip(dicts={'reals_prioritys': {'reals_priority':[self.NEGATIVE_ATTR]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_priority_wrong_number(self):
        response = self.alter_vip(dicts={'reals_prioritys': {'reals_priority':[1, 2]}})
        self._attr_invalid(response, CodeError.VIP_REALS_PRIORITY_WRONG_NUMBER_LIST)
    ...
    def test_vip_alter_reals_weight_none(self):
        response = self.alter_vip(dicts={'reals_weights': {'reals_weight': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)

    def test_vip_alter_reals_weight_empty(self):
        response = self.alter_vip(dicts={'reals_weights': {'reals_weight': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)

    def test_vip_alter_reals_weight_letter(self):
        response = self.alter_vip(dicts={'reals_weights': {'reals_weight': [self.LETTER_ATTR]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_weight_negative(self):
        response = self.alter_vip(dicts={'reals_weights': {'reals_weight':[self.NEGATIVE_ATTR]}})
        self._attr_invalid(response)

    def test_vip_alter_reals_weight_wrong_number(self):
        response = self.alter_vip(dicts={'reals_weights': {'reals_weight':[1, 2]}})
        self._attr_invalid(response, CodeError.VIP_REALS_WEIGHT_WRONG_NUMBER_LIST)
...
class VipRequestAlter2Test(VipRequestConfigTest, AttrTest):
    ...
    # # Invalid id_healthcheck_expect
    def test_vip_alter_healthcheck_expect_nonexistent(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.HEALTHCHECKEXPECT_NOT_FOUND)

    def test_vip_alter_healthcheck_expect_negative(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_healthcheck_expect_letter(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_alter_healthcheck_expect_empty(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_alter_healthcheck_expect_none(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_HTTP_NOT_NONE)

    def test_vip_alter_healthcheck_expect_zero(self):
        response = self.alter2_vip(dicts={'id_healthcheck_expect': self.ZERO_ATTR})
        self._attr_invalid(response)
    ...
    # # Invalid persistence
    def test_vip_alter_persistence_nonexistent(self):
        response = self.alter2_vip(dicts={'persistencia': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_PERSISTENCE_VALUE)
    ...
    def test_vip_alter_persistence_maxsize(self):
        response = self.alter2_vip(dicts={'persistencia': string_generator(51)})
        self._attr_invalid(response, CodeError.VIP_INVALID_PERSISTENCE_VALUE)
    ...
    # # Invalid healthcheck_type
    def test_vip_alter_healthcheck_type_nonexistent(self):
        response = self.alter2_vip(dicts={'healthcheck_type': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_HEALTHCHECK_TYPE_VALUE)
    ...
    def test_vip_alter_healthcheck_invalid_tcp(self):
        response = self.alter2_vip(dicts={'healthcheck_type': 'TCP', 'id_healthcheck_expect': self.NONE_ATTR, 'healthcheck': string_generator(10)})
        self._attr_invalid(response, CodeError.HEALTHCHECK_EXPECT_NOT_HTTP_NOT_NONE)
    ...
    # # Invalid service ports
    def test_vip_alter_service_ports_invalid_first(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta':['njk:8000']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_invalid_second(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta':['80:---m']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_empty_first(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta':[':8000']}})
        self._attr_invalid(response)

    def test_vip_alter_service_ports_empty_second(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta':['80:']}})
        self._attr_invalid(response)
    ...
    def test_vip_alter_service_ports_none(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta': self.NONE_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)

     
    def test_vip_alter_service_ports_empty(self):
        response = self.alter2_vip(dicts={'portas_servicos': {'porta': self.EMPTY_ATTR}})
        self._attr_invalid(response, CodeError.VIP_INVALID_SERVICE_PORTS_VALUE)
...
class VipRequestEnableRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_enable_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_enable_real(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_enable_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_enable_real(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_enable_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_enable_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_enable_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_enable_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_enable_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_enable_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_enable_real(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestDisableRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_disable_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_disable_real(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_disable_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_disable_real(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_disable_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_disable_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_disable_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_disable_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_disable_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_disable_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_disable_real(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestEnableRealsV6Test(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_enable_real_v6_no_permission(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_enable_real_v6_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_enable_real_v6_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_enable_real_v6_vip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_vip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_vip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_vip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_vip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_enable_real_v6_ip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_ip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_ip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_ip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_ip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_enable_real_v6_equip_negative(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_equip_zero(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_equip_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_equip_none(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_equip_letter(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_operation_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_operation_none(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_enable_real_v6_network_version_none(self):
        response = self.script_real(mock=self.mock_to_enable_real_v6(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestDisableRealsV6Test(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_disable_real_v6_no_permission(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_disable_real_v6_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_disable_real_v6_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_disable_real_v6_vip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_vip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_vip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_vip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_vip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_disable_real_v6_ip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_ip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_ip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_ip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_ip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_disable_real_v6_equip_negative(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_equip_zero(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_equip_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_equip_none(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_equip_letter(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_operation_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_operation_none(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_disable_real_v6_network_version_none(self):
        response = self.script_real(mock=self.mock_to_disable_real_v6(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestRemoveRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_remove_real_no_permission(self):
        response = self.script_real(mock=self.mock_to_remove_real(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_remove_real_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_remove_real(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_remove_real_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_remove_real_vip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_vip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_vip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_vip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_vip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_remove_real_ip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_ip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_ip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_ip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_ip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_remove_real_equip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_equip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_equip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_equip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_equip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_remove_real_operation_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_operation_none(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_remove_real_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_network_version_none(self):
        response = self.script_real(mock=self.mock_to_remove_real(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestRemoveRealsV6Test(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_remove_real_v6_no_permission(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_remove_real_v6_no_read_permission(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_remove_real_v6_vip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_vip_remove_real_v6_vip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_vip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_vip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_vip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_vip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'vip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_ip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.IP_NOT_REGISTERED_FOR_EQUIP)

    def test_vip_remove_real_v6_ip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_ip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_ip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_ip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_ip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'ip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_equip_nonexistent(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_vip_remove_real_v6_equip_negative(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.NEGATIVE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_equip_zero(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.ZERO_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_equip_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_equip_none(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_equip_letter(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'equip_id': self.LETTER_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_operation_invalid(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'operation': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_operation_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'operation': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_operation_none(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'operation': self.NONE_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_network_version_invalid(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'network_version': string_generator(10)})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_network_version_empty(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'network_version': self.EMPTY_ATTR})
        self._attr_invalid(response)

    def test_vip_remove_real_v6_network_version_none(self):
        response = self.script_real(mock=self.mock_to_remove_real_v6(), dicts={'network_version': self.NONE_ATTR})
        self._attr_invalid(response)
...
class VipRequestValidateRealsTest(VipRequestConfigTest, AttrTest):
    ...
    def test_vip_validate_real_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_VALIDATE_REAL, {'real': mock})
        return response

    def test_vip_validate_real_no_permission(self):
        mock = self.mock_to_valid_real()
        response = self.client_no_permission().postXML(self.URL_VALIDATE_REAL, { 'real' : mock })
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_vip_validate_real_no_read_permission(self):
        mock = self.mock_to_valid_real()
        response = self.client_no_read_permission().postXML(self.URL_VALIDATE_REAL, { 'real' : mock })
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class VipRequestAlterHealthcheckTest(VipRequestConfigTest, AttrTest):
    ...
    def test_alter_healthcheck_no_write_permission(self):
        response = self.alter_healthcheck(client=CLIENT_TYPES.NO_WRITE_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_alter_healthcheck_healthcheck_type_invalid_str(self):
        response = self.alter_healthcheck(dicts={'healthcheck_type': string_generator(10)})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
    ...
    def test_alter_healthcheck_healthcheck_type_empty(self):
        response = self.alter_healthcheck(dicts={'healthcheck_type': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_healthcheck_type_none(self):
        response = self.alter_healthcheck(dicts={'healthcheck_type': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    # # healthcheck
    def test_alter_healthcheck_healthcheck_empty(self):
        response = self.alter_healthcheck(dicts={'healthcheck': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
    ...
    def test_alter_healthcheck_id_healthcheck_expect_negative(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.NEGATIVE_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_id_healthcheck_expect_zero(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.ZERO_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_id_healthcheck_expect_letter(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.LETTER_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_id_healthcheck_expect_nonexistent(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_id_healthcheck_expect_empty(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.EMPTY_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_healthcheck_id_healthcheck_expect_none(self):
        response = self.alter_healthcheck(dicts={'id_healthcheck_expect': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
...
#    'reals_prioritys': {'reals_priority': [2]}
    def test_alter_priority_priority_letter(self):
        response = self.alter_priority(dicts={'reals_prioritys': {'reals_priority': [self.LETTER_ATTR]}})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_priority_priority_negative(self):
        response = self.alter_priority(dicts={'reals_prioritys': {'reals_priority': [self.NEGATIVE_ATTR]}})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_priority_priority_empty(self):
        response = self.alter_priority(dicts={'reals_prioritys': {'reals_priority': [self.EMPTY_ATTR]}})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_priority_priority_none(self):
        response = self.alter_priority(dicts={'reals_prioritys': {'reals_priority': [self.NONE_ATTR]}})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_alter_priority_priority_wrong_number(self):
        response = self.alter_priority(dicts={'reals_prioritys': {'reals_priority': ['n', -2]}})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
    ...
    def test_alter_priority_priority_none_dict(self):
        response = self.alter_priority(dicts={'reals_prioritys': self.NONE_ATTR})
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
...
class VipRequestEditMaxconTest(VipRequestConfigTest, RemoveTest):
    ...
    def test_edit_valid(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.MAXCON_VALID))
        valid_response(response)

    def test_edit_no_permission(self):
        response = self.client_no_permission().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.MAXCON_VALID))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_edit_no_write_permission(self):
        response = self.client_no_write_permission().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.MAXCON_VALID))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_edit_vip_nonexistent(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ID_NONEXISTENT, self.MAXCON_VALID))
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

    def test_edit_vip_negative(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.NEGATIVE_ATTR, self.MAXCON_VALID))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_vip_letter(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.LETTER_ATTR, self.MAXCON_VALID))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_vip_zero(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ZERO_ATTR, self.MAXCON_VALID))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_vip_empty(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.EMPTY_ATTR, self.MAXCON_VALID))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_maxcon_negative(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.NEGATIVE_ATTR))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_maxcon_letter(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.LETTER_ATTR))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)

    def test_edit_maxcon_empty(self):
        response = self.client_autenticado().put(self.URL_EDIT_MAXCON % (self.ID_VALID_VIP_EDIT_MAXCON, self.EMPTY_ATTR))
        self._attr_invalid(response, CodeError.VIP_INVALID_PARAMETER)
...
class VipRequestConsultationSearchTest(VipRequestConfigTest, ConsultationTest):
    ...
    def test_search_paginate_ipv6_valid(self):
        response = self.search_paginate(dicts={'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:0122'})
        valid_response(response)
        total = valid_content(response, 'total')
        content = valid_content(response, 'vips')
        if type(content) == dict:
    ...
    def test_search_paginate_ipv6_nonexistent(self):
        response = self.search_paginate(dicts={'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:2122'})
        valid_response(response)
        total = valid_content(response, 'total')
        assert int(total) == 0

    def test_search_paginate_ipv6_invalid_chars(self):
        response = self.search_paginate(dicts={'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:uugd'})
        self._attr_invalid(response)

    def test_search_paginate_ipv6_invalid_range(self):
        response = self.search_paginate(dicts={'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:90122'})
        self._attr_invalid(response)
...
class VipRequestAddBlock(VipRequestConfigTest, AttrTest):
    ...
    def test_add_block_valid(self):
        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK, self.ID_BLOCK, self.OVERRIDE))
        valid_response(response)

    def test_add_block_nonexistent(self):
        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.NONEXISTENT, self.ID_BLOCK, self.OVERRIDE))
        self._attr_invalid(response, CodeError.VIP_REQUEST_NOT_FOUND)

        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK, self.NONEXISTENT, self.OVERRIDE))
        self._attr_invalid(response, CodeError.BLOCK_NOT_FOUND)

    def test_add_block_no_permission(self):
        response = self.client_no_permission().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK, self.ID_BLOCK, self.OVERRIDE))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_add_block_no_write_permission(self):
        response = self.client_no_write_permission().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK, self.ID_BLOCK, self.OVERRIDE))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_add_block_no_block_associated(self):
        # No block for environment
        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_WITHOUT_BLOCK_IN_ENVIRONMENT, self.ID_BLOCK, self.OVERRIDE))
        self._attr_invalid(response, CodeError.BLOCK_NOT_FOUND)

    def test_add_block_already_in_rule(self):
        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK, self.ID_BLOCK_ALREADY_IN_RULE, self.OVERRIDE))
        self._attr_invalid(response, CodeError.VIPREQUESTBLOCKALREADYINRULE)

    def test_add_block_no_blocks_in_filter(self):
        response = self.client_autenticado().get(self.URL_ADD_BLOCK % (self.ID_VIP_TO_ADD_BLOCK_NO_BLOCKS_IN_FILTER, self.ID_BLOCK, self.OVERRIDE))
