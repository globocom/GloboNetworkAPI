...
# Método que processará o signal enviado após a invocação do método save dos models 
def networkapi_post_save(sender, instance, created, **kwargs):
    
    # Não podemos logar o save da classe EventLog já que isso invocaria um novo signal gerando um loop infinito
        ...
        if not (str(val) == '_state'):
            if not str(val)[0] == '_':
                if str(val) == 'pwd' or str(val) == 'password' or str(val) == 'enable_pass':
                    ...
                    parametro_atual += str(val) + ' = ' + str(values_map[val]) + ' / '
    ...
    if created:
        ...
        if (values_map['id']):
            id_objeto = values_map['id']
            try:
                parametro_anterior = EventLog.objects.values_list('parametro_atual', flat=True).filter(id_objeto=id_objeto, funcionalidade=classe.__name__).order_by('-hora_evento')[0]
            except:
                parametro_anterior = ''
...
# Método que processará o signal enviado após a invocação do método delete dos models 
        ...
        if not (str(val) == '_state'):
            if not str(val)[0] == '_':
                if str(val) == 'pwd' or str(val) == 'password' or str(val) == 'enable_pass':
                    ...
                    parametro_anterior += str(val) + ' = ' + str(values_map[val]) + ' / '
