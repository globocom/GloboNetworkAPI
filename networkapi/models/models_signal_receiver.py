...
def networkapi_post_save(sender, instance, created, **kwargs):
    ...
    if (not issubclass(instance.__class__, BaseModel)) or (
            instance.__class__ is EventLog):
        return
    ...
    for val in values_map:
        if not (str(val) == '_state'):
            if not str(val)[0] == '_':
                if str(val) == 'pwd' or str(
                        val) == 'password' or str(val) == 'enable_pass':
    ...
    if created:
        ...
        if (values_map['id']):
            id_objeto = values_map['id']
            try:
                parametro_anterior = EventLog.objects.values_list(
                    'parametro_atual',
                    flat=True).filter(
                    id_objeto=id_objeto,
                    funcionalidade=classe.__name__).order_by('-hora_evento')[0]
            except:
                parametro_anterior = ''
