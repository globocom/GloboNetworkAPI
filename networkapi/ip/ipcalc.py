...
class IP(object):
    ...
    def _dqtoi(self, dq):
        ...
        if ':' in dq:
            ...
            if len(hx) > 8:
                ...
                if not '' in hx:
                    raise ValueError(
                        "%r: IPv6 address invalid: compressed format malformed" %
                        dq)
        ...
        if '.' in dq:
            ...
            if len(q) > 4:
                raise ValueError(
                    "%r: IPv4 address invalid: more than 4 bytes" %
                    dq)
            for x in q:
                if 0 > int(x) > 255:
                    raise ValueError(
    ...
    def _itodq(self, n):
        ...
        if self.v == 4:
            return '.'.join(
                map(str, [(n >> 24) & 0xff, (n >> 16) & 0xff, (n >> 8) & 0xff, n & 0xff]))
        else:
            n = '%032x' % n
    ...
    def to_ipv4(self):
        ...
        if self.v == 4:
            ...
            elif long(self) & 0x20020000000000000000000000000000:
                return IP(
                    (long(self) - 0x20020000000000000000000000000000) >> 80,
                    version=4)
            else:
    ...
    def to_ipv6(self, type='6-to-4'):
        ...
        if self.v == 4:
            if type == '6-to-4':
                return IP(
                    0x20020000000000000000000000000000 | long(self) << 80,
                    version=6)
            elif type == 'compat':
...
class Network(IP):
    ...
    def in_network(self, other):
        ...
        '''
        other = Network(other)
        return long(other) >= long(self) and long(
            other) < long(self) + self.size() - other.size() + 1
