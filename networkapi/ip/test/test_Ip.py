...
class IpConfigTest(BasicTestCase):
    ...
    def search_ipv6_env(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_GET_BY_IPV6_ENV, {'ipv6_map' : mock})
        return response
    
    def get4or6(self, arg, client=CLIENT_TYPES.TEST):
        mock = {}
        mock['ip'] = arg
        response = self.switch(client).postXML(self.URL_GET_IP4_OR_IP6, {'ip_map' : mock})
        return response
    ...
    def assoc_ipv4(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_ASSOC_IPV4, {'ip_map' : mock})
        return response
    ...
    def assoc_ipv6(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_ASSOC_IPV6, {'ip_map' : mock})
        return response
    ...
    def check_vip_ip(self, dicts=None, client=CLIENT_TYPES.TEST):
        ...
        if isinstance(dicts, dict):
            for (k, v) in dicts.iteritems():
                mock[k] = v
        response = self.switch(client).postXML(self.URL_CHECK_VIP_IP, {'ip_map' : mock})
        return response
...
class IpGetByIpEnvConsultationTest(IpConfigTest, AttrTest):
    ...
    def test_get_by_ip_env_valid(self):       
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.ID_ENVIRONMENT_VALID))
        valid_response(response)
        valid_content(response, 'ip')
        
    def test_get_by_ip_env_no_permission(self):       
        response = self.client_no_permission().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.ID_ENVIRONMENT_VALID))
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_ip_env_no_read_permission(self):       
        response = self.client_no_read_permission().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.ID_ENVIRONMENT_VALID))
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_ip_env_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.ID_NONEXISTENT))
        self._attr_invalid(response, CodeError.ENVIRONMENT_NOT_FOUND)
        
    def test_get_by_ip_env_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.NEGATIVE_ATTR))
        self._attr_invalid(response)
        
    def test_get_by_ip_env_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.LETTER_ATTR))
        self._attr_invalid(response)
        
    def test_get_by_ip_env_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.ZERO_ATTR))
        self._attr_invalid(response)
    
    def test_get_by_ip_env_empty(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % (self.IP_VALID_FOR_GET_IP_ENV, self.EMPTY_ATTR))
        self._attr_invalid(response)
    
    def test_get_by_ip_nonexistent_env(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % ('192.168.30.1', self.ID_ENVIRONMENT_VALID))
        self._attr_invalid(response, CodeError.IP_NOT_FOUND)
        
    def test_get_by_ip_invalid_chars_env(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % ('192.168.20.1za', self.ID_ENVIRONMENT_VALID))
        self._attr_invalid(response)
        
    def test_get_by_ip_invalid_range_env(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % ('192.168.20.1000', self.ID_ENVIRONMENT_VALID))
        self._attr_invalid(response)
        
    def test_get_by_ip_invalid_comma_env(self):
        response = self.client_autenticado().get(self.URL_GET_BY_IP_ENV % ('192,168,20,11', self.ID_ENVIRONMENT_VALID))
        self._attr_invalid(response)
...
class Ipv6GetByIpEnvConsultationTest(IpConfigTest, AttrTest):
    ...
    def test_get_by_ipv6_env_nonexistent(self):
        response = self.search_ipv6_env({'id_environment': self.ID_NONEXISTENT})
        self._attr_invalid(response, CodeError.ENVIRONMENT_NOT_FOUND)
    ...
    def test_get_by_ipv6_nonexistent_env(self):
        response = self.search_ipv6_env({'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:0015'})
        self._attr_invalid(response, CodeError.IP_NOT_FOUND)
        
    def test_get_by_ipv6_invalid_chars_env(self):
        response = self.search_ipv6_env({'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:0t1g'})
        self._attr_invalid(response)
        
    def test_get_by_ipv6_invalid_range_env(self):
        response = self.search_ipv6_env({'ipv6': 'ffab:cdef:ffff:ffff:0000:0000:0000:150000'})
        self._attr_invalid(response)
        
    def test_get_by_ipv6_invalid_comma_env(self):
        response = self.search_ipv6_env({'ipv6': 'ffab,cdef,ffff,ffff,0000,0000,0000,0011'})
        self._attr_invalid(response)
...
class IpGet4or6ConsultationTest(IpConfigTest, AttrTest):
    ...
    def test_get_4or6_no_permission(self):
        response = self.get4or6('192.168.20.11', client=CLIENT_TYPES.NO_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_4or6_no_read_permission(self):
        response = self.get4or6('192.168.20.11', client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class IpGetAvailableIpv4ConsultationTest(IpConfigTest, AttrTest):
    ...
    def test_get_available_ip4_valid(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response)
        valid_content(response, 'ip')
        
    def test_get_available_ip4_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_AVAILABLE_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_available_ip4_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_AVAILABLE_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_available_ip4_net_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.NETWORK_IPV4_NOT_FOUND)
        
    def test_get_available_ip4_net_negative(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip4_net_zero(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip4_net_empty(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.EMPTY_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip4_net_none(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.NONE_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip4_net_letter(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP4 % self.LETTER_ATTR)
        self._attr_invalid(response)
        
class IpGetAvailableIpv6ConsultationTest(IpConfigTest, AttrTest):
    
    def test_get_available_ip6_valid(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response)
        valid_content(response, 'ip6')
        
    def test_get_available_ip6_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_AVAILABLE_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_available_ip6_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_AVAILABLE_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_available_ip6_net_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.NETWORK_IPV6_NOT_FOUND)
        
    def test_get_available_ip6_net_negative(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip6_net_zero(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip6_net_empty(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.EMPTY_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip6_net_none(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.NONE_ATTR)
        self._attr_invalid(response)
        
    def test_get_available_ip6_net_letter(self):
        response = self.client_autenticado().get(self.URL_GET_AVAILABLE_IP6 % self.LETTER_ATTR)
        self._attr_invalid(response)
        
class IpFindByNetIp4ConsultationTest(IpConfigTest, AttrTest):
    
    def test_get_by_net_ip4_valid(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response)
        valid_content(response, self.XML_KEY)
        
    def test_get_by_net_ip4_no_permission(self):
        response = self.client_no_permission().get(self.URL_FIND_BY_NET_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_net_ip4_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_FIND_BY_NET_IP4 % self.ID_NETIPV4_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_net_ip4_net_nonexistent(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.NETWORK_IPV4_NOT_FOUND)
        
    def test_get_by_net_ip4_net_negative(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip4_net_zero(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip4_net_empty(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.EMPTY_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip4_net_none(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.NONE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip4_net_letter(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP4 % self.LETTER_ATTR)
        self._attr_invalid(response)
        
class IpFindByNetIp6ConsultationTest(IpConfigTest, AttrTest):
    
    def test_get_by_net_ip6_valid(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response)
        valid_content(response, self.XML_KEY)
        
    def test_get_by_net_ip6_no_permission(self):
        response = self.client_no_permission().get(self.URL_FIND_BY_NET_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_net_ip6_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_FIND_BY_NET_IP6 % self.ID_NETIPV6_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_net_ip6_net_nonexistent(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.NETWORK_IPV6_NOT_FOUND)
        
    def test_get_by_net_ip6_net_negative(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip6_net_zero(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip6_net_empty(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.EMPTY_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip6_net_none(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.NONE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_net_ip6_net_letter(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_NET_IP6 % self.LETTER_ATTR)
        self._attr_invalid(response)
        
class IpFindByEquipConsultationTest(IpConfigTest, AttrTest):
    
    def test_find_by_equip_valid(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.ID_EQUIP_VALID)
        valid_response(response)
        valid_content(response, self.XML_KEY)
    
    def test_find_by_equip_no_permission(self):
        response = self.client_no_permission().get(self.URL_FIND_BY_EQUIP % self.ID_EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_find_by_equip_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_FIND_BY_EQUIP % self.ID_EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_find_by_equip_nonexistent(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)
        
    def test_find_by_equip_negative(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_find_by_equip_zero(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_find_by_equip_empty(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.EMPTY_ATTR)
        self._attr_invalid(response)
        
    def test_find_by_equip_none(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.NONE_ATTR)
        self._attr_invalid(response)
        
    def test_find_by_equip_letter(self):
        response = self.client_autenticado().get(self.URL_FIND_BY_EQUIP % self.LETTER_ATTR)
        self._attr_invalid(response)
...
class Ipv4Test(IpConfigTest, AttrTest):
    ...
    def test_save_ipv4_no_write_permission(self):
        mock = self.mock_save_ipv4_valid()
        response = self.save({'ip_map': mock}, CLIENT_TYPES.NO_WRITE_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_alter_no_permission(self):
        mock = self.mock_alter_ipv4_valid()
        response = self.client_no_permission().postXML(self.URL_ALTER_IPV4, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_alter_no_write_permission(self):
        mock = self.mock_alter_ipv4_valid()
        response = self.client_no_write_permission().postXML(self.URL_ALTER_IPV4, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class Ipv6Test(IpConfigTest, AttrTest):
    ...
    def test_alter_no_permission(self):
        mock = self.mock_alter_ipv6_valid()
        response = self.client_no_permission().postXML(self.URL_ALTER_IPV6, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_alter_no_write_permission(self):
        mock = self.mock_alter_ipv6_valid()
        response = self.client_no_write_permission().postXML(self.URL_ALTER_IPV6, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class Ipv4RemoveTest(IpConfigTest, RemoveTest):
    ...
    def test_remove_no_permission(self):
        response = self.client_no_permission().get(self.URL_REMOVE % self.ID_IPV4_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_remove_no_write_permission(self):
        response = self.client_no_write_permission().get(self.URL_REMOVE % self.ID_IPV4_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class Ipv6RemoveTest(IpConfigTest, RemoveTest):
    ...
    def test_remove_no_permission(self):
        response = self.client_no_permission().get(self.URL_REMOVE % self.ID_IPV6_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_remove_no_write_permission(self):
        response = self.client_no_write_permission().get(self.URL_REMOVE % self.ID_IPV6_REMOVE_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class Ipv4NetworkAttrTest(IpConfigTest, AttrTest):
    ...
    def test_save_nonexistent(self):
        self.save_ipv4_attr_invalid(self.ID_NONEXISTENT, CodeError.NETWORK_IPV4_NOT_FOUND)
...
class Ipv4EquipAttrTest(IpConfigTest, AttrTest):
    ...
    def test_save_nonexistent(self):
        self.save_ipv4_attr_invalid(self.ID_NONEXISTENT, CodeError.EQUIPMENT_NOT_FOUND)
...
class Ipv6NetworkAttrTest(IpConfigTest, AttrTest):
    ...
    def test_save_nonexistent(self):
        self.save_ipv6_attr_invalid(self.ID_NONEXISTENT, CodeError.NETWORK_IPV6_NOT_FOUND)
...
class Ipv6IpAttrTest(IpConfigTest, AttrTest):
    ...
    def test_alter_range(self):
        self.alter_ipv6_attr_invalid('ffab:cdef:ffff:ffff:0000:0000:0000:90101')
...
class Ipv6EquipAttrTest(IpConfigTest, AttrTest):
    ...
    def test_save_nonexistent(self):
        self.save_ipv6_attr_invalid(self.ID_NONEXISTENT, CodeError.EQUIPMENT_NOT_FOUND)
...
class IpCheckVipIpTest(IpConfigTest, AttrTest):
    ...
    def test_check_vip_valid_ipv6(self):
        response = self.check_vip_ip(dicts={'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:0121'})
        valid_response(response)
        valid_content(response, 'ip')
    ...
    def test_check_vip_comma_ip6(self):
        response = self.check_vip_ip({'ip': 'ffab,cdef,ffff,ffff,0000,0000,0000,0121'})
        self._attr_invalid(response)
    ...
    def test_check_vip_range_ip6(self):
        response = self.check_vip_ip({'ip': 'ffab:cdef:ffff:ffff:0000:0000:0000:90121'})
        self._attr_invalid(response)
...
class IpGetAvailableIpv4forVip(IpConfigTest, AttrTest):
    ...
    def test_get_available_for_vip_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
        valid_response(response)
        valid_content(response, 'ip')
    ...
    def test_get_available_for_vip_no_permission(self):
        ...
        response = self.client_no_permission().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_get_available_for_vip_no_write_permission(self):
        ...
        response = self.client_no_write_permission().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_get_available_for_vip_env_vip_nonexistent(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.ID_NONEXISTENT, {'ip_map': mock})
        self._attr_invalid(response, CodeError.ENV_VIP_NOT_FOUND)
    ...
    def test_get_available_for_vip_env_vip_letter(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.LETTER_ATTR, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_zero(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.ZERO_ATTR, {'ip_map': mock})
        self._attr_invalid(response)
    ...
    def test_get_available_for_vip_env_vip_negative(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.NEGATIVE_ATTR, {'ip_map': mock})
        self._attr_invalid(response)
    ...
    def test_get_available_for_vip_env_vip_empty(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.EMPTY_ATTR, {'ip_map': mock})
        self._attr_invalid(response)
    ...
    def test_get_available_for_vip_env_vip_none(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV4 % self.NONE_ATTR, {'ip_map': mock})
        self._attr_invalid(response)
...
class IpGetAvailableIpv6forVip(IpConfigTest, AttrTest):
    ...
    def test_get_available_for_vip_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
    ...
    def test_get_available_for_vip_no_permission(self):
        ...
        response = self.client_no_permission().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
    ...
    def test_get_available_for_vip_no_write_permission(self):
        ...
        response = self.client_no_write_permission().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.ID_VIP_ENVIRONMENT_VALID, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_nonexistent(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.ID_NONEXISTENT, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_letter(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.LETTER_ATTR, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_zero(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.ZERO_ATTR, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_negative(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.NEGATIVE_ATTR, {'ip_map': mock})
    ...
    def test_get_available_for_vip_env_vip_empty(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.EMPTY_ATTR, {'ip_map': mock})
        self._attr_invalid(response)
    ...
    def test_get_available_for_vip_env_vip_none(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_AVAILABLE_VIP_IPV6 % self.NONE_ATTR, {'ip_map': mock})
...
class IpGetByEquipVipTest(IpConfigTest, AttrTest):
    ...
    def test_get_ip_by_equip_env_vip_no_read_permission(self):
        response = self.get_ip_by_equip_vip(client=CLIENT_TYPES.NO_READ_PERMISSION)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_get_ip_equip_name_minsize(self):
        response = self.get_ip_by_equip_vip({'equip_name': string_generator(2)})
        self._attr_invalid(response)
        
    def test_get_ip_equip_name_maxsize(self):
        response = self.get_ip_by_equip_vip({'equip_name': string_generator(81)})
        self._attr_invalid(response)
