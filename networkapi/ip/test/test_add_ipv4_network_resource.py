...
from networkapi.equipamento.models import EquipamentoAmbiente, Equipamento
...
from networkapi.vlan.models import NetworkTypeNotFoundError, TipoRede, VlanNotFoundError
...
class NetworkAddTestCase(unittest.TestCase):
    ...
    def test_add_network_given_user_without_permission(self):
        ...
        response = self.client.post("/network/ipv4/add/", "", content_type= "text/xml")
        self.assertEquals("402 - Usuário não autorizado para executar a operação.", response.content)
        self.assertEquals(402, response.status_code)

    @mock_login
    def test_add_network_given_xml_without_vlan_xml_node(self):
        response = self.client.post("/network/ipv4/add/", '<networkapi></networkapi>', content_type= "text/xml")
    ...
    def test_add_network_given_invalid_cidr(self):
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='a', vlan=1, vip_env=1, net_type=1), content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter prefix is invalid. Value: a", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_add_network_given_invalid_cidr_greater_than_33(self):
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='33', vlan=1, vip_env=1, net_type=1), content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter prefix is invalid. Value: 33", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_add_network_given_invalid_vlan_id(self):
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan='a', vip_env=1, net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter id_vlan is invalid. Value: a", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_add_network_given_invalid_network_type(self):
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env=1, net_type='a') , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter id_tipo_rede is invalid. Value: a", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
    ...
    def test_add_network_given_invalid_network_type_not_found(self):
        ...
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env=1, net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Type of network not registered", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
    ...
    def test_add_network_given_invalid_vip_environment_id(self):
        ...
        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env='a', net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter id_ambiente_vip is invalid. Value: a", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
    ...
    def test_add_network_given_vip_environment_not_found(self):
        ...
        xml_map = self.parse_response(response)
        self.assertEquals("Environment VIP not registered", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
    ...
    def test_add_network_given_vlan_not_found(self):
        ...
        xml_map = self.parse_response(response)
        self.assertEquals("VLAN not registered", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
    ...
    def test_add_network_given_invalid_environment_config(self):
        self.mock_get_network_type_by_pk(TipoRede(id = 1))
        self.mock_get_vip_environment_by_pk(EnvironmentVip(id = 1))
        add_network_mock = self.mock_add_network_ipv4(ConfigEnvironmentInvalidError(''))

        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env=1, net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Invalid Environment Configuration or not registered", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
        self.assertTrue(add_network_mock.called)
    ...
    def test_add_network_given_no_more_available_ip_addresses(self):
        self.mock_get_network_type_by_pk(TipoRede(id = 1))
        self.mock_get_vip_environment_by_pk(EnvironmentVip(id = 1))
        add_network_mock = self.mock_add_network_ipv4(NetworkIPv4AddressNotAvailableError(''))

        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env=1, net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Unavailable address to create a NetworkIPv4", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
        self.assertTrue(add_network_mock.called)
    ...
    def test_add_network_given_no_first_ip_available(self):
        ...
        self.mock_get_equipamento_ambiente(EquipamentoAmbiente(equipamento=Equipamento(id = 1)))
        self.mock_get_first_available_ip(IpNotAvailableError('Endereço IP não dispoínvel'))

        response = self.client.post("/network/ipv4/add/", XML.format(cidr='24', vlan=1, vip_env=1, net_type=1) , content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Causa: Endereço IP não dispoínvel, Mensagem: None", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)
        self.assertTrue(add_network_mock.called)
    ...
    def mock_user_has_permission(self, has_permission):
        permission_decorator = patch('networkapi.ip.resource.NetworkIPv4AddResource.has_perm').start()
    ...
    def mock_get_vip_environment_by_pk(self, response):
        mock = patch('networkapi.ambiente.models.EnvironmentVip.get_by_pk').start()
        if issubclass(response.__class__, Exception):
            mock.side_effect = response
        else:
            mock.return_value = response
    ...
    def mock_add_network_ipv4(self, response):
        mock = patch('networkapi.ip.models.NetworkIPv4.add_network_ipv4').start()
        if issubclass(response.__class__, Exception):
            mock.side_effect = response
        else:
            mock.return_value = response
    ...
    def mock_get_equipamento_ambiente(self, response):
        ...
        mock = patch('networkapi.equipamento.models.EquipamentoAmbiente.get_routers_by_environment').start()
        mock.return_value = [response]
        return mock
