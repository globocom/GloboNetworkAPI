...
class CreateVipTestCase(unittest.TestCase):
    ...
    def test_check_available_ip_given_user_withou_permission(self):
        ...
        response = self.client.post("/ip/availableip4/vip/1/", XML, content_type= "text/xml")
    ...
    def test_check_available_ip_given_invalid_vip_environment_id(self):
        ...
        response = self.client.post("/ip/availableip4/vip/1/", XML % 'a', content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Parameter id_evip is invalid. Value: a", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_check_available_ip_given_vip_environment_without_networks(self):
        self.mock_user_has_permission(True)
        self.mock_get_vip_environment_by_pk(self.mock_vip_environment(1, 'production env', networks = []))

        response = self.client.post("/ip/availableip4/vip/1/", XML % 1, content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Não há rede ipv4 no ambiente vip fornecido.", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_check_available_ip_given_no_ip_available_on_network(self):
        self.mock_user_has_permission(True)
        self.mock_get_vip_environment_by_pk(self.mock_vip_environment(1, 'prod', networks = [NetworkIPv4()]))
        self.mock_network_get_available_ip(IpNotAvailableError(None, ""))

        response = self.client.post("/ip/availableip4/vip/1/", XML % 1, content_type= "text/xml")

        xml_map = self.parse_response(response)
        self.assertEquals("Não há ipv4 disponivel para as redes associdas com o Ambiente Vip: prod - prod - prod", xml_map.get('networkapi').get('erro').get('descricao'))
        self.assertEquals(500, response.status_code)

    @mock_login
    def test_check_available_ip_given_no_load_balancer_equipment_available_on_network(self):
        ip_equipamento = IpEquipamento(equipamento = Equipamento(tipo_equipamento = TipoEquipamento(id = 2, tipo_equipamento = 'router')))
        network = self.mock_network(ip_equipamento)

        self.mock_user_has_permission(True)
        self.mock_get_vip_environment_by_pk(self.mock_vip_environment(1, 'production env', networks = [network]))
        self.mock_network_get_available_ip(IPv4Address('192.168.1.1'))
        self.mock_get_tipo_balanceador(TipoEquipamento(id = 1, tipo_equipamento = 'balanceador'))
    ...
    def test_check_available_ip(self):
        ip_equipamento = IpEquipamento(equipamento = Equipamento(tipo_equipamento = TipoEquipamento(id = 1, tipo_equipamento = 'balanceador')))
        network = self.mock_network(ip_equipamento)
        ...
        print(response)
        xml_map = self.parse_response(response)
        self.assertEquals("192", xml_map.get('networkapi').get('ip').get('oct1'))
        self.assertEquals("168", xml_map.get('networkapi').get('ip').get('oct2'))
    ...
    def mock_network(self, equip_environment):
        ...
        network.vlan.ambiente.equipamentoambiente_set.all = lambda : [equip_environment]
        return network

    #MOCKS
    def mock_vip_environment(self, id, desc, networks):
        mock = Mock(id = id, description = desc, finalidade_txt= desc, ambiente_p44_txt= desc, cliente_txt= desc)
        mock.networkipv4_set = Mock(all = lambda : networks)
        return mock

    def mock_user_has_permission(self, has_permission):
        permission_decorator = patch('networkapi.ip.resource.Ipv4GetAvailableForVipResource.has_perm').start()
    ...
    def mock_get_tipo_balanceador(self, response):
        mock = patch("networkapi.equipamento.models.TipoEquipamento.get_tipo_balanceador").start()
        mock.return_value = response
