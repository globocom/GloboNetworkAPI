    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(ip_id):
                self.log.error(u'Parameter ip_id is invalid. Value: %s.', ip_id)
            ...
            with distributedlock(LOCK_NETWORK_IPV6 % network_ipv6_id):
                ...
                try:
                    ...
                    try:
                        ...
                        raise IpEquipmentAlreadyAssociation(None,u'Ipv6 %s:%s:%s:%s:%s:%s:%s:%s already has association with Equipament %s.' % (ip.block1, ip.block2, ip.block3, ip.block4, ip.block5, ip.block6, ip.block7, ip.block8, equip_id))
                    ...
                    ip_equips = Ipv6Equipament.objects.filter(equipamento=equip_id)
                    ...
                    self.log.error(e)
                    raise IpNotAvailableError(None, u'Ipv6 %s:%s:%s:%s:%s:%s:%s:%s is invalid' % (ip.block1, ip.block2, ip.block3, ip.block4, ip.block5, ip.block6, ip.block7, ip.block8))
                except IpNotAvailableError, e:
                    raise IpNotAvailableError(None, u'Ipv6 %s:%s:%s:%s:%s:%s:%s:%s not available for network %s.' % (ip.block1, ip.block2, ip.block3, ip.block4, ip.block5, ip.block6, ip.block7, ip.block8, net.id))
                except IpError, e:
