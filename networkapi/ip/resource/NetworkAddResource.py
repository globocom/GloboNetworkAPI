    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if version == IP_VERSION.IPv4[0]:
                ...
                nets = NetworkIPv4.objects.select_related().filter(vlan__ambiente__id=vlan.ambiente.id)
                ...
                if env_vip is not None:
                    ...
                    nets = NetworkIPv4.objects.select_related().filter(ambient_vip__id=env_vip.id)
                ...
                nets = NetworkIPv4.objects.filter(oct1=expl[0], oct2=expl[1], oct3=expl[2], oct4=expl[3], block=expl[4])
                env_ids = list()
                for net_ip in nets:
                ...
                if len(env_ids) > 0:
                    ...
                    # Verify equipments related with all other environments that contains networks with same ip range
                        ...
                        for env_equip in env_equips:
                            if env_equip.equipamento.id in equip_list:
                                ...
                                if other_env_equips[0].ambiente.filter is None or vlan.ambiente.filter is None:
                                    raise NetworkIPRangeEnvError(None, u'Um dos equipamentos associados com o ambiente desta rede também está associado com outro ambiente que tem uma rede com essa mesma faixa, adicione filtros nos ambientes se necessário.')
                                else:
                ...
                network_ip.mask_oct1, network_ip.mask_oct2, network_ip.mask_oct3, network_ip.mask_oct4 = str(net.netmask).split('.')
                ...
                if env_vip is not None:
                    ...
                    # Find all networks related to environment vip  
                    nets = NetworkIPv6.objects.select_related().filter(ambient_vip__id=env_vip.id)
                ...
                # # Filter case 1 - Adding new network with same ip range to another network on other environment ##
                # Get environments with networks with the same ip range
                nets = NetworkIPv6.objects.filter(block1=expl[0], block2=expl[1], block3=expl[2], block4=expl[3], block5=expl[4], block6=expl[5], block7=expl[6], block8=expl[7], block=expl[8])
                env_ids = list()
                for net_ip in nets:
                    env_ids.append(net_ip.vlan.ambiente.id)
                ...
                # Set block by network generated
                network_ip.block1, network_ip.block2, network_ip.block3, network_ip.block4, network_ip.block5, network_ip.block6, network_ip.block7, network_ip.block8 = str(net.network.exploded).split(':')
                # Set block by network generated
                network_ip.block = net.prefixlen
                # Set mask by network generated
                network_ip.mask1, network_ip.mask2, network_ip.mask3, network_ip.mask4, network_ip.mask5, network_ip.mask6, network_ip.mask7, network_ip.mask8 = str(net.netmask.exploded).split(':')
             
            
            
            
            
            # Get all vlans environments from equipments of the current environment
            ...
            # Check if have duplicated vlan's with same net range in an environment with shared equipment
            ...
            try:
                ...
                list_equip_routers_ambient = EquipamentoAmbiente.objects.filter(ambiente=network_ip.vlan.ambiente.id, is_router=True)
            ...
            network_map['id_ambiente_vip'] = env_vip.id if env_vip != None else ''
