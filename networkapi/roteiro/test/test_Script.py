...
class ScriptAttrTypeTest(ScriptConfigTest, AttrTest):
    ...
    def test_alter_unique(self):
        ...
        mock[self.KEY_ATTR] = 'Script'
        mock['id_script_type'] = 1
        self.process_alter_attr_invalid(self.ID_ALTER_VALID, mock, CodeError.SCRIPT_DUPLICATE)
...
class ScriptConsultationScriptTypeTest(ScriptConfigTest, AttrTest):
    ...
    def test_get_by_script_type(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.ID_VALID )
        valid_response(response)
    ...
    def test_get_by_script_type_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_SCRIPT_TYPE % self.ID_VALID )
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_script_type_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_SCRIPT_TYPE % self.ID_VALID )
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_script_type_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.ID_NONEXISTENT)
        self._not_found(response)
    
    def test_get_by_script_type_negative(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_script_type_letter(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.LETTER_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_script_type_zero(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_script_type_empty(self):
        response = self.client_autenticado().get(self.URL_GET_SCRIPT_TYPE % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class ScriptConsultationEquipmentTest(ScriptConfigTest, AttrTest):
    ...
    def test_get_by_equipment(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.ID_VALID )
        valid_response(response)
        content = valid_content(response, "script", True)
    ...
    def test_get_by_equipment_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_EQUIPMENT % self.ID_VALID )
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_equipment_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_EQUIPMENT % self.ID_VALID )
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_equipment_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.ID_NONEXISTENT)
        self._not_found(response)
    
    def test_get_by_equipment_negative(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_equipment_letter(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.LETTER_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_equipment_zero(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.ZERO_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_equipment_empty(self):
        response = self.client_autenticado().get(self.URL_GET_EQUIPMENT % self.EMPTY_ATTR)
        self._attr_invalid(response)