...
def prepares_logs(log):
    ...
    log_dict['hora_evento']  = log_dict['hora_evento'].strftime('%d/%m/%Y %H:%M') 
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not has_perm(user, AdminPermission.USER_ADMINISTRATION, AdminPermission.READ_OPERATION):
                self.log.error('User does not have permission to perform this operation.')
            ...
            if data_inicial is not None:   
                if data_final is not None:
                    ...
                    eventlog = eventlog.filter(hora_evento__gte=data_inicial, hora_evento__lte = data_final)
            else:
                #Concatenate strings
                ...
                eventlog = eventlog.extra(where=["time(hora_evento) >= '"+ hora_inicial +"' and time(hora_evento) <= '"+ hora_final +"'"])
                 
            if parametro is not None:
                eventlog = eventlog.filter(parametro_atual__contains = parametro) | eventlog.filter(parametro_anterior__contains = parametro)
                pass
            
            eventlog, total = build_query_to_datatable(eventlog, asorting_cols, custom_search, searchable_columns, start_record, end_record)
