...
class OpenDayLightACLTestCase(NetworkApiTestCase):
    ...
    def test_acl_flow_builder_missing_destination_data(self):
        ...
        assert_in("flow-name", acl.flows[0]["flow"])
        assert_equal(acl.flows[0]["flow"]["flow-name"],
    ...
    def test_acl_should_have_tcp_destination_port(self):
        ...
        acl = AclFlowBuilder(data)
        acl.build()
        assert_equal(acl.flows[0]["flow"]["match"]["tcp-destination-port"],
    ...
    def test_acl_should_have_tcp_source_port(self):
        ...
        acl = AclFlowBuilder(data)
        acl.build()
        assert_equal(acl.flows[0]["flow"]["match"]["tcp-source-port"],
    ...
    def test_acl_should_have_tcp_as_ip_protocol(self):
        ...
        acl.build()
        assert_equal(6,
                     acl.flows[0]["flow"]["match"]["ip-match"]["ip-protocol"])
    ...
    def test_acl_should_have_udp_as_ip_protocol(self):
        ...
        acl.build()
        assert_equal(
            17, acl.flows[0]["flow"]["match"]["ip-match"]["ip-protocol"])
    ...
    def test_acl_should_have_udp_source_port(self):
        ...
        acl = AclFlowBuilder(data)
        acl.build()
        assert_equal(acl.flows[0]["flow"]["match"]["udp-source-port"],
                     data["rules"][0]["l4-options"]["src-port-start"])
    ...
    def test_acl_should_have_udp_destination_port(self):
        ...
        acl = AclFlowBuilder(data)
        acl.build()
        assert_equal(acl.flows[0]["flow"]["match"]["udp-destination-port"],
                     data["rules"][0]["l4-options"]["dest-port-start"])
    ...
    def test_acl_should_have_icmp_as_ip_protocol(self):
        ...
        acl.build()
        assert_equal(
            1, acl.flows[0]["flow"]["match"]["ip-match"]["ip-protocol"])
    ...
    def test_acl_should_have_icmp_code_and_type_at_flow(self):
        ...
        assert_equal(
            ...
            acl.flows[0]["flow"]["match"]["icmpv4-match"]["icmpv4-code"])
        assert_equal(
            data["rules"][0]["icmp-options"]["icmp-type"],
            acl.flows[0]["flow"]["match"]["icmpv4-match"]["icmpv4-type"])
    ...
    def test_acl_should_create_internet_protocol_flow(self):
        ...
        acl = AclFlowBuilder(data)
        acl.build()
        assert_equal(acl.flows[0]['flow']['match']['ipv4-source'],
            data['rules'][0]['source'])
        assert_equal(acl.flows[0]['flow']['match']['ipv4-destination'],
    ...
    def test_acl_should_have_action_local(self):
        ...
        assert_in("instructions", acl.flows[0]["flow"])
        instruction = acl.flows[0]["flow"]["instructions"]["instruction"][0]
    ...
    def test_acl_should_not_have_actions(self):
        ...
        assert_not_in("instructions", acl.flows[0]["flow"])
