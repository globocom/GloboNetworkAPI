...
class InterfaceGetByEquipConsultationTest(InterfaceConfigTest, AttrTest):
    ...
    def test_get_by_equip_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP % self.EQUIP_VALID)
        valid_response(response)
    ...
    def test_get_by_equip_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_EQUIP % self.EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_equip_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_EQUIP % self.EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_equip_old_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.EQUIP_VALID)
        valid_response(response)
    ...
    def test_get_by_equip_old_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_EQUIP_OLD % self.EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
        
    def test_get_by_equip_old_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_EQUIP_OLD % self.EQUIP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_get_by_equip_old_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.ID_NONEXISTENT)
        self._not_found(response)
        
    def test_get_by_equip_old_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_equip_old_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.LETTER_ATTR)
        self._attr_invalid(response)
        
    def test_get_by_equip_old_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.ZERO_ATTR)
        self._attr_invalid(response)
    
    def test_get_by_equip_old_empty(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIP_OLD % self.EMPTY_ATTR)
        self._attr_invalid(response)
...
class InterfaceTest(InterfaceConfigTest):
    ...
    def test_save_valid(self):
        ...
        response = self.client_autenticado().get(self.URL_GET_BY_ID % interface['id'])
        valid_response(response)
    ...
    def test_alter_valid(self):
        ...
        response = self.client_autenticado().get(self.URL_GET_BY_ID % self.ID_ALTER_VALID)
        valid_response(response)
        interface_from_db = valid_content(response, self.XML_KEY)
...
class InterfaceDisconnectTest(InterfaceConfigTest, AttrTest):
    ...
    def test_disconnect_valid(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, "0"))
        valid_response(response)
        
    def test_disconnect_not_connected(self):
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_REMOVE_VALID, "0"))
        valid_response(response, status_code = httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, codigo=self.CODE_ERROR_IF_NOT_CONNECTED)
    
    def test_disconnect_no_permission(self):
        mock = self.mock_for_disconnect()
        self.alter(self.ID_FOR_CONNECTION, { self.XML_KEY : mock })
        
        response = self.client_no_permission().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, "0"))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_disconnect_no__write_permission(self):
        ...
        response = self.client_no_write_permission().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, "0"))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    ...
    def test_disconnect_id_nonexistent(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_NONEXISTENT, "0"))
    ...
    def test_disconnect_id_negative(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.NEGATIVE_ATTR, "0"))
        self._attr_invalid(response)
    ...
    def test_disconnect_id_letter(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.LETTER_ATTR, "0"))
        self._attr_invalid(response)
    ...
    def test_disconnect_id_zero(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ZERO_ATTR, "0"))
        self._attr_invalid(response)
    ...
    def test_disconnect_id_empty(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.EMPTY_ATTR, "0"))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_letter(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, self.LETTER_ATTR))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_not_zero_one(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, "3"))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_empty(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_none(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, self.NONE_ATTR))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_false(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, 'False'))
        self._attr_invalid(response)
    ...
    def test_disconnect_back_front_true(self):
        ...
        response = self.client_autenticado().delete(self.URL_REMOVE_CONNECTION % (self.ID_DISCONNECT_VALID, 'True'))
        self._attr_invalid(response)

class InterfaceListConnectionsTest(InterfaceConfigTest, AttrTest):
    
    def test_list_conns_valid(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response)
        valid_content(response, 'interfaces')
    
    def test_list_conns_no_permission(self):
        response = self.client_no_permission().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_list_conns_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_list_conns_interface_inexistent(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (string_generator(15), self.EQUIP_FOR_LIST))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.INTERFACE_NOT_FOUND)
    
    def test_list_conns_interface_maxsize(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (string_generator(21), self.EQUIP_FOR_LIST))
        self._attr_invalid(response)
    
    def test_list_conns_interface_minsize(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (string_generator(2), self.EQUIP_FOR_LIST))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.INTERFACE_NOT_FOUND)
    
    def test_list_conns_interface_empty(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.EMPTY_ATTR, self.EQUIP_FOR_LIST))
        self._attr_invalid(response)
    
    def test_list_conns_equip_inexistent(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.ID_NONEXISTENT))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.EQUIPMENT_NOT_FOUND)
    
    def test_list_conns_equip_letter(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.LETTER_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_equip_zero(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.ZERO_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_equip_empty(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.EMPTY_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_equip_none(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.NONE_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_equip_negative(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS % (self.INTERFACE_FOR_LIST, self.NEGATIVE_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_old_valid(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response)
        valid_content(response, self.XML_KEY)
    
    def test_list_conns_old_no_permission(self):
        response = self.client_no_permission().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_list_conns_old_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.EQUIP_FOR_LIST))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_list_conns_old_interface_inexistent(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (string_generator(15), self.EQUIP_FOR_LIST))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.INTERFACE_NOT_FOUND)
    
    def test_list_conns_old_interface_maxsize(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (string_generator(21), self.EQUIP_FOR_LIST))
        self._attr_invalid(response)
    
    def test_list_conns_old_interface_minsize(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (string_generator(2), self.EQUIP_FOR_LIST))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.INTERFACE_NOT_FOUND)
    
    def test_list_conns_old_interface_empty(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.EMPTY_ATTR, self.EQUIP_FOR_LIST))
        self._attr_invalid(response)
        
    def test_list_conns_old_equip_inexistent(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.ID_NONEXISTENT))
        valid_response(response, httplib.INTERNAL_SERVER_ERROR)
        assert_response_error(response, CodeError.EQUIPMENT_NOT_FOUND)
    
    def test_list_conns_old_equip_letter(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.LETTER_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_old_equip_zero(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.ZERO_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_old_equip_empty(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.EMPTY_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_old_equip_none(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.NONE_ATTR))
        self._attr_invalid(response)
    
    def test_list_conns_old_equip_negative(self):
        response = self.client_autenticado().get(self.URL_LIST_CONNECTIONS_OLD % (self.INTERFACE_FOR_LIST, self.NEGATIVE_ATTR))
        self._attr_invalid(response)
