...
class Interface(BaseModel):
    ...
    ligacao_front = models.ForeignKey(
        'self',
        null=True,
        db_column='id_ligacao_front',
        blank=True,
        related_name='interfaces_front')
    ligacao_back = models.ForeignKey(
        'self',
        null=True,
        db_column='id_ligacao_back',
        blank=True,
        related_name='interfaces_back')
    ...
    def get_by_interface_equipment(cls, interface, equipment_id):
        try:
            return Interface.objects.get(
                interface__iexact=interface,
                equipamento__id=equipment_id)
        except ObjectDoesNotExist as e:
            raise InterfaceNotFoundError(
                e, u'Interface com nome %s e equipamento %s não cadastrada.' %
                (interface, equipment_id))
    ...
    def get_by_pk(cls, id):
        try:
            ...
            raise InterfaceNotFoundError(
                e,
                u'Can not find a Interface with id = %s.' %
                id)
        except Exception as e:
            cls.log.error(u'Falha ao pesquisar a interface.')
            raise InterfaceError(e, u'Falha ao pesquisar a interface.')
    ...
    def search_interfaces(self, from_interface):
        ...
        try:
            ...
            while (interface is not None):
                ...
                if (interface.ligacao_back is not None) and (
                        from_interface.id != interface.ligacao_back_id):
        ...
        if self.ligacao_back is not None:
            try:
                ...
                raise BackLinkNotFoundError(
                    e,
                    u'Backend interface does not exist')

        try:
            # Check if interface name already exists for this equipment
            if Interface.objects.filter(
                    equipamento=self.equipamento,
                    interface__iexact=self.interface).count() > 0:
                raise InterfaceForEquipmentDuplicatedError(
                    None,
                    u'An interface with the same name on the same equipment already exists')
            ...
            raise InterfaceError(
                e,
                u'Failed to add interface for the equipment.')
    ...
    def update(cls, authenticated_user, id_interface, **kwargs):
        ...
        try:
            ...
            try:
                nome = kwargs['interface']
                if Interface.objects.filter(
                        equipamento=interface.equipamento,
                        interface__iexact=nome).exclude(
                        id=interface.id).count() > 0:
                    raise InterfaceForEquipmentDuplicatedError(
                        None,
                        u'An interface with the same name on the same equipment already exists')
            except KeyError:
            ...
            if Interface.objects.filter(ligacao_front=interface).count() > 0 or \
               Interface.objects.filter(ligacao_back=interface).count() > 0:
                raise InterfaceUsedByOtherInterfaceError(
                    None,
                    u'Interface used by other interface.')
