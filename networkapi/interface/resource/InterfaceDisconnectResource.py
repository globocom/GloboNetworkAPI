    ...
    def handle_delete(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_zero_one_param(back_or_front):
                self.log.error(u'The back_or_front parameter is not a valid value: %s.', back_or_front)
            ...
            with distributedlock(LOCK_INTERFACE % id_interface):
                ...
                if back_or_front:
                    try:
                        interface_2 = Interface.get_by_pk(interface_1.ligacao_front_id)
                    except InterfaceNotFoundError:
                        raise InterfaceInvalidBackFrontError(None, "Interface two has no connection with front of Interface one")
                else:
                    try:
                        interface_2 = Interface.get_by_pk(interface_1.ligacao_back_id)
                    except InterfaceNotFoundError:
                        raise InterfaceInvalidBackFrontError(None, "Interface two has no connection with back of Interface one")
                ...
                if interface_2.ligacao_front_id == interface_1.id:
                    ...
                    raise InterfaceInvalidBackFrontError(None, "Interface one has no connection with front or back of Interface two")
