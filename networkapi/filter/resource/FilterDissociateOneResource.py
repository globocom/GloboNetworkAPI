    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(kwargs['id_equip_type']):
                self.log.error(u'Parameter id_equip_type is invalid. Value: %s.', kwargs['id_equip_type'])
                raise InvalidValueError(None, 'id_equip_type', kwargs['id_equip_type'])
            else:
                #Check existence
            ...
            #Delete association
            try:
                association = FilterEquipType.objects.get(filter=fil.id, equiptype=eq_tp.id)
