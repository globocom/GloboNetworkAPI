...
class Filter(BaseModel):
    ...
    name = models.CharField(max_length=100, blank=False, unique=True, db_column='name')
    description = models.CharField(max_length=200, null=True, blank=True, db_column='description')
    ...
    def get_by_pk(cls, id_):
        ...
        try:
            ...
            raise FilterNotFoundError(e, u'There is no Filter with pk = %s.' % id_)      
        except Exception, e:
    ...
    def validate_filter(self, filter_map):
        ...
        if len(Filter.objects.filter(name=name).exclude(id=self.id)) > 0:
            raise FilterDuplicateError(None, u'Já existe um filtro com o nome %s no banco de dados.' % name)
...
def check_filter_use(new_filter_id, env):
    ...
    if old_fil is not None:
        ...
        for i in range(0, len(nets_ipv4)):
            ...
            if verify_subnet_and_equip(nets_ipv4_aux, network_ip_verify, 'v4', net, nets_ipv4[i].get('vlan_env')):
                ...
                if env.id == env_aux_id:
                    raise CannotDissociateFilterError(old_fil.name, u'Filter %s cannot be dissociated, its in use.' % old_fil.name)
        ...
        old_tp_equips = [fet.equiptype.id for fet in old_fil.filterequiptype_set.all()]
        if new_fil is not None:
            new_tp_equips = [fet.equiptype.id for fet in new_fil.filterequiptype_set.all()]
        else:
            new_tp_equips = []
        ...
        if len(diff_tp_equips) > 0:
            ...
            nets_same_range = NetworkIPv4.objects.values('oct1', 'oct2', 'oct3', 'oct4', 'block').annotate(count=Count('id')).filter(count__gt=1)
            
            if len(nets_same_range) > 0:
                for net_gp in nets_same_range:
                    nets_current_range = NetworkIPv4.objects.filter(oct1=net_gp['oct1'], oct2=net_gp['oct2'], oct3=net_gp['oct3'], oct4=net_gp['oct4'], block=net_gp['block'])
                    envs_of_nets = [net.vlan.ambiente.id for net in nets_current_range]
                    if env.id in envs_of_nets:
                        
                        eqas = EquipamentoAmbiente.objects.filter(equipamento__tipo_equipamento__in=diff_tp_equips, ambiente=env.id)
                        equips_in_env = [eqa.equipamento.id for eqa in eqas]
                                    
                        #Get other environments with these equips
                        other_envs = [eqa.ambiente.id for eqa in EquipamentoAmbiente.objects.filter(equipamento__in=equips_in_env, ambiente__in=envs_of_nets).exclude(ambiente=env.id)]
                        
                        if len(other_envs) > 0:
                            raise CannotDissociateFilterError(old_fil.name, u'Filter %s cannot be dissociated, its in use.' % old_fil.name)
                        
            #Check for networks v6 with same ip range
            nets_same_range_v6 = NetworkIPv6.objects.values('block1', 'block2', 'block3', 'block4', 'block5', 'block6', 'block7', 'block8', 'block').annotate(count=Count('id')).filter(count__gt=1)
            
            if len(nets_same_range_v6) > 0:
                for net_gp in nets_same_range_v6:
                    nets_current_range = NetworkIPv6.objects.filter(block1=net_gp['block1'], block2=net_gp['block2'], block3=net_gp['block3'], block4=net_gp['block4'], block5=net_gp['block5'], block6=net_gp['block6'], block7=net_gp['block7'], block8=net_gp['block8'], block=net_gp['block'])
                    envs_of_nets = [net.vlan.ambiente.id for net in nets_current_range]
                    if env.id in envs_of_nets:
            ...
            vlans_same_number = Vlan.objects.values('num_vlan').annotate(count=Count('id')).filter(count__gt=1)
            
            if len(vlans_same_number) > 0:
                for vlan_gp in vlans_same_number:
                    vlans_current_number = Vlan.objects.filter(num_vlan=vlan_gp['num_vlan'])
                    envs_of_vlans = [vlan.ambiente.id for vlan in vlans_current_number]
                    
                    if env.id in envs_of_vlans:
                        
                        eqas = EquipamentoAmbiente.objects.filter(ambiente=env.id)
                        equips_in_env = [eqa.equipamento.id for eqa in eqas]
                                    
                        #Get other environments with these equips
                        other_envs = [eqa.ambiente.id for eqa in EquipamentoAmbiente.objects.filter(equipamento__in=equips_in_env, ambiente__in=envs_of_vlans).exclude(ambiente=env.id)]
    ...
    for net_env in vlan_net:
        ...
        if ip_net in network_ip:
            ...
            if len(set(equip_list) & set(equip_list_aux)) > 0:
                # This vlan must be in vlans founded, dont need to continue checking
                return True
        # If some network, inside this vlan, is supernet of network search param
