...
class FilterAssociateTest(FilterConfigTest, AttrTest):
    ...
    def test_associate_valid(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response)
        valid_content(response, "equiptype_filter_xref")
        
        #Remove filter association so it does not require cleaning db manually
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response)
    
    def test_associate_no_permission(self):
        response = self.client_no_permission().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_associate_no__write_permission(self):
        response = self.client_no_write_permission().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_associate_filter_nonexistent(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_NONEXISTENT, self.ID_TYPE_EQUIPMENT ))
        self._not_found(response, CodeError.FILTER_NOT_FOUND)
    
    def test_associate_filter_negative(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.NEGATIVE_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)

    def test_associate_filter_letter(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.LETTER_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)

    def test_associate_filter_zero(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ZERO_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)

    def test_associate_filter_empty(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.EMPTY_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_associate_filter_none(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.NONE_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_associate_type_equipment_nonexistent(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.ID_NONEXISTENT ))
        self._not_found(response, CodeError.EQUIPMENT_TYPE_NOT_FOUND)

    def test_associate_type_equipment_negative(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.NEGATIVE_ATTR ))
        self._attr_invalid(response)

    def test_associate_type_equipment_letter(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.LETTER_ATTR ))
        self._attr_invalid(response)

    def test_associate_type_equipment_zero(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.ZERO_ATTR ))
        self._attr_invalid(response)

    def test_associate_type_equipment_empty(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.EMPTY_ATTR ))
        self._attr_invalid(response)
    
    def test_associate_type_equipment_none(self):
        response = self.client_autenticado().put(self.URL_ASSOCIATE % ( self.ID_VALID, self.NONE_ATTR ))
        self._attr_invalid(response)

class FilterDissociateTest(FilterConfigTest, AttrTest):

    def test_dissociate_valid(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response)
    
    def test_dissociate_cant_because_in_use_case_1(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_IN_USE_CASE1, self.ID_TYPE_EQUIPMENT_IN_USE_CASE1))
        self._attr_invalid(response, CodeError.FILTER_EQUIPTYPE_CANT_DISSOCIATE)
    
    def test_dissociate_cant_because_in_use_case_2(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_IN_USE_CASE2, self.ID_TYPE_EQUIPMENT_IN_USE_CASE2))
        self._attr_invalid(response, CodeError.FILTER_EQUIPTYPE_CANT_DISSOCIATE)
    
    def test_dissociate_cant_because_in_use_case_3(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_IN_USE_CASE3, self.ID_TYPE_EQUIPMENT_IN_USE_CASE3))
        self._attr_invalid(response, CodeError.FILTER_EQUIPTYPE_CANT_DISSOCIATE)
    
    def test_dissociate_valid_associated(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_ENV_ASSOCIATED, self.ID_TYPE_EQUIPMENT_ENV_ASSOCIATED))
        valid_response(response)
    
    def test_dissociate_no_permission(self):
        response = self.client_no_permission().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_dissociate_no__write_permission(self):
        response = self.client_no_write_permission().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.ID_TYPE_EQUIPMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_dissociate_filter_nonexistent(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_NONEXISTENT, self.ID_TYPE_EQUIPMENT ))
        self._not_found(response, CodeError.FILTER_NOT_FOUND)
    
    def test_dissociate_filter_negative(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.NEGATIVE_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_dissociate_filter_letter(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.LETTER_ATTR , self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_dissociate_filter_zero(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ZERO_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_dissociate_filter_empty(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.EMPTY_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_dissociate_filter_none(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.NONE_ATTR, self.ID_TYPE_EQUIPMENT ))
        self._attr_invalid(response)
    
    def test_dissociate_type_equipment_nonexistent(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.ID_NONEXISTENT ))
        self._not_found(response, CodeError.EQUIPMENT_TYPE_NOT_FOUND)
    
    def test_dissociate_type_equipment_negative(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.NEGATIVE_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_type_equipment_letter(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.LETTER_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_type_equipment_zero(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.ZERO_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_type_equipment_empty(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.EMPTY_ATTR ))
        self._attr_invalid(response)
    
    def test_dissociate_type_equipment_none(self):
        response = self.client_autenticado().put(self.URL_DISSOCIATE % ( self.ID_DISSOCIATE_VALID, self.NONE_ATTR ))
        self._attr_invalid(response)
