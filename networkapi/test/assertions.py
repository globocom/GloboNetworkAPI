...
def assert_response_error(response, codigo, descricao=None):
    ...
    try:
        ...
        assert codigo_resposta in codigos, u"Código de resposta inválido: %d (descricao: %s). Esperados: %s" % (codigo_resposta, descricao_resposta, repr(codigos))
        assert descricao_resposta != None
        assert len(descricao_resposta) > 0
        if descricao:
            assert descricao_resposta == descricao
    except:
        # Se houver algum erro na formação (parsing) da resposta, imprimo qual ela era para facilitar a investigação
...
def assert_response_success(response, status_code=200, codigo=0, stdout=None, stderr=None):
    ...
    try:
        ...
        assert codigo_resposta == codigo, u"Código de resposta inválido: %d. Esperado: %d" % (codigo_resposta, codigo)

        if stdout:
            assert networkapi_response['sucesso']['descricao']['stdout'] == stdout
            
        if stderr:
            assert networkapi_response['sucesso']['descricao']['stderr'] == stderr
    except:
        # Se houver algum erro na formação (parsing) da resposta, imprimo qual ela era para facilitar a investigação
        log.error("Erro fazendo parsing da resposta:\n%s\n", (response or response.content))
        raise
