...
class HealthcheckExpect(BaseModel):
    ...
    def search(self, environment_id=None):
        try:
            ...
            if (environment_id is not None):
                healthcheckexpects = healthcheckexpects.filter(ambiente__id=environment_id)
                
            return healthcheckexpects
        except Exception, e:
            self.log.error(u'Falha ao pesquisar os healthcheck_expects.')
            raise HealthcheckExpectError(e, u'Falha ao pesquisar os healthcheck_expects.')
        ...
        try:
            ...
            if (environment_id is not None):
                ...
                for hce in hces:
                    ...
                    #If any RequsicaoVips associated, dissociate healthcheck expect from Ambiente
                    if vip_criado:
                        hce.ambiente = None
            ...
            raise HealthcheckExpectError(e, u'Falha ao desassociar os healthcheck_expects.')
    ...
    def get_by_pk(self, id):
        try:
            ...
            raise HealthcheckExpectNotFoundError(e, u'Não existe um HealthcheckExpect com a pk = %s.' % id)
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o healthcheck_expect.')
            raise HealthcheckExpectError(e, u'Falha ao pesquisar o healthcheck_expect.')
    ...
    def insert(self,authenticated_user,match_list,expect_string,environment):
        try:
            ...
            try:
                HealthcheckExpect.objects.get(ambiente=environment,match_list=match_list,expect_string=expect_string)
                raise HealthcheckEqualError(None,'healthcheckexpect com os dados : match_list : %s, expect_string: %s, ambiente_id: % s, já cadastrado' % (match_list, expect_string, str(environment.id)) )
            except ObjectDoesNotExist, e:
                pass
            ...
            raise HealthcheckExpectError(e, u'Falha ao inserir healthcheck_expect.')
    ...
    def get_expect_strings(self):
        try:
            ...
            raise HealthcheckExpectNotFoundError(e, u'Erro ao pequisar Healthcheks_expects')
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o healthcheck_expect.')
            raise HealthcheckExpectError(e, u'Falha ao pesquisar o healthcheck_expect.')
