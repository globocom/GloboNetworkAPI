    ...
    def search(self, environment_id=None):
        try:
            ...
            raise HealthcheckExpectError(
                e,
                u'Falha ao pesquisar os healthcheck_expects.')

    @classmethod
    def dissociate_environment_and_delete(
            self,
            authenticated_user,
            environment_id=None):
        ...
        try:
            ...
            raise HealthcheckExpectError(
                e,
                u'Falha ao desassociar os healthcheck_expects.')
    ...
    def get_by_pk(self, id):
        try:
            ...
            raise HealthcheckExpectNotFoundError(
                e,
                u'Não existe um HealthcheckExpect com a pk = %s.' %
                id)
        except Exception as e:
            self.log.error(u'Falha ao pesquisar o healthcheck_expect.')
            raise HealthcheckExpectError(
                e,
                u'Falha ao pesquisar o healthcheck_expect.')
    ...
    def insert(
        ...
        try:
            ...
            try:
                ...
                raise HealthcheckEqualError(
                    None, 'healthcheckexpect com os dados : match_list : %s, expect_string: %s, ambiente_id: % s, já cadastrado' %
                    (match_list, expect_string, str(
                        environment.id)))
            except ObjectDoesNotExist as e:
                pass
            ...
            raise HealthcheckExpectError(
                e,
                u'Falha ao inserir healthcheck_expect.')
