...
class UGrupo(BaseModel):
    ...
    def get_by_pk(cls, id):
        ...
        try:
            ...
            raise UGrupoNotFoundError(e, u'Dont there is a Group Use by pk = %s.' % id)
        except Exception, e:
...
class Permission(BaseModel):
    ...
    def get_by_pk(cls, pk):
        ...
        try:
            ...
            raise PermissionNotFoundError(e, u'There is Permission with pk = %s.' % pk)
        except Exception, e:
...
class PermissaoAdministrativa(BaseModel):
    id = models.AutoField(primary_key=True, db_column='id_permissoes_administrativas')
    ...
    def create(self, authenticated_user):
        ...
        try:
            PermissaoAdministrativa.get_permission_by_function_ugroup(self.funcao, self.ugrupo.id)
            raise PermissaoAdministrativaDuplicatedError(None, u'Permissão administrativa com grupo de usuário %s e função %s já cadastrada' % (self.ugrupo.id, self.funcao))
        ...
        try:
            ...
            raise GrupoError(e, u'Falha ao inserir uma Permissao Administrativa.')
    ...
    def get_by_pk(cls, pk):
        ...
        try:
            ...
            raise PermissaoAdministrativaNotFoundError(e, u'Não existe uma Permissao Administrativa com a pk = %s.' % pk)
        except Exception, e:
            cls.log.error(u'Falha ao pesquisar uma Permissao Administrativa.')
            raise GrupoError(e, u'Falha ao pesquisar uma Permissao Administrativa.')    
    #ok
    @classmethod
    def get_permission_by_function_ugroup(cls, function, ugroup_id):
        try:
            ...
            raise PermissaoAdministrativaNotFoundError(o, 'Grupo do Usuário %s não tem permissão na função %s.' % (ugroup_id, function))
        except Exception, e:
            cls.log.error(u'Falha ao pesquisar a permissão administrativa.')
            raise GrupoError(e, u'Falha ao pesquisar a permissão administrativa.')
    ...
    def get_permission_by_permission_ugroup(cls, permission_id, ugroup_id):
        ...
        try:
            ...
            #Arrumar MSG
            raise PermissaoAdministrativaNotFoundError(o, 'Grupo do Usuário %s não tem permissão na função %s.' % (ugroup_id, permission_id))
        except Exception, e:
            cls.log.error(u'Falha ao pesquisar a permissão administrativa.')
            raise GrupoError(e, u'Falha ao pesquisar a permissão administrativa.')
    ...
    def get_permission(self, function, ugroup, operation):
        ...
        try:
            ...
            raise PermissaoAdministrativaNotFoundError(o, 'Grupo do Usuário %s não tem permissão de %s na função %s.' % (ugroup.nome, operation, function))
        except Exception, e:
            self.log.error(u'Falha ao pesquisar as permissões administrativas.')
            raise GrupoError(e, u'Falha ao pesquisar as permissões administrativas.')
...
class EGrupo(BaseModel):
    ...
    def get_by_pk(cls, pk):
        try:
            ...
            raise EGrupoNotFoundError(e, u'Não existe um egrupo com a pk = %s.' % pk)
        except Exception, e:
    ...
    def search(cls):
        try:
            ...
            raise GrupoError(e, u'Falha ao pesquisar os grupos de equipamento.')
    ...
    def create(self, authenticated_user):
        ...
        try:
            try:
                ...
                raise EGrupoNameDuplicatedError(None, u'Grupo de equipamento com o nome %s já cadastrado' % self.nome)
        ...
        try:
            ...
            try:
                ...
                if (egrupo.nome.lower() != nome.lower()):
                    ...
                    raise EGrupoNameDuplicatedError(None, u'Grupo de equipamento com o nome %s já cadastrado' % nome)
...
class DireitosGrupoEquipamento(BaseModel):
    ...
    def get_by_pk(cls, pk):
        try:
            ...
            cls.log.error(u'Falha ao pesquisar os direitos de um grupo de usuário em um grupo de equipamento.')
            raise GrupoError(e, u'Falha ao pesquisar os direitos de um grupo de usuário em um grupo de equipamento.')  
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            if DireitosGrupoEquipamento.objects.filter(egrupo__id=self.egrupo.id, ugrupo__id=self.ugrupo.id).count() > 0:
                raise DireitoGrupoEquipamentoDuplicatedError(None, u'Direito Grupo Equipamento para o grupo de usuário %d e grupo de equipamento %s já cadastrado.' % (self.ugrupo_id, self.egrupo_id))
            ...
            self.log.error(u'Falha ao inserir o direito do grupo de usuário no grupo de equipamento.')
            raise GrupoError(e, u'Falha ao inserir o direito do grupo de usuário no grupo de equipamento.')
    ...
    def update(cls, authenticated_user, pk, **kwargs):
        ...
        try:
            ...
            cls.log.error(u'Falha ao atualizar os direitos de um grupo de usuário em um grupo de equipamento.')
            raise GrupoError(e, u'Falha ao atualizar os direitos de um grupo de usuário em um grupo de equipamento.')  
    ...
    def remove(cls, authenticated_user, pk):
        ...
        try:
            ...
            cls.log.error(u'Falha ao remover os direitos de um grupo de usuário em um grupo de equipamento.')
            raise GrupoError(e, u'Falha ao remover os direitos de um grupo de usuário em um grupo de equipamento.')
    ...
    def search(cls, ugroup_id=None, equip_operation=None, egroup_id=None):
        try:
            ...
            cls.log.error(u'Falha ao pesquisar os direitos do grupo-equipamento.')
            raise GrupoError(e, u'Falha ao pesquisar os direitos do grupo-equipamento.')