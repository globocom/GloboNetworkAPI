...
class PermissionAdministrativeTest(PermissionAdministrativeConfigTest, AttrTest): 
    ...
    def test_save_duplicate(self):
        ...
        response = self.save({ self.XML_KEY : mock })
        self._attr_invalid(response, CodeError.PERMISSION_ADMINISTRATIVE_DUPLICATE)
...
class PermissionAdministrativeGetByUGroupTest(PermissionAdministrativeConfigTest, AttrTest):
    ...
    def test_get_by_ugroup_valid(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.ID_UGROUP_VALID)
        valid_response(response)
        valid_content(response)
    
    def test_get_by_ugroup_no_permission(self):
        response = self.client_no_permission().get(self.URL_LIST_BY_GROUP % self.ID_UGROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_by_ugroup_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_LIST_BY_GROUP % self.ID_UGROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_by_ugroup_nonexistent(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.ID_NONEXISTENT)
        self._not_found(response)

    def test_get_by_ugroup_negative(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.NEGATIVE_ATTR)
        self._attr_invalid(response)
    
    def test_get_by_ugroup_letter(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.LETTER_ATTR)
        self._attr_invalid(response)
    
    def test_get_by_ugroup_zero(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_get_by_ugroup_empty(self):
        response = self.client_autenticado().get(self.URL_LIST_BY_GROUP % self.EMPTY_ATTR)
        self._attr_invalid(response)
