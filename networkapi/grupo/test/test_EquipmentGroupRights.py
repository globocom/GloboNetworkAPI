...
class EquipmentGroupRightsConfigTest(BasicTestCase):
    ...
    #Constants
    fixtures = ['networkapi/equipamento/fixtures/initial_data.yaml','networkapi/grupo/fixtures/initial_data.yaml']
...
class EquipmentGroupRightsTest(EquipmentGroupRightsConfigTest):
    ...
    def test_alter_valid(self):
        ...
        valid_response(response)
        equip_group_right = DireitosGrupoEquipamento.get_by_pk(self.ID_ALTER_VALID)
...
class EquipmentGroupRightsAttrEquipmentGroupRightsTest(EquipmentGroupRightsConfigTest, AttrTest):
...
class EquipmentGroupRightsAttrLeituraTest(EquipmentGroupRightsConfigTest, AttrTest):
...
class EquipmentGroupRightsAttrEscritaTest(EquipmentGroupRightsConfigTest, AttrTest):
...
class EquipmentGroupRightsAttrAlterarConfigTest(EquipmentGroupRightsConfigTest, AttrTest):
...
class EquipmentGroupRightsAttrExclusaoTest(EquipmentGroupRightsConfigTest, AttrTest):
...
class EquipmentGroupRightsConsultationTest(EquipmentGroupRightsConfigTest, ConsultationTest):
...
class EquipmentGroupRightsConsultationEquipmentGroupRightsTest(EquipmentGroupRightsConfigTest, ConsultationTest):


    def test_get_by_user_group_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_USER_GROUP % self.ID_USER_GROUP_VALID)
        valid_response(response)

    def test_get_by_user_group_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_USER_GROUP % self.ID_USER_GROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_get_by_user_group_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_USER_GROUP % self.ID_USER_GROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_get_by_equipment_group_valid(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.ID_EQUIPMENT_GROUP_VALID)
        valid_response(response)

    def test_get_by_equipment_group_no_permission(self):
        response = self.client_no_permission().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.ID_EQUIPMENT_GROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_get_by_equipment_group_no_read_permission(self):
        response = self.client_no_read_permission().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.ID_EQUIPMENT_GROUP_VALID)
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_get_by_user_group_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_USER_GROUP % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.UGROUP_NOT_FOUND)

    def test_get_by_user_group_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_USER_GROUP % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_get_by_user_group_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_USER_GROUP % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_get_by_user_group_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_USER_GROUP % self.ZERO_ATTR)
        self._attr_invalid(response)

    def test_get_by_equipment_group_nonexistent(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.ID_NONEXISTENT)
        self._not_found(response, CodeError.EQUIPMENT_GROUP_NOT_FOUND)

    def test_get_by_equipment_group_negative(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.NEGATIVE_ATTR)
        self._attr_invalid(response)

    def test_get_by_equipment_group_letter(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.LETTER_ATTR)
        self._attr_invalid(response)

    def test_get_by_equipment_group_zero(self):
        response = self.client_autenticado().get(self.URL_GET_BY_EQUIPMENT_GROUP % self.ZERO_ATTR)
        self._attr_invalid(response)


class EquipmentGroupRightsRemoveTest(EquipmentGroupRightsConfigTest, RemoveTest):
