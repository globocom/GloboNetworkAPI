    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if ugroup_map is None:
                return self.response_error(
                    3,
                    u'There is no user_group tag in request XML.')
            ...
            if not is_valid_yes_no_choice(remove):
                self.log.error(
                    u'Parameter remove is invalid. Value: %s',
                    remove)
            ...
            try:
                ...
                raise UGrupoNameDuplicatedError(
                    None,
                    u'User group with name %s already exists' %
                    name)
            ...
            return self.response(
                dumps_networkapi({'user_group': {'id': ugroup.id}}))
