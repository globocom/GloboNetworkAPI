    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(egroup_id):
                self.log.error(u'The egroup_id parameter is not a valid value: %s.', egroup_id)
...
class DireitoGrupoEquipamentoResource(RestResource):
    ...
    def handle_get(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(right_id, False):
                self.log.error(u'The right_id parameter is not a valid value: %s.', right_id)
                raise InvalidValueError(None, 'right_id', right_id)
            
            if right_id is not None:
                map_list.append(self.__get_direito_map(DireitosGrupoEquipamento.get_by_pk(right_id)))
            else:
                ...
                if not is_valid_int_greater_zero_param(ugroup, False):
                    self.log.error(u'The ugroup_id parameter is not a valid value: %s.', ugroup)
                    raise InvalidValueError(None, 'ugroup_id', ugroup)

                if not is_valid_int_greater_zero_param(egroup, False):
                    self.log.error(u'The egroup_id parameter is not a valid value: %s.', egroup)
    ...
    def __valida_id_ugrupo(self, id_grupo):
        ...
        try:
            ...
            self.log.error(u'Valor do id_grupo_usuario inválido: %s.', id_grupo)
    ...
    def __valida_id_egrupo(self, id_grupo):
        ...
        try:
            ...
            self.log.error(u'Valor do id_grupo_equipamento inválido: %s.', id_grupo)
            return self.response_error(102)
    ...
    def __valida_request(self, direito_map, handle_post=True):
        ...
        elif (read == "0"): direito_map['leitura'] = False
        elif (read == "1"): direito_map['leitura'] = True
        else: raise InvalidValueError(None, 'leitura', read)

        write = direito_map.get('escrita')
        if write is None:
            raise InvalidValueError(None, 'escrita', write)
        elif (write == "0"): direito_map['escrita'] = False
        elif (write == "1"): direito_map['escrita'] = True
        else: raise InvalidValueError(None, 'escrita', write)
    
        update_config = direito_map.get('alterar_config')
        if update_config is None:
            raise InvalidValueError(None, 'alterar_config', update_config)
        elif (update_config == "0"): direito_map['alterar_config'] = False
        elif (update_config == "1"): direito_map['alterar_config'] = True
        else: raise InvalidValueError(None, 'alterar_config', update_config)

        delete = direito_map.get('exclusao')
        if delete is None:
            raise InvalidValueError(None, 'exclusao', delete)
        elif (delete == "0"): direito_map['exclusao'] = False
        elif (delete == "1"): direito_map['exclusao'] = True
        else: raise InvalidValueError(None, 'exclusao', delete)
