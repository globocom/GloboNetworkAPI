    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(id_permission):
                self.log.error(u'The id_permission parameter is not a valid value: %s.', id_permission)
            ...
            if not is_valid_boolean_param(read):
                self.log.error(u'The read parameter is not a valid value: %s.', read)
                raise InvalidValueError(None, 'read', read)
            
            # Valid Read
            if not is_valid_boolean_param(write):
                self.log.error(u'The write parameter is not a valid value: %s.', write)
            ...
            try:
                PermissaoAdministrativa.get_permission_by_permission_ugroup(id_permission, id_group)
                raise PermissaoAdministrativaDuplicatedError(None, permission.function)
            ...
            try:
                ...
                self.log.error(u'Failed to save the administrative permission.')
                raise GrupoError(e, u'Failed to save the administrative permission.')
            
            perm_map = dict()
            perm_map['perm'] = model_to_dict(adm_perm, exclude=["permission", "leitura", "escrita", "ugrupo"])
