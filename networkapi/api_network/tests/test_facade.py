...
from networkapi.api_network.facade.v1 import deploy_networkIPv4_configuration
from networkapi.api_network.facade.v1 import deploy_networkIPv6_configuration
...
class NetworkFacadeTestCase(unittest.TestCase):
    ...
    def test_deploy_networkipv4_configuration_given_network_already_active(self):
        ...
        response = deploy_networkIPv4_configuration(
            self.user, self.networkv4, self.equipment_list)
        self.assertEqual('Network already active. Nothing to do.', response[
                         'output'], 'Nothing to be done.No actions should me taken')
    ...
    def test_deploy_networkipv4_configuration_given_gateway_ip_not_found(self):
        ...
        with self.assertRaises(IncorrectRedundantGatewayRegistryException):
            deploy_networkIPv4_configuration(
                self.user, self.networkv4, self.equipment_list)
    ...
    def test_deploy_networkipv4_configuration_with_active_vlan(self):
        ...
        self.mock_find_roteiro(EquipamentoRoteiro(
    ...
    def test_deploy_networkipv6_configuration_given_network_already_active(self):
        ...
        response = deploy_networkIPv6_configuration(
            self.user, self.networkv6, self.equipment_list)
        self.assertEqual('Network already active. Nothing to do.', response[
                         'output'], 'Nothing to be done.No actions should me taken')
    ...
    def test_deploy_networkipv6_configuration_given_gateway_ip_not_found(self):
        ...
        with self.assertRaises(IncorrectRedundantGatewayRegistryException):
            deploy_networkIPv6_configuration(
                self.user, self.networkv6, self.equipment_list)
    ...
    def mock_distributed_lock(self):
        patch('networkapi.api_network.facade.v1.distributedlock').start()
        patch('networkapi.api_deploy.facade.distributedlock').start()

    def mock_ip_get_by_octets(self, ip):
        get_by_octs_and_net_mock = patch(
            'networkapi.ip.models.Ip.get_by_octs_and_net').start()
        get_by_octs_and_net_mock.return_value = ip

    def mock_ip_get_by_blocks_and_net(self, ipv6):
        get_by_octs_and_net_mock = patch(
            'networkapi.ip.models.Ipv6.get_by_blocks_and_net').start()
        get_by_octs_and_net_mock.return_value = ipv6

    def mock_find_equipamento_ip(self, ip_equipamento_list):
        find_ip_equipamento_mock = patch(
            'networkapi.ip.models.IpEquipamento.objects.filter').start()
        find_ip_equipamento_mock.return_value = ip_equipamento_list

    def mock_find_equipamento_ipv6(self, ip_equipamento_list):
        find_ip_equipamento_mock = patch(
            'networkapi.ip.models.Ipv6Equipament.objects.filter').start()
        find_ip_equipamento_mock.return_value = ip_equipamento_list

    def mock_find_roteiro(self, equip_roteiro):
        find_equip_roteiro_mock = patch(
            'networkapi.equipamento.models.EquipamentoRoteiro.search').start()
    ...
    def mock_template_file_read(self, template_content):
        open_mock = patch(
            'networkapi.api_network.facade.v1.open', create=True).start()
        file_handle = MagicMock()
        file_handle.read.return_value = template_content
        open_mock.return_value = file_handle

    def mock_deploy_config(self, response):
        deploy_config_mock = patch(
            'networkapi.api_network.facade.v1.deploy_config_in_equipment_synchronous').start()
    ...
    def mock_transaction(self):
        patch('networkapi.api_network.facade.v1.transaction').start()

    def mock_dhcp_relay_find(self, response):
        dhcp_relay_find_mock = patch(
            'networkapi.api_network.models.DHCPRelayIPv4.objects.filter').start()
