...
class NetworkIPv6AsyncDeployPostSuccessTestCase(NetworkApiTestCase):
    ...
    def test_task_id_create_in_post_deploy_netipv6_success(self, test_patch):
        """Test success of id task generate for netipv6 post deploy and
           success for deploy netipv6.
        """
        ...
        obj_id = 3
        response = tasks.deploy_networkv6.apply(args=[obj_id, self.user.id],
                                                queue='napi.network')
        id_task = response.id
        self.compare_values(36, len(id_task))

        response = self.client.get(
            '/api/v3/networkv6/3/?fields=active',
...
class NetworkIPv6AsyncDeployPostErrorTestCase(NetworkApiTestCase):
    ...
    @patch('networkapi.plugins.factory.PluginFactory.factory')
    def test_task_id_create_in_post_deploy_netipv6_error(self, test_patch):
        """Test success of id task generate for netipv6 post deploy and
           error for deploy netipv6.
        """
