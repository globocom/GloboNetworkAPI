...
class EquipmentEnvironmentTest(EquipmentEnvironmentConfigTest): 
    ...
    def test_save_valid(self):
        ...
        from_db = EquipamentoAmbiente.objects.get(equipamento=self.ID_EQUIPMENT_VALID,ambiente=self.ID_ENVIRONMENT_VALID)
...
class EquipmentEnvironmentUpdateTest(EquipmentEnvironmentConfigTest, AttrTest):
    ...
    def test_update_valid(self):
        ...
        from_db = EquipamentoAmbiente.objects.get(equipamento=mock['id_equipamento'],ambiente=mock['id_ambiente'])
    ...
    def test_update_nonexisten(self):
        ...
        response = self.client_autenticado().putXML(self.URL_UPDATE, { self.XML_KEY : mock })
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)


    def test_update_no_permission(self):
        mock = self.mock_update()
        response = self.client_no_permission().putXML(self.URL_UPDATE, { self.XML_KEY : mock })
        valid_response(response, httplib.PAYMENT_REQUIRED)


    def test_update_no_write_permission(self):
        mock = self.mock_update()
        response = self.client_no_write_permission().putXML(self.URL_UPDATE, { self.XML_KEY : mock })
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class EquipmentEnvironmentRemoveTest(EquipmentEnvironmentConfigTest, RemoveTest):
    ...
    def test_remove_valid(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_ENVIRONMENT ))
        valid_response(response)
    
    def test_remove_no_permission(self):
        response = self.client_no_permission().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_ENVIRONMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_no_write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_ENVIRONMENT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_remove_equipment_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ID_NONEXISTENT, self.ID_ENVIRONMENT ))
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)
    
    def test_remove_equipment_negative(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.NEGATIVE_ATTR, self.ID_ENVIRONMENT ))
        self._attr_invalid(response)

    def test_remove_equipment_letter(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.LETTER_ATTR, self.ID_ENVIRONMENT ))
        self._attr_invalid(response)

    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ZERO_ATTR, self.ID_ENVIRONMENT ))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.EMPTY_ATTR, self.ID_ENVIRONMENT ))
        self._attr_invalid(response)
    
    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.NONE_ATTR, self.ID_ENVIRONMENT ))
        self._attr_invalid(response)
