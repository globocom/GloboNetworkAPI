...
class EquipmentScriptConsultationTest(
        EquipmentScriptConfigTest,
        ConsultationTest):
...
class EquipmentScriptByEquipNameConsultationTest(
        EquipmentScriptConfigTest,
        ConsultationTest):
    ...
    def test_get_by_equip_name_valid(self):
        ...
        from_db = EquipamentoRoteiro.objects.filter(
            equipamento__nome__iexact="Balanceador de Teste",
...
class EquipmentScriptRemoveTest(EquipmentScriptConfigTest, RemoveTest):
    ...
    def test_remove_valid(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.ID_EQUIPMENT, self.ID_SCRIPT))
        valid_response(response)
    ...
    def test_remove_equipment_nonexistent(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.ID_NONEXISTENT, self.ID_SCRIPT))
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_remove_equipment_negative(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.NEGATIVE_ATTR, self.ID_SCRIPT))
        self._attr_invalid(response)

    def test_remove_equipment_letter(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.LETTER_ATTR, self.ID_SCRIPT))
        self._attr_invalid(response)

    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.ZERO_ATTR, self.ID_SCRIPT))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.EMPTY_ATTR, self.ID_SCRIPT))
        self._attr_invalid(response)

    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.NONE_ATTR, self.ID_SCRIPT))
        self._attr_invalid(response)
