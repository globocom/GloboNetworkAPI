...
class EquipmentScriptByEquipNameConsultationTest(EquipmentScriptConfigTest, ConsultationTest):
    ...
    def test_get_by_equip_name_valid(self):
        ...
        response = self.client_autenticado().postXML(self.URL_GET_BY_EQUIP_NAME, data)
        valid_response(response)
        content = valid_content(response, "equipamento_roteiro", force_list=True)
        
        # Verify
        user = Usuario.get_by_user('TEST')
        from_db = EquipamentoRoteiro.objects.filter(equipamento__nome__iexact="Balanceador de Teste", equipamento__grupos__direitosgrupoequipamento__ugrupo__in=user.grupos.all(), equipamento__grupos__direitosgrupoequipamento__escrita='1')
        assert len(from_db) == len(content)
    
    def test_get_by_equip_name_no_permission(self):
        data = self.dict_for_get_by_equip_name()
        response = self.client_no_permission().postXML(self.URL_GET_BY_EQUIP_NAME, data)
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_get_by_equip_name_no_read_permission(self):
        data = self.dict_for_get_by_equip_name()
        response = self.client_no_read_permission().postXML(self.URL_GET_BY_EQUIP_NAME, data)
        valid_response(response, httplib.PAYMENT_REQUIRED)
...
class EquipmentScriptRemoveTest(EquipmentScriptConfigTest, RemoveTest):
    ...
    def test_remove_valid(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_SCRIPT ))
        valid_response(response)
    
    def test_remove_no_permission(self):
        response = self.client_no_permission().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_SCRIPT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)

    def test_remove_no_write_permission(self):
        response = self.client_no_write_permission().delete(self.URL_REMOVE % ( self.ID_EQUIPMENT, self.ID_SCRIPT ))
        valid_response(response, httplib.PAYMENT_REQUIRED)
    
    def test_remove_equipment_nonexistent(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ID_NONEXISTENT, self.ID_SCRIPT ))
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)
    
    def test_remove_equipment_negative(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.NEGATIVE_ATTR, self.ID_SCRIPT ))
        self._attr_invalid(response)

    def test_remove_equipment_letter(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.LETTER_ATTR, self.ID_SCRIPT ))
        self._attr_invalid(response)

    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.ZERO_ATTR, self.ID_SCRIPT ))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.EMPTY_ATTR, self.ID_SCRIPT ))
        self._attr_invalid(response)
    
    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(self.URL_REMOVE % ( self.NONE_ATTR, self.ID_SCRIPT ))
        self._attr_invalid(response)
