...
class EquipmentAccessConsultationTest(
        EquipmentAccessConfigTest,
        ConsultationTest):
...
class EquipmentAccessConsultationEquipmentTest(
        EquipmentAccessConfigTest,
        AttrTest):
    ...
    def test_get_by_equipment(self):
        ...
        response = self.client_autenticado().postXML(
            self.URL_GET_BY_EQUIP_NAME, {
                self.XML_KEY: equip_access_map})
        valid_response(response)
        content = valid_content(response, self.XML_KEY, True)
...
class EquipmentAccessConsultationEquipmentAccessTest(
        EquipmentAccessConfigTest,
        AttrTest):
    ...
    def test_get_by_equipment_access(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_EQUIP_ACCESS_ID %
            self.ID_VALID)
        valid_response(response)
        content = valid_content(response, self.XML_KEY, True)
...
class EquipmentAccessRemoveTest(EquipmentAccessConfigTest, RemoveTest):
    ...
    def test_remove_valid(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE %
            (self.ID_EQUIPMENT, self.ID_ACCESS))
        valid_response(response)
