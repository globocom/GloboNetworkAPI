...
class EquipmentConfigTest(BasicTestCase):
    ...
    def search_attr(self, dicts):
        ...
        response = self.client_autenticado().postXML(
            self.URL_FIND_EQUIPMENT, {
                self.XML_KEY_EQUIPMENT: mock})
        return response
...
class EquipmentConsultationNameTest(EquipmentConfigTest, ConsultationTest):
    ...
    def test_get_by_name_valid(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_NAME %
            self.EQUIPMENT_NAME_VALID)
        valid_response(response)
        valid_content(response, self.XML_KEY_EQUIPMENT)
...
class EquipmentConsultationEnvironmenTest(
        EquipmentConfigTest,
        ConsultationTest):

    def test_get_by_environment_valid(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_ENVIRONMENT %
    ...
    def test_get_by_equipment_type_letter(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_ENVIRONMENT %
            (self.ID_EQUIPMENT_TYPE_VALID, self.LETTER_ATTR))
        self._attr_invalid(response)

    def test_get_by_equipment_type_zero(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_ENVIRONMENT %
            (self.ID_EQUIPMENT_TYPE_VALID, self.ZERO_ATTR))
        self._attr_invalid(response)

    def test_get_by_equipment_type_empty(self):
        response = self.client_autenticado().get(
            self.URL_GET_BY_ENVIRONMENT %
            (self.ID_EQUIPMENT_TYPE_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)
...
class EquipmentRemoveGroupTest(EquipmentConfigTest, RemoveTest):
    ...
    def test_remove_equipment_valid(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_EQUIPMENT_GROUP %
    ...
    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_EQUIPMENT_GROUP %
            (self.ZERO_ATTR, self.ID_REMOVE_GROUP_VALID))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_EQUIPMENT_GROUP %
            (self.EMPTY_ATTR, self.ID_REMOVE_GROUP_VALID))
        self._attr_invalid(response)

    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_EQUIPMENT_GROUP %
            (self.NONE_ATTR, self.ID_REMOVE_GROUP_VALID))
        self._attr_invalid(response)
...
class EquipmentRemoveIpv4AssocTest(EquipmentConfigTest, RemoveTest):
    ...
    def test_remove_ip_valid(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.ID_VALID))
        valid_response(response)
    ...
    def test_remove_ip_nonexistent(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_NONEXISTENT, self.ID_VALID))
        self._not_found(response, CodeError.IP_NOT_FOUND)

    def test_remove_ip_negative(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.NEGATIVE_ATTR, self.ID_VALID))
        self._attr_invalid(response)

    def test_remove_ip_letter(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.LETTER_ATTR, self.ID_VALID))
        self._attr_invalid(response)

    def test_remove_ip_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ZERO_ATTR, self.ID_VALID))
        self._attr_invalid(response)

    def test_remove_ip_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.EMPTY_ATTR, self.ID_VALID))
        self._attr_invalid(response)

    def test_remove_ip_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.NONE_ATTR, self.ID_VALID))
        self._attr_invalid(response)

    def test_remove_equipment_nonexistent(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.ID_NONEXISTENT))
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_remove_equipment_negative(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.NEGATIVE_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_letter(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.LETTER_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.ZERO_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV4 %
            (self.ID_IPV4_VALID, self.NONE_ATTR))
        self._attr_invalid(response)
...
class EquipmentRemoveIpv6AssocTest(EquipmentConfigTest, RemoveTest):
    ...
    def test_remove_ip_valid(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.ID_EQUIPMENT_IPV6_VALID))
        valid_response(response)
    ...
    def test_remove_ip_negative(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.NEGATIVE_ATTR, self.ID_EQUIPMENT_IPV6_VALID))
        self._attr_invalid(response)

    def test_remove_ip_letter(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.LETTER_ATTR, self.ID_EQUIPMENT_IPV6_VALID))
        self._attr_invalid(response)

    def test_remove_ip_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ZERO_ATTR, self.ID_EQUIPMENT_IPV6_VALID))
        self._attr_invalid(response)

    def test_remove_ip_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.EMPTY_ATTR, self.ID_EQUIPMENT_IPV6_VALID))
        self._attr_invalid(response)

    def test_remove_ip_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.NONE_ATTR, self.ID_EQUIPMENT_IPV6_VALID))
        self._attr_invalid(response)

    def test_remove_equipment_nonexistent(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.ID_NONEXISTENT))
        self._not_found(response, CodeError.EQUIPMENT_NOT_FOUND)

    def test_remove_equipment_negative(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.NEGATIVE_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_letter(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.LETTER_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_zero(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.ZERO_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_empty(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)

    def test_remove_equipment_none(self):
        response = self.client_autenticado().delete(
            self.URL_REMOVE_ASSOC_IPV6 %
            (self.ID_IPV6_VALID, self.NONE_ATTR))
        self._attr_invalid(response)
...
class EquipmentTestIp(EquipmentConfigTest, RemoveTest):
    ...
    def test_save_ipv4_valid(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV4 %
            (self.ID_IPV4_ASSOC_EQUIPMENT_VALID, self.ID_VALID))
        valid_response(response)
    ...
    def test_save_equipment_ipv4_zero(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV4 %
            (self.ID_IPV4_ASSOC_EQUIPMENT_VALID, self.ZERO_ATTR))
        self._attr_invalid(response)

    def test_save_equipment_ipv4_empty(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV4 %
            (self.ID_IPV4_ASSOC_EQUIPMENT_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)

    def test_save_equipment_ipv4_none(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV4 %
            (self.ID_IPV4_ASSOC_EQUIPMENT_VALID, self.NONE_ATTR))
        self._attr_invalid(response)

    def test_save_ipv6_valid(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.ID_IPV6_ASSOC_EQUIPMENT_VALID,
             self.ID_EQUIPMENT_IPV6_ASSOC_VALID))
        valid_response(response)
    ...
    def test_save_ipv6_equipment_zero(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.ZERO_ATTR, self.ID_EQUIPMENT_IPV6_ASSOC_VALID))
        self._attr_invalid(response)

    def test_save_ipv6_equipment_empty(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.EMPTY_ATTR, self.ID_EQUIPMENT_IPV6_ASSOC_VALID))
        self._attr_invalid(response)

    def test_save_ipv6_equipment_none(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.NONE_ATTR, self.ID_EQUIPMENT_IPV6_ASSOC_VALID))
        self._attr_invalid(response)
    ...
    def test_save_equipment_ipv6_zero(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.ID_IPV6_ASSOC_EQUIPMENT_VALID, self.ZERO_ATTR))
        self._attr_invalid(response)

    def test_save_equipment_ipv6_empty(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.ID_IPV6_ASSOC_EQUIPMENT_VALID, self.EMPTY_ATTR))
        self._attr_invalid(response)

    def test_save_equipment_ipv6_none(self):
        response = self.client_autenticado().put(
            self.URL_ADD_ASSOC_IPV6 %
            (self.ID_IPV6_ASSOC_EQUIPMENT_VALID, self.NONE_ATTR))
        self._attr_invalid(response)
...
class EquipmentGroupTest(EquipmentConfigTest):
    ...
    def test_save_valid(self):
        ...
        response = self.client_autenticado().postXML(
            self.URL_ADD_ASSOC_GROUP, {
                self.XML_KEY_GROUP: mock})
        valid_response(response)
