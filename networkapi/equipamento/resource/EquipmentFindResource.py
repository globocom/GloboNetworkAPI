...
def break_ip(ip):
    ...
    if version == IP_VERSION.IPv4[0]:
        ...
        ip_str = blocks[0] + ":" + blocks[1] + ":" + blocks[2] + ":" + blocks[3] + ":" + blocks[4] + ":" + blocks[5] + ":" + blocks[6] + ":" + blocks[7]
...
def prepares_equips(equip):
    ...
    for ipe in equip.ipequipamento_set.all():
        ...
        ip_dict = dict()
        ip_dict["ip"] = str(ipp.oct1) + "." + str(ipp.oct2) + "." + str(ipp.oct3) + "." + str(ipp.oct4)
        ip_dict["vlan"] = ipp.networkipv4.vlan.nome
        ip_dict["ambiente"] = ipp.networkipv4.vlan.ambiente.divisao_dc.nome + "-" + ipp.networkipv4.vlan.ambiente.ambiente_logico.nome + "-" + ipp.networkipv4.vlan.ambiente.grupo_l3.nome
    ...
    for ipv6e in equip.ipv6equipament_set.all():
        ...
        ipv6_dict = dict()
        ipv6_dict["ip"] = ipp.block1 + ":" + ipp.block2 + ":" + ipp.block3 + ":" + ipp.block4 + ":" + ipp.block5 + ":" + ipp.block6 + ":" + ipp.block7 + ":" + ipp.block8
        ipv6_dict["vlan"] = ipp.networkipv6.vlan.nome
        ipv6_dict["ambiente"] = ipp.networkipv6.vlan.ambiente.divisao_dc.nome + "-" + ipp.networkipv6.vlan.ambiente.ambiente_logico.nome + "-" + ipp.networkipv6.vlan.ambiente.grupo_l3.nome
    ...
    for env in equip.equipamentoambiente_set.all():
        if not env.ambiente.id in env_list:
            ...
            ambiente= Ambiente.objects.select_related().get(id=env.ambiente.id)
            
            ip_dict = dict()
            ip_dict["ip"] = "-"
            ip_dict["vlan"] = "-"
            ip_dict["ambiente"] = ambiente.divisao_dc.nome + "-" + ambiente.ambiente_logico.nome + "-" + ambiente.grupo_l3.nome
            ips_list.append(ip_dict)
        ...
        try:
            ...
            if environment is not None:
                if not is_valid_int_greater_zero_param(environment, False):
                    ...
                    equip = equip.filter(equipamentoambiente__ambiente__pk=environment)
                    
            if equip_type is not None:
                # If equip_type is valid, add to filter
                if not is_valid_int_greater_zero_param(equip_type, False):
                    raise InvalidValueError(None, 'tipo_equipamento', equip_type)
                else:
            ...
            if ip is not None:
                ...
                if not is_valid_string_minsize(ip, 1, False):
                    ...
                    if version == IP_VERSION.IPv4[0]:
                        ...
                        if len(blocks[0]) != 0:
                            oct1 = Q(ipv6equipament__ip__block1__iexact=blocks[0])
                        if len(blocks[1]) != 0:
                            oct2 = Q(ipv6equipament__ip__block2__iexact=blocks[1])
                        if len(blocks[2]) != 0:
                            oct3 = Q(ipv6equipament__ip__block3__iexact=blocks[2])
                        if len(blocks[3]) != 0:
                            oct4 = Q(ipv6equipament__ip__block4__iexact=blocks[3])
                        if len(blocks[4]) != 0:
                            oct5 = Q(ipv6equipament__ip__block5__iexact=blocks[4])
                        if len(blocks[5]) != 0:
                            oct6 = Q(ipv6equipament__ip__block6__iexact=blocks[5])
                        if len(blocks[6]) != 0:
                            oct7 = Q(ipv6equipament__ip__block7__iexact=blocks[6])
                        if len(blocks[7]) != 0:
                            oct8 = Q(ipv6equipament__ip__block8__iexact=blocks[7])
                            
                        equip = equip.filter(oct1 & oct2 & oct3 & oct4 & oct5 & oct6 & oct7 & oct8)
                        
            equip = equip.distinct()
            
            # Datatable paginator
            equip, total = build_query_to_datatable(equip, asorting_cols, custom_search, searchable_columns, start_record, end_record)
