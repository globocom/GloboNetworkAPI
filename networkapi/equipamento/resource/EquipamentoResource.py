...
def insert_equipment(equipment_map, user):
    ...
    if not is_valid_int_greater_zero_param(group_id):
        log.error(u'The group_id parameter is not a valid value: %s.', group_id)
    ...
    if not is_valid_int_greater_zero_param(equipment_type_id):
        log.error(u'The equipment_type_id parameter is not a valid value: %s.', equipment_type_id)
    ...
    if not is_valid_int_greater_zero_param(model_id):
        log.error(u'The model_id parameter is not a valid value: %s.', model_id)
        ...
        try:
            ...
            if equipment_id is not None:
                ...
                if not is_valid_int_greater_zero_param(equipment_id):
                    self.log.error(u'The equipment_id parameter is not a valid value: %s.', equipment_id)
                ...
                if not is_valid_int_greater_zero_param(equipment_type_id):
                    self.log.error(u'The equipment_type_id parameter is not a valid value: %s.', equipment_type_id)
                    raise InvalidValueError(None, 'equipment_type_id', equipment_type_id)

                Ambiente.get_by_pk(environment_id)
                TipoEquipamento.get_by_pk(equipment_type_id)

                equipments = Equipamento().search(None, equipment_type_id, environment_id, user.grupos.all())
            ...
            for equipment in equipments:
                ...
                equip_map['id_tipo_equipamento'] = equipment.tipo_equipamento.id
                equip_map['nome_tipo_equipamento'] = equipment.tipo_equipamento.tipo_equipamento
    ...
    def handle_delete(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            with distributedlock(LOCK_EQUIPMENT % equipment_id):
                ...
                ip_equipamento_list = IpEquipamento.objects.filter(equipamento = equipment_id)
                ip6_equipamento_list = Ipv6Equipament.objects.filter(equipamento = equipment_id)
...
class EquipamentoAmbienteResource(RestResource):
    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            equip_environment = EquipamentoAmbiente(equipamento=Equipamento(id=equip_id),
                                                    ambiente=Ambiente(id=environment_id),
                                                    is_router=is_router)
    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            try:
                equip_environment = EquipamentoAmbiente().get_by_equipment_environment(equip_id, environment_id)
                equip_environment.is_router = is_router
                equip_environment.save(user)

            except EquipamentoAmbienteNotFoundError:
                self.log.warning(u'Falha ao alterar a associação equipamento/ambiente, associação não existe %s/%s.' % (equip_id, environment_id))
                pass
            except Exception, e:
                self.log.error(u'Falha ao alterar a associação equipamento/ambiente: %s/%s.' % (equip_id, environment_id)) 
                raise EquipamentoError(e, u'Falha ao alterar a associação equipamento/ambiente: %s/%s.' % (equip_id, environment_id))
