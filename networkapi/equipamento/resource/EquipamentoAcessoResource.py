    ...
    def handle_post(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            if not is_valid_int_greater_zero_param(id_equipamento):
                self.log.error(u'The id_equipamento parameter is not a valid value: %s.', id_equipamento)
                raise InvalidValueError(None, 'id_equipamento', id_equipamento)

            try:
                id_equipamento = int(id_equipamento)
            except (TypeError, ValueError):
                self.log.error(u'Valor do id_equipamento inválido: %s.', id_equipamento)
            ...
            if not is_valid_int_greater_zero_param(id_tipo_acesso):
                self.log.error(u'The id_tipo_acesso parameter is not a valid value: %s.', id_tipo_acesso)
                raise InvalidValueError(None, 'id_tipo_acesso', id_tipo_acesso)
            
            try:
                id_tipo_acesso = int(id_tipo_acesso)
            except (TypeError, ValueError):
                self.log.error(u'Valor do id_tipo_acesso inválido: %s.', id_tipo_acesso)
    ...
    def handle_put(self, request, user, *args, **kwargs):
        ...
        try:
            ...
            with distributedlock(LOCK_EQUIPMENT_ACCESS % id_tipo_acesso):
                ...
                # Altera a informação de acesso ao equipamentoconforme dados recebidos no XML
