...
class EquipAccessEditResource(RestResource):
    ...
    def handle_post(self, request, auth, *args, **kwargs):
        ...
        try:
            ...
            # Password must NOT be none and 20 is the maxsize and 3 is the minsize
            ...
            # Enable Pass must NOT be none and 20 is the maxsize and 3 is the minsize
            ...
            if not is_valid_int_greater_zero_param(type_access):
                self.log.error(u'Parameter type_access_id is invalid. Value: %s.', type_access)
            ...
            with distributedlock(LOCK_EQUIPMENT_ACCESS % type_access.id):
                ...
                equip_access.__dict__.update(fqdn=fqdn, user = user, password = password, enable_pass = enable_pass)
                ...
                equipmentaccess_map['equipamento_acesso'] = model_to_dict(equip_access)
