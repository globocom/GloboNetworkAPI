...
class Marca(BaseModel):
    ...
    def get_by_pk(cls, idt):
        ...
        try:
            ...
            raise MarcaNotFoundError(
                e,
                u'Dont there is a Brand by pk = %s.' %
                idt)
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise MarcaNotFoundError(
                e,
                u'Dont there is a Brand by name = %s.' %
                name)
        except Exception as e:
            cls.log.error(u'Failure to search the Brand.')
            raise EquipamentoError(e, u'Failure to search the Brand.')
...
class Modelo(BaseModel):
    ...
    def get_by_pk(cls, idt):
        ...
        try:
            ...
            raise ModeloNotFoundError(
                e,
                u'Dont there is a Model by pk = %s.' %
                idt)
        except Exception as e:
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise ModeloNotFoundError(
                e,
                u'Dont there is a Model by name = %s.' %
                name)
        except Exception as e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
    ...
    def get_by_name_brand(cls, name, id_brand):
        ...
        try:
            ...
            raise ModeloNotFoundError(
                e, u'Dont there is a Model by name = %s and Brand = %s.' %
                (name, id_brand))
        except Exception as e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
    ...
    def get_by_brand(cls, id_brand):
        ...
        try:
            ...
            raise ModeloNotFoundError(
                e,
                u'Dont there is a Model by Brand = %s.' %
                id_brand)
        except Exception as e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
...
class TipoEquipamento(BaseModel):
    ...
    def get_by_pk(self, idt):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(
                e,
                u'Dont there is a Equipment Type by pk = %s.' %
                idt)
        except Exception as e:
    ...
    def get_tipo(self, tipo):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(
                e,
                u'Dont there is a Equipment Type by type = %s.' %
                tipo)
        except Exception as e:
            self.log.error(u'Failure to search the Equipment Type.')
            raise EquipamentoError(e, u'Failure to search the Equipment Type.')
    ...
    def get_tipo_balanceador(self):
        ...
        """
        try:
            return TipoEquipamento.objects.get(
                tipo_equipamento__iexact="balanceador")
        except ObjectDoesNotExist as e:
            raise TipoEquipamentoNotFoundError(
                e,
                u'Dont there is a Equipment Type by type is balanceador.')
        except Exception as e:
            self.log.error(u'Failure to search the Equipment Type.')
            raise EquipamentoError(e, u'Failure to search the Equipment Type.')
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(
                e,
                u'Dont there is a Equipment Type by name = %s.' %
                name)
        except Exception as e:
    ...
    def search(self):
        try:
            ...
            raise EquipamentoError(
                e,
                u'Falha ao pesquisar os tipos de equipamentos.')
        ...
        try:
            ...
            try:
                ...
                raise TipoEquipamentoDuplicateNameError(
                    None,
                    u'Tipo equipamento %s, já cadastrado' %
                    (name))
            except ObjectDoesNotExist as e:
                pass
            ...
            raise EquipamentoError(
                e,
                u'Falha ao inserir tipo de equipamentos.')
...
class Equipamento(BaseModel):
    ...
    def get_next_name_by_prefix(cls, prefix):
        try:
            names = Equipamento.objects.filter(
                nome__istartswith=prefix).values_list(
                'nome',
                flat=True).distinct()
            if len(names) == 0:
                return '%s%d' % (prefix, 1)
            ...
            values = sorted([int(name[start_value:]) for name in names])
    ...
    def get_by_pk(cls, pk):
        ...
        try:
            ...
            raise EquipamentoNotFoundError(
                e,
                u'Dont there is a equipament by pk = %s.' %
                id)
    ...
    def get_by_name(cls, name):
        try:
            ...
            raise EquipamentoNotFoundError(
                e,
                u'Não existe um equipamento com o nome = %s.' %
                name)
        except Exception as e:
        ...
        try:
            ...
            if ugroups is not None:
                equips = equips.filter(
                    grupos__direitosgrupoequipamento__ugrupo__in=ugroups,
                    grupos__direitosgrupoequipamento__leitura='1')
...
class EquipamentoAmbiente(BaseModel):
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            raise EquipamentoError(
                e, u'Falha ao inserir a associação equipamento/ambiente: %s/%s.' %
                (self.equipamento.id, self.ambiente.id))

    def get_by_equipment_environment(self, equipment_id, environment_id):
        try:
            return EquipamentoAmbiente.objects.get(
                ambiente__id=environment_id,
                equipamento__id=equipment_id)
        except ObjectDoesNotExist as e:
            raise EquipamentoAmbienteNotFoundError(
                e, u'Não existe um equipamento_ambiente com o equipamento = %s e o ambiente = %s.' %
                (equipment_id, environment_id))
        except Exception as e:
            self.log.error(u'Falha ao pesquisar o equipamento-ambiente.')
            raise EquipamentoError(
                e,
                u'Falha ao pesquisar o equipamento-ambiente.')

    @classmethod
    def get_by_equipment(self, equipment_id):
        try:
            return EquipamentoAmbiente.objects.filter(
                equipamento__id=equipment_id)
        except ObjectDoesNotExist as e:
            raise EquipamentoAmbienteNotFoundError(
                e,
                u'Não existe um equipamento_ambiente com o equipamento = %s.' %
                equipment_id)
        except Exception as e:
            self.log.error(u'Falha ao pesquisar o equipamento-ambiente.')
            raise EquipamentoError(
    ...
    def remove(cls, authenticated_user, equip_id, environ_id):
        ...
        try:
            ...
            cls.log.error(
                ...
                (equip_id, environ_id))
            raise EquipamentoAmbienteNotFoundError(
                n, u'Não existe um equipamento_ambiente com o equipamento = %s e o ambiente = %s.' %
                (equip_id, environ_id))
        except Exception as e:
            cls.log.error(
...
class EquipamentoGrupo(BaseModel):
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            raise EquipamentoError(
                e, u'Falha ao inserir a associação equipamento/grupo: %d/%d.' %
                (self.equipamento.id, self.egrupo.id))

    @classmethod
    def get_by_equipment(self, equipment_id):
        try:
            return EquipamentoGrupo.objects.filter(
                equipamento__id=equipment_id)
        except ObjectDoesNotExist as e:
            raise EquipamentoGrupoNotFoundError(
                e,
                u'Não existe um equipamento_grupo com o equipamento = %s ' %
                (equipment_id))
        except Exception as e:
            self.log.error(u'Falha ao pesquisar o equipamento-grupo.')
            raise EquipamentoError(
                e,
                u'Falha ao pesquisar o equipamento-grupo.')

    def get_by_equipment_group(self, equipment_id, egroup_id):
        try:
            return EquipamentoGrupo.objects.get(
                egrupo__id=egroup_id,
                equipamento__id=equipment_id)
        except ObjectDoesNotExist as e:
            raise EquipamentoGrupoNotFoundError(
                e, u'Não existe um equipamento_grupo com o equipamento = %s e o grupo = %s.' %
                (equipment_id, egroup_id))
        except Exception as e:
            self.log.error(u'Falha ao pesquisar o equipamento-grupo.')
            raise EquipamentoError(
                e,
    ...
    def get_by_pk(self, id):
        ...
        try:
            ...
            raise EquipamentoAccessNotFoundError(
                e,
                u'Dont there is a EquipamentoAcesso by pk = %s.' %
                id)
        except OperationalError as e:
            ...
            raise EquipamentoError(
                e,
                u'Failure to search the EquipamentoAcesso.')
        ...
        try:
            ...
            if ugroups is not None:
                results = results.filter(
                    equipamento__grupos__direitosgrupoequipamento__ugrupo__in=ugroups,
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            if EquipamentoAcesso.objects.filter(
                    ...
                    tipo_acesso=self.tipo_acesso).count() > 0:
    ...
    def update(
        ...
        try:
            ...
            equipamento_acesso = EquipamentoAcesso.objects.get(
                equipamento__id=id_equipamento,
                tipo_acesso__id=id_tipo_acesso)
        ...
        try:
            try:
                EquipamentoRoteiro.objects.get(
                    equipamento__id=self.equipamento.id,
                    roteiro__id=self.roteiro.id)
            ...
            raise EquipamentoError(
                e, u'Falha ao inserir a associação equipamento/roteiro: %s/%s.' %
                (self.equipamento.id, self.roteiro.id))
