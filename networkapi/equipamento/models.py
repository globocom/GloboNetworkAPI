...
class Marca(BaseModel):
    ...
    def get_by_pk(cls, idt):
        ...
        try:
            ...
            raise MarcaNotFoundError(e, u'Dont there is a Brand by pk = %s.' % idt)
        except Exception, e:
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise MarcaNotFoundError(e, u'Dont there is a Brand by name = %s.' % name)
        except Exception, e:
            cls.log.error(u'Failure to search the Brand.')
            raise EquipamentoError(e, u'Failure to search the Brand.')
...
class Modelo(BaseModel):
    ...
    def get_by_pk(cls, idt):
        ...
        try:
            ...
            raise ModeloNotFoundError(e, u'Dont there is a Model by pk = %s.' % idt)
        except Exception, e:
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise ModeloNotFoundError(e, u'Dont there is a Model by name = %s.' % name)
        except Exception, e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
    ...
    def get_by_name_brand(cls, name, id_brand):
        ...
        try:
            ...
            raise ModeloNotFoundError(e, u'Dont there is a Model by name = %s and Brand = %s.' % ( name , id_brand ) )
        except Exception, e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
    ...
    def get_by_brand(cls, id_brand):
        ...
        try:
            ...
            raise ModeloNotFoundError(e, u'Dont there is a Model by Brand = %s.' %  id_brand )
        except Exception, e:
            cls.log.error(u'Failure to search the Model.')
            raise EquipamentoError(e, u'Failure to search the Model.')
...
class TipoEquipamento(BaseModel):
    ...
    def get_by_pk(self, idt):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(e, u'Dont there is a Equipment Type by pk = %s.' % idt)
        except Exception, e:
    ...
    def get_tipo(self, tipo):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(e, u'Dont there is a Equipment Type by type = %s.' % tipo)
        except Exception, e:
            self.log.error(u'Failure to search the Equipment Type.')
            raise EquipamentoError(e, u'Failure to search the Equipment Type.')
    ...
    def get_tipo_balanceador(self):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(e, u'Dont there is a Equipment Type by type is balanceador.')
        except Exception, e:
            self.log.error(u'Failure to search the Equipment Type.')
            raise EquipamentoError(e, u'Failure to search the Equipment Type.')
    ...
    def get_by_name(cls, name):
        ...
        try:
            ...
            raise TipoEquipamentoNotFoundError(e, u'Dont there is a Equipment Type by name = %s.' % name)
        except Exception, e:
    ...
    def search(self):
        try:
            ...
            raise EquipamentoError(e, u'Falha ao pesquisar os tipos de equipamentos.')
    ...
    def insert_new(self,authenticated_user, name):
        try:
            ...
            try:
                ...
                raise TipoEquipamentoDuplicateNameError(None,u'Tipo equipamento %s, já cadastrado' % (name))
            except  ObjectDoesNotExist, e:
                pass
            ...
            raise EquipamentoError(e, u'Falha ao inserir tipo de equipamentos.')
...
class Equipamento(BaseModel):
    ...
    def get_next_name_by_prefix(cls, prefix):
        try:
            names = Equipamento.objects.filter(nome__istartswith=prefix).values_list('nome', flat=True).distinct()
            if len(names) == 0:
                return '%s%d' % (prefix, 1)
    ...
    def create(self, authenticated_user, group_id):
        ...
        if group_id == EGrupo.GRUPO_EQUIPAMENTO_ORQUESTRACAO and self.tipo_equipamento.id != TipoEquipamento.TIPO_EQUIPAMENTO_SERVIDOR_VIRTUAL:
            raise InvalidGroupToEquipmentTypeError(None, u'Equipamento do grupo “Equipamentos Orquestração” somente poderá ser criado com tipo igual a “Servidor Virtual”.')
        
        self.tipo_equipamento = TipoEquipamento().get_by_pk(self.tipo_equipamento.id)
        
        self.modelo = Modelo.get_by_pk(self.modelo.id)
        
        try:
            self.get_by_name(self.nome)
            raise EquipamentoNameDuplicatedError(None, u'Equipamento com nome duplicado.')
    ...
    def get_by_pk(cls, pk):
        ...
        try:
            ...
            raise EquipamentoNotFoundError(e, u'Dont there is a equipament by pk = %s.' % id)
    ...
    def get_by_name(cls, name):
        try:
            ...
            raise EquipamentoNotFoundError(e, u'Não existe um equipamento com o nome = %s.' % name)
        except Exception, e:
    ...
    def search(self, equip_name=None, equip_type_id=None, environment_id=None, ugroups=None):
        try:
            ...
            if ugroups is not None:
                equips = equips.filter(grupos__direitosgrupoequipamento__ugrupo__in=ugroups, grupos__direitosgrupoequipamento__leitura='1')
            ...
            if environment_id is not None:
                equips = equips.filter(equipamentoambiente__ambiente__id=environment_id)
...
class EquipamentoAmbiente(BaseModel):
    ...
    def create(self, authenticated_user):
        ...
        try:
            exist = EquipamentoAmbiente().get_by_equipment_environment(self.equipamento.id, self.ambiente.id)
            raise EquipamentoAmbienteDuplicatedError(None, u'Equipamento já está cadastrado no ambiente.')
        ...
        try:
            ...
            self.log.error(u'Falha ao inserir a associação equipamento/ambiente: %s/%s.' % (self.equipamento.id, self.ambiente.id)) 
            raise EquipamentoError(e, u'Falha ao inserir a associação equipamento/ambiente: %s/%s.' % (self.equipamento.id, self.ambiente.id))
    ...
    def get_by_equipment_environment(self, equipment_id, environment_id):
        try:
            ...
            raise EquipamentoAmbienteNotFoundError(e, u'Não existe um equipamento_ambiente com o equipamento = %s e o ambiente = %s.' % (equipment_id, environment_id))
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o equipamento-ambiente.')
            raise EquipamentoError(e, u'Falha ao pesquisar o equipamento-ambiente.')
    ...
    def get_by_equipment(self, equipment_id):
        try:
            ...
            raise EquipamentoAmbienteNotFoundError(e, u'Não existe um equipamento_ambiente com o equipamento = %s.' % equipment_id)
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o equipamento-ambiente.')
            raise EquipamentoError(e, u'Falha ao pesquisar o equipamento-ambiente.')
    ...
    def remove(cls, authenticated_user, equip_id, environ_id):
        ...
        try:
            equipenvironment = EquipamentoAmbiente.objects.get(equipamento__id=equip_id, ambiente__id=environ_id)
            equipenvironment.delete(authenticated_user)
        
        except ObjectDoesNotExist, n:
            cls.log.error(u'Não existe um equipamento_ambiente com o equipamento = %s e o ambiente = %s.' % (equip_id, environ_id))
            raise EquipamentoAmbienteNotFoundError(n, u'Não existe um equipamento_ambiente com o equipamento = %s e o ambiente = %s.' % (equip_id, environ_id))
        except Exception, e:
            cls.log.error(u'Falha ao remover uma associação entre um Equipamento e um Ambiente.')
            raise EquipamentoError(e, u'Falha ao remover uma associação entre um Equipamento e um Ambiente.')
...
class EquipamentoGrupo(BaseModel):
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            raise EquipamentoGrupoDuplicatedError(None, u'Equipamento já está cadastrado no grupo.')
        ...
        try:
            ...
            self.log.error(u'Falha ao inserir a associação equipamento/grupo: %d/%d.' % (self.equipamento.id, self.egrupo.id)) 
            raise EquipamentoError(e, u'Falha ao inserir a associação equipamento/grupo: %d/%d.' % (self.equipamento.id, self.egrupo.id))
    ...
    def get_by_equipment(self, equipment_id):
        try:
            ...
            raise EquipamentoGrupoNotFoundError(e, u'Não existe um equipamento_grupo com o equipamento = %s ' % (equipment_id))
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o equipamento-grupo.')
            raise EquipamentoError(e, u'Falha ao pesquisar o equipamento-grupo.')
    ...
    def get_by_equipment_group(self, equipment_id, egroup_id):
        try:
            ...
            raise EquipamentoGrupoNotFoundError(e, u'Não existe um equipamento_grupo com o equipamento = %s e o grupo = %s.' % (equipment_id, egroup_id))
        except Exception, e:
            self.log.error(u'Falha ao pesquisar o equipamento-grupo.')
            raise EquipamentoError(e, u'Falha ao pesquisar o equipamento-grupo.')        
    ...
    def remove(cls, authenticated_user, equip_id, egroup_id):
        ...
        '''
        equip_group = EquipamentoGrupo().get_by_equipment_group(equip_id, egroup_id)
        ...
        if len(equipments) > 1:
            try:
                ...
                cls.log.error(u'Falha ao remover uma associação entre um Equipamento e um Grupo.')
                raise EquipamentoError(e, u'Falha ao remover uma associação entre um Equipamento e um Grupo.')
        else:
            raise EquipmentDontRemoveError(u'Failure to remove an association between an equipment and a group because the group is related only to a group.')
...
class EquipamentoAcesso(BaseModel):
    ...
    password = models.CharField(max_length=20, db_column='pass') # Field renamed because it was a Python reserved word. Field name made lowercase.
    ...
    def get_by_pk(self, id):
        ...
        try:
            ...
            raise EquipamentoAccessNotFoundError(e,u'Dont there is a EquipamentoAcesso by pk = %s.' % id) 
        except OperationalError, e:
            self.log.error(u'Lock wait timeout exceeded.')
            raise OperationalError(e, u'Lock wait timeout exceeded; try restarting transaction')
        except Exception, e:
            self.log.error(u'Failure to search the EquipamentoAcesso.')
            raise EquipamentoError(e, u'Failure to search the EquipamentoAcesso.')
    ...
    def search(cls, ugroups=None):
        ...
        try:
            ...
            if ugroups is not None:
                results = results.filter(equipamento__grupos__direitosgrupoequipamento__ugrupo__in=ugroups, equipamento__grupos__direitosgrupoequipamento__escrita='1')
            return results
        except Exception, e:
            cls.log.error(u'Falha ao pesquisar as informações de acesso a equipamentos.')
            raise EquipamentoError(e, u'Falha ao pesquisar oas informações de acesso a equipamentos.')       
    ...
    def create(self, authenticated_user):
        ...
        try:
            ...
            if EquipamentoAcesso.objects.filter(equipamento=self.equipamento, tipo_acesso=self.tipo_acesso).count() > 0:
                raise EquipamentoAccessDuplicatedError(None, u'Já existe esta associação de equipamento e tipo de acesso cadastrada.')
            ...
            self.log.error(u'Falha ao inserir informação de acesso a equipamento.')
            raise EquipamentoError(e, u'Falha ao inserir informação de acesso a equipamento.')        
    ...
    def update(cls, authenticated_user, id_equipamento, id_tipo_acesso, **kwargs):
        ...
        try:
            ...
            equipamento_acesso = EquipamentoAcesso.objects.get(equipamento__id=id_equipamento, tipo_acesso__id=id_tipo_acesso)
            ...
            cls.log.error(u'Falha ao alterar informação de acesso a equipamento.')
            raise EquipamentoError(e, u'Falha ao alterar informação de acesso a equipamento.')    
    ...
    def remove(cls, authenticated_user, id_equipamento, id_tipo_acesso):
        ...
        try:
            ...
            EquipamentoAcesso.log.error(u'Falha ao excluir informação de acesso a equipamento.')
            raise EquipamentoError(e, u'Falha ao excluir informação de acesso a equipamento.')
...
class EquipamentoRoteiro(BaseModel):
    ...
    def search(cls, ugroups=None, equip_id=None):
        try:
            ...
            if ugroups is not None:
                er = er.filter(equipamento__grupos__direitosgrupoequipamento__ugrupo__in=ugroups, equipamento__grupos__direitosgrupoequipamento__leitura='1')
            ...
            cls.log.error(u'Falha ao pesquisar os equipamentos e roteiros associados.')
            raise EquipamentoError(e, u'Falha ao pesquisar os equipamentos e roteiros associados.')
    ...
    def create(self, authenticated_user):
        ...
        try:
            try:
                EquipamentoRoteiro.objects.get(equipamento__id=self.equipamento.id, roteiro__id=self.roteiro.id)
                raise EquipamentoRoteiroDuplicatedError(None, u'Equipamento já está associado ao roteiro.')
            ...
            self.log.error(u'Falha ao inserir a associação equipamento/roteiro: %s/%s.' % (self.equipamento.id, self.roteiro.id)) 
            raise EquipamentoError(e, u'Falha ao inserir a associação equipamento/roteiro: %s/%s.' % (self.equipamento.id, self.roteiro.id))
    ...
    def remove(cls, authenticated_user, equip_id, script_id):
        ...
        '''
        try:
            equip_script = EquipamentoRoteiro.objects.get(equipamento__id=equip_id, roteiro__id=script_id)
            equip_script.delete(authenticated_user)
        
        except EquipamentoRoteiro.DoesNotExist, n:
            cls.log.debug(u'Não existe um equipamento_roteiro com o equipamento = %s e o roteiro = %s.' % (equip_id, script_id))
            raise EquipamentoRoteiroNotFoundError(n, u'Não existe um equipamento_roteiro com o equipamento = %s e o roteiro = %s.' % (equip_id, script_id))
        except Exception, e:
            cls.log.error(u'Falha ao remover uma associação entre um Equipamento e um Roteiro.')
            raise EquipamentoError(e, u'Falha ao remover uma associação entre um Equipamento e um Roteiro.')
