...
class FilterEquipType(BaseModel):
    ...
    def validate(self):
        ...
        try:
            ...
            raise FilterEquipTypeDuplicateError(
                None,
                u'EquipType already registered for the Filter.')
    ...
    def delete(self, authenticated_user):
        ...
        envs_using_this_filter = [
            env.id for env in Ambiente.objects.filter(
        ...
        if len(envs_using_this_filter) > 0:
            ...
            filters_using_eq_tp = [
                fet.filter.id for fet in FilterEquipType.objects.filter(
                    equiptype=self.equiptype.id).exclude(
                    filter=self.filter.id)]

            # Get environments using other filters
            envs_using_filters = [
                env.id for env in Ambiente.objects.filter(
                    filter__in=filters_using_eq_tp)]

            # Get all networks from environments using this filter, grouped by
            # ip range
            this_filter_nets = NetworkIPv4.objects.filter(
                vlan__ambiente__in=envs_using_this_filter).values(
                'oct1',
                'oct2',
                'oct3',
                'oct4',
            ...
            for net in this_filter_nets:
                if net['count'] > 1:
                    ...
                    raise CantDissociateError(
                        {
                            'equiptype': self.equiptype.tipo_equipamento,
                            'filter_name': self.filter.name},
                        u'Cannot dissociate filter and equip type')

            # Get all networks from environments using other filters, grouped
            # by ip range
            other_filter_nets = NetworkIPv4.objects.filter(
                vlan__ambiente__in=envs_using_filters).values(
                'oct1',
                'oct2',
                'oct3',
                'oct4',
            ...
            # Get all networks from environments using this filter, grouped by
            # ip range
            this_filter_nets = NetworkIPv6.objects.filter(
                vlan__ambiente__in=envs_using_this_filter).values(
                'block1',
                'block2',
                'block3',
            ...
            # Get all networks from environments using other filters, grouped
            # by ip range
            other_filter_nets = NetworkIPv6.objects.filter(
                vlan__ambiente__in=envs_using_filters).values(
                'block1',
                'block2',
                'block3',
            ...
            vlans_in_environments = Vlan.objects.filter(
                ambiente__in=envs_using_this_filter +
                envs_using_filters).values('num_vlan').annotate(
                count=Count('id'))
            ...
            for vlan_group in vlans_in_environments:
                ...
                if vlan_group['count'] > 1:
                    ...
                    vlans_same_number = [
                        vlan.id for vlan in Vlan.objects.filter(
                            ambiente__in=envs_using_this_filter +
                            envs_using_filters,
                            num_vlan=vlan_group['num_vlan'])]
                    ...
                    if len(list(set(final_list))) != total_len:
                        ...
                        # filter
                        if len(
                            IpEquipamento.objects.filter(
                                ip__networkipv4__vlan__ambiente__in=envs_using_this_filter,
                                equipamento__in=equips)) > 0 or len(
                            Ipv6Equipament.objects.filter(
                                ip__networkipv6__vlan__ambiente__in=envs_using_this_filter,
                                equipamento__in=equips)) > 0:
                            self.log.error(
