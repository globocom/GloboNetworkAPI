...
from networkapi.api_pools.facade.facade_v1 import apply_health_check
from networkapi.api_pools.facade.facade_v1 import apply_priorities
from networkapi.api_pools.facade.facade_v1 import apply_service_down_action
from networkapi.api_pools.facade.facade_v1 import deploy_pool_member_config
from networkapi.api_pools.facade.facade_v1 import get_ip_objects
from networkapi.api_pools.facade.facade_v1 import get_or_create_healthcheck
from networkapi.api_pools.facade.facade_v1 import get_pool_members_to_be_removed
from networkapi.api_pools.facade.facade_v1 import MultipleObjectsReturned
from networkapi.api_pools.facade.facade_v1 import ObjectDoesNotExist
from networkapi.api_pools.facade.facade_v1 import prepare_to_save_reals
from networkapi.api_pools.facade.facade_v1 import remove_pool_members
from networkapi.api_pools.facade.facade_v1 import update_load_balancing_method
from networkapi.api_pools.facade.facade_v1 import update_pool_fields
from networkapi.api_pools.facade.facade_v1 import update_pool_maxconn
from networkapi.api_pools.facade.facade_v1 import update_pool_member
from networkapi.api_pools.facade.facade_v1 import validate_change_of_environment
...
class PoolFacadeTestCase(unittest.TestCase):
    ...
    def test_get_healthcheck_given_invalid_healthcheck_identifier(self):
        find_health_check_mock = self.mock_find_healthcheck(
            ObjectDoesNotExist())
        save_health_check_mock = self.mock_healthcheck_save()
        health_check = get_or_create_healthcheck(
            self.user, 'WORKING', 'HTTP', '/healthcheck', '*:*', '1')
    ...
    def test_get_healthcheck_given_multiple_healthcheck_found(self):
        healthcheck = self.create_health_check(
            expect='WORKING', type='HTTP', req='/healthcheck', dest='*:*', ident='1')
        find_health_check_mock = self.mock_find_healthcheck(
    ...
    def test_create_new_healthcheck(self):
        ...
        save_health_check_mock = self.mock_healthcheck_save()
        health_check = get_or_create_healthcheck(
            self.user, 'WORKING', 'HTTP', '/healthcheck', '*:*')
    ...
    def test_update_maxconn_given_created_pool_and_having_members(self):
        ...
        pool = self.mock_server_pool(
            maxconn=200, created=True, members=[member])
        new_max_conn = 500
        update_pool_maxconn(new_max_conn, 300, pool, self.user)
    ...
    def test_apply_health_check_given_script_error_output(self):
        ...
        with self.assertRaises(ScriptCreatePoolException):
            apply_health_check(
                new_healtcheck, pool.healthcheck, pool, self.user)
            self.assertNotEquals(new_healtcheck, pool.healthcheck)
            self.assertTrue(script_exec_mock.called)

    def test_apply_service_down_action_given_not_created_pool(self):
        pool = self.mock_server_pool(created=False)
        pool.servicedownaction = OptionPool(
            id=1, type='ServiceDownAction', name='none')
        new_servicedownaction = OptionPool(
            id=2, type='ServiceDownAction', name='drop')
        script_exec_mock = self.mock_exec_script(1)

        apply_service_down_action(
            pool.servicedownaction, new_servicedownaction, pool, self.user)
    ...
    def test_update_load_balancing_method_given_not_created_pool(self):
        ...
        update_load_balancing_method(
            new_lb_method, pool.lb_method, pool, self.user)
    ...
    def test_prepare_to_save_reals_given_invalid_priority(self):
        ...
        try:
            ...
            self.assertEquals(
                'Parametros invalidos do real: <<O valor da Prioridade deve estar entre 0 e 4294967295.>>', e.detail)
    ...
    def test_prepare_to_save_reals_given_invalid_port(self):
        ...
        try:
            ...
            self.assertEquals(
                'Parametros invalidos do real: <<O número da porta deve estar entre 1 e 65535.>>', e.detail)
    ...
    def test_prepare_to_save_reals_given_port_count_differs_from_equipment_count(self):
        ...
        id_equips = [1]
        try:
            prepare_to_save_reals([], ports, [], priorities,
                                  [], pool_member_ids, id_equips)
            self.fail()
        except InvalidRealPoolException, e:
            self.assertEquals(
                'Parametros invalidos do real: <<Quantidade de portas e equipamento difere.>>', e.detail)
    ...
    def test_prepare_to_save_reals_given_different_ips_with_matching_ports(self):
        ...
        id_equips = [1, 2]
        try:
            prepare_to_save_reals(ips, ports, [], priorities,
                                  [], pool_member_ids, id_equips)
            self.fail()
        except InvalidRealPoolException, e:
            self.assertEquals(
                'Parametros invalidos do real: <<Ips com portas iguais.>>', e.detail)
    ...
    def test_prepare_to_save_reals_given_valid_parameter_combination(self):
        ...
        reals = prepare_to_save_reals(
            ips, ports, equip_names, priorities, weights, pool_member_ids, id_equips)
    ...
    def test_update_pool_member(self):
        ...
        dict = {'nome_equips': 'equip_name',
                'weight': 1, 'priority': 1, 'port_real': 80}
        ip = Ip(id=1)
        ipv6 = Ipv6(id=1)
    ...
    def test_get_ipv6(self):
        ...
        ip = get_ip_objects(
            {'ip': '2001:0db8:85a3:08d3:1319:8a2e:0370:7344', 'id': 1})
    ...
    def test_get_pool_members_to_be_removed(self):
        list_server_pool_member = [
            {'id_pool_member': 1}, {'id_pool_member': ''}]
        self.assertEquals(
            1, len(get_pool_members_to_be_removed(list_server_pool_member)))
    ...
    def mock_find_healthcheck(self, response):
        mock = patch(
            'networkapi.healthcheckexpect.models.Healthcheck.objects.get').start()
    ...
    def mock_healthcheck_filter(self, healthchecks):
        mock = patch(
            'networkapi.healthcheckexpect.models.Healthcheck.objects.filter').start()
    ...
    def mock_exec_script(self, script_output):
        exec_script_mock = patch(
            'networkapi.api_pools.facade.facade_v1.exec_script').start()
