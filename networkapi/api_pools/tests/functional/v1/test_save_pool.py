...
class PoolSaveTestCase(unittest.TestCase):
    ...
    def test_save_pool_given_no_service_down_action_available(self):
        ...
        self.assertEquals(
            'Invalid value for Service-Down-Action.', response.data.get('detail'))
    ...
    def test_save_pool_given_invalid_service_down_action_id(self):
        ...
        response = self.client.post(
            '/api/pools/save/', self.load_pool_json('pool_with_service_down_action.json'), format='json')
        self.assertEquals(400, response.status_code)
        self.assertEquals(
            'Invalid value for Service-Down-Action.', response.data.get('detail'))
    ...
    def test_save_pool_given_invalid_identifier(self):
        ...
        response = self.client.post(
            '/api/pools/save/', self.load_pool_json('pool_with_invalid_identifier.json'), format='json')
        self.assertEquals(400, response.status_code)
        self.assertEquals(
            'The first character of the identifier field can not be a number.', response.data.get('detail'))
    ...
    def test_save_pool_given_invalid_health_check(self):
        ...
        response = self.client.post(
            '/api/pools/save/', self.load_pool_json('pool_with_invalid_health_check.json'), format='json')
        self.assertEquals(500, response.status_code)
        self.assertEquals('Failed to access the data source.',
                          response.data.get('detail'))
    ...
    def test_save_pool_given_pool_associated_with_one_or_more_vips(self):
        ...
        self.assertEquals(400, response.status_code)
        self.assertEquals(
            'Ambiente nao pode ser alterado pois o server pool esta associado com um ou mais VIP.', response.data.get('detail'))
        self.assertTrue(save_pool_mock.called)
    ...
    def test_save_pool_given_fail_on_execute_limit_script_for_pool(self):
        ...
        self.assertEquals(400, response.status_code)
        self.assertEquals(
            'Failed to execute limits script for pool.', response.data.get('detail'))
        self.assertTrue(save_pool_mock.called)
    ...
    def test_save_pool_given_fail_on_execute_create_pool_script(self):
        ...
        self.assertEquals(400, response.status_code)
        self.assertEquals(
            'Failed to execute create script for pool.', response.data.get('detail'))
        self.assertTrue(save_pool_mock.called)
...
# UTILS
    ...
    def mock_get_service_down_action_by_environment(self, option_pool):
        mock = patch(
            'networkapi.api_pools.models.OptionPool.get_all_by_type_and_environment').start()
    ...
    def mock_get_service_down_action_by_pk(self, option_pool):
        mock = patch(
            'networkapi.api_pools.models.OptionPool.get_by_pk').start()
        if option_pool is None:
    ...
    def mock_get_or_create_healthcheck(self, healthcheck):
        mock = patch(
            'networkapi.api_pools.views.v1.get_or_create_healthcheck').start()
    ...
    def mock_prepare_to_save_reals(self, response):
        mock = patch(
            'networkapi.api_pools.views.v1.prepare_to_save_reals').start()
        if(issubclass(response.__class__, Exception)):
            mock.side_effect = response
        else:
            mock.return_value = response
    ...
    def mock_reals_can_associate_server_pool(self, response):
        mock = patch(
            'networkapi.api_pools.views.v1.reals_can_associate_server_pool').start()
        if(issubclass(response.__class__, Exception)):
            mock.side_effect = response
        else:
            mock.return_value = response
    ...
    def mock_find_environment_by_id(self, environment):
        mock = patch(
            'networkapi.ambiente.models.Ambiente.objects.filter').start()
        mock.return_value = MagicMock()
    ...
    def mock_save_server_pool_member(self, response):
        mock = patch(
            'networkapi.api_pools.views.v1.save_server_pool_member').start()
        if(issubclass(response.__class__, Exception)):
            mock.side_effect = response
        else:
            mock.return_value = response
