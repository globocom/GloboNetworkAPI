...
UPDATE `server_pool` SET `identifier` = (
SELECT
CONCAT(
'VIP',
`requisicao_vips`.`id_requisicao_vips`,'_',
`ips`.`oct1`,'.',
`ips`.`oct2`,'.',
`ips`.`oct3`,'.',
`ips`.`oct4`,'_',
...
FROM `vip_port_to_pool`
INNER JOIN `requisicao_vips`
ON (`requisicao_vips`.`id_requisicao_vips` = `vip_port_to_pool`.`id_requisicao_vips`)
INNER JOIN `ips`
ON (`ips`.`id_ip` = `requisicao_vips`.`ips_id_ip`)
WHERE (`server_pool`.`id_server_pool` = `vip_port_to_pool`.`id_server_pool`)
)
WHERE `server_pool`.`identifier` = '' or `server_pool`.`identifier` is null
